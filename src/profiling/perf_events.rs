...
mod tests {
    ...
    fn hw_counters_debug_format() {
        ...
        let s = format!("{:?}", c);
        assert!(
            s.contains("cycles: 42"),
            "Debug output should contain cycles field"
        );
        assert!(
            s.contains("instructions: 100"),
            "Debug output should contain instructions field"
        );
        assert!(
            s.contains("l1d_misses: 5"),
            "Debug output should contain l1d_misses field"
        );
        assert!(
            s.contains("llc_misses: 1"),
            "Debug output should contain llc_misses field"
        );
        assert!(
            s.contains("branch_misses: 3"),
            "Debug output should contain branch_misses field"
        );
    }
    ...
    fn hw_counters_ipc_high_ratio() {
        ...
        assert!(
            ipc > 0.0,
            "IPC should be positive when instructions > 0 and cycles > 0"
        );
