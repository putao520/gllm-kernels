...
mod tests {
    ...
    #[test]
    fn gemv_workload_flops() {
        let w = gemv_workload(1024, 4096);
        match w {
            OpWorkload::Compute { flops } => assert_eq!(flops, 2 * 1024 * 4096),
            _ => panic!("expected Compute"),
        }
    }

    #[test]
    fn elementwise_unary_workload_bytes() {
        let w = elementwise_unary_workload(1024, 4);
        match w {
            OpWorkload::Memory { bytes } => assert_eq!(bytes, 2 * 1024 * 4),
            _ => panic!("expected Memory"),
        }
    }

    #[test]
    fn elementwise_binary_workload_bytes() {
        let w = elementwise_binary_workload(512, 4);
        match w {
            OpWorkload::Memory { bytes } => assert_eq!(bytes, 3 * 512 * 4),
            _ => panic!("expected Memory"),
        }
    }

    #[test]
    fn dot_workload_mixed() {
        let w = dot_workload(256, 4);
        match w {
            OpWorkload::Mixed { flops, bytes } => {
                assert_eq!(flops, 2 * 256);
                assert_eq!(bytes, 2 * 256 * 4);
            }
    ...
    #[test]
    fn rms_norm_workload_mixed() {
        let w = rms_norm_workload(4096, 4);
        match w {
            OpWorkload::Mixed { flops, bytes } => {
                assert_eq!(flops, 5 * 4096);
                assert_eq!(bytes, 3 * 4096 * 4);
            }
            _ => panic!("expected Mixed"),
        }
    }
    ...
    #[test]
    fn softmax_workload_mixed() {
        let w = softmax_workload(1024, 2);
        match w {
            OpWorkload::Mixed { flops, bytes } => {
                assert_eq!(flops, 5 * 1024);
                assert_eq!(bytes, 2 * 1024 * 2);
            }
            _ => panic!("expected Mixed"),
        }
    }
    ...
    #[test]
    fn quant_gemv_workload_mixed() {
        let w = quant_gemv_workload(1, 4096, 4096 / 2, 4);
        match w {
            OpWorkload::Mixed { flops, bytes } => {
                assert_eq!(flops, 2 * 1 * 4096);
                assert_eq!(bytes, 1 * (4096 / 2) as u64 + 4096 * 4);
            }
            _ => panic!("expected Mixed"),
        }
    }

    // ── OpWorkload Copy/Clone ────────────────────────────────────────────

    #[test]
    fn op_workload_copy() {
        let w = gemm_workload(64, 64, 64);
        let w2 = w;
        match w2 {
            OpWorkload::Compute { flops } => assert_eq!(flops, 2 * 64 * 64 * 64),
            _ => panic!("expected Compute"),
        }
    }
    ...
    #[test]
    fn hw_peak_manual_copy() {
        let peak = HwPeak::manual(200.0, 80.0);
        let peak2 = peak;
        assert!((peak2.gflops - 200.0).abs() < 1e-6);
        assert!((peak2.bandwidth_gbs - 80.0).abs() < 1e-6);
    }

    // ── PerfMetrics struct ───────────────────────────────────────────────

    #[test]
    fn perf_metrics_fields() {
        let m = PerfMetrics {
            elapsed_secs: 0.001,
            elapsed_cycles: 3000000,
            gflops: 50.0,
            bandwidth_gbs: 0.0,
            efficiency: 0.5,
        };
        assert!((m.elapsed_secs - 0.001).abs() < 1e-10);
        assert_eq!(m.elapsed_cycles, 3000000);
        assert!((m.gflops - 50.0).abs() < 1e-6);
    }
}
