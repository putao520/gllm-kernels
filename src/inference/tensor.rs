...
mod tests {
    ...
    // ── Test 7: DeviceKind variants and equality ──

    #[test]
    fn device_kind_variants_and_equality() {
        ...
        assert_eq!(DeviceKind::Cuda(2), DeviceKind::Cuda(2));
        assert_eq!(DeviceKind::Metal(0), DeviceKind::Metal(0));
    }

    // ── Test 8: DeviceKind Copy and Hash ──

    #[test]
    fn device_kind_copy_and_hash() {
        use std::collections::HashSet;
        let a = DeviceKind::Cpu;
        let b = a; // Copy
        assert_eq!(a, b);

        let mut set = HashSet::new();
        set.insert(DeviceKind::Cpu);
        set.insert(DeviceKind::Cuda(0));
        assert_eq!(set.len(), 2);
    }

    // ── Test 9: DeviceKind Debug format ──

    #[test]
    fn device_kind_debug_format() {
        assert!(format!("{:?}", DeviceKind::Cpu).contains("Cpu"));
        assert!(format!("{:?}", DeviceKind::Cuda(3)).contains("Cuda"));
        assert!(format!("{:?}", DeviceKind::Metal(1)).contains("Metal"));
    }

    // ── Test 10: alloc_cpu with F16 dtype ──

    #[test]
    fn alloc_cpu_f16_dtype() {
        let t = DeviceTensor::alloc_cpu(100, DType::F16).unwrap();
        assert_eq!(t.num_elements(), 100);
        assert_eq!(t.len_bytes(), 200); // F16 = 2 bytes each
        assert_eq!(t.dtype(), DType::F16);
    }

    // ── Test 11: from_mut_slice with f32 ──

    #[test]
    fn from_mut_slice_preserves_len() {
        let mut data = vec![0.0f32, 1.0, 2.0, 3.0];
        let t = unsafe { DeviceTensor::from_mut_slice(&mut data) };
        assert_eq!(t.num_elements(), 4);
        assert_eq!(t.len_bytes(), 16);
        assert_eq!(t.dtype(), DType::F32);
    }

    // ── Test 12: alloc_cpu produces zeroed memory ──

    #[test]
    fn alloc_cpu_produces_zeroed_memory() {
        let t = DeviceTensor::alloc_cpu(64, DType::F32).unwrap();
        let s: &[f32] = unsafe { t.as_slice() };
        for &v in s {
            assert_eq!(v, 0.0f32);
        }
    ...
    // ── Test 13: device() returns Cpu for alloc_cpu ──

    #[test]
    fn alloc_cpu_device_kind() {
        let t = DeviceTensor::alloc_cpu(10, DType::F32).unwrap();
        assert!(matches!(t.device(), DeviceKind::Cpu));
    }

    // ── Test 14: from_slice device is Cpu ──

    #[test]
    fn from_slice_device_is_cpu() {
        let data = vec![1.0f32];
        let t = unsafe { DeviceTensor::from_slice(&data) };
        assert!(t.is_cpu());
    }

    // ── Test 15: len_bytes consistency ──

    #[test]
    fn len_bytes_consistency() {
        let t = DeviceTensor::alloc_cpu(32, DType::F32).unwrap();
        assert_eq!(t.len_bytes(), t.num_elements() * t.dtype().size_bytes());
    }

    // ── Test 16: Drop frees memory without panic ──

    #[test]
    fn drop_frees_without_panic() {
        let t = DeviceTensor::alloc_cpu(256, DType::F32).unwrap();
        drop(t); // should not panic
    }
}
