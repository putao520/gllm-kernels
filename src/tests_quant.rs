...
mod tests {
    ...
    fn test_dequant_q2_k() {
        ...
        // Q2_K is 2-bit quantization with llama.cpp interleaved layout.
        // The macro-based decode iterates: for l in 0..16 { q = (qs[l] >> shift) & 3 }
        // So element l (within a 16-element sub-group) comes from qs[l]'s low 2 bits (shift=0).
        // Formula: out[idx] = d * (sc & 0xF) * q - dmin * (sc >> 4)
        // Set scale for sub-block 0: sc=1 (low nibble), m=0 (high nibble)
        block.scales[0] = 0x01;
        // Set q-values in separate bytes (interleaved layout: element i uses qs[i]'s low 2 bits)
        block.qs[0] = 3; // element 0: q=3
        block.qs[1] = 2; // element 1: q=2
        block.qs[2] = 1; // element 2: q=1
        block.qs[3] = 0; // element 3: q=0
    ...
    fn test_dequant_iq1_s_nonzero() {
        let kernels = CpuKernels::<f32>::new();
        // BlockIQ1S layout (66B): d(f16=2B) + qs[u8;32] + qh[u16;8=16B] + scales[u8;16]
        let mut block = vec![0u8; 66];
        let d_bytes = f16::from_f32(1.0).to_le_bytes();
        block[0] = d_bytes[0];
        block[1] = d_bytes[1];
        // Set qs[0] = 1 (grid index for group 0)
        block[2] = 1;
        // Set scales[0] = 0xFF (max scale so group_d = d * 1.0) at offset 50
        for i in 0..16 {
            block[50 + i] = 0xFF;
        }
        let mut out = vec![0.0f32; 256];
    ...
    fn test_dequant_iq2_s_nonzero() {
        ...
        // BlockIQ2S layout: d(2) + qs(64) + qh(8) + scales(8) = 82
        // Set qs[0] = 3 (grid index low byte)
        block[2] = 3;
        // scales starts at offset 2+64+8 = 74
        block[74] = 0xFF; // non-zero scale
        let mut out = vec![0.0f32; 256];
    ...
    fn test_dequant_iq3_s_nonzero() {
        ...
        // BlockIQ3S layout: d(2) + qs(64) + qh(8) + signs(32) + scales(4) = 110
    ...
    #[test]
    fn test_dequant_iq4_nl_nonzero() {
        let kernels = CpuKernels::<f32>::new();
        // BlockIQ4NL: d(f16=2B) + qs[u8;128] = 130 bytes, 256 elements
        let mut block = vec![0u8; 130];
        let d_bytes = half::f16::from_f32(1.0).to_le_bytes();
        block[0] = d_bytes[0];
        block[1] = d_bytes[1];
        // Set qs bytes to non-zero nibbles (0xAB = nibbles 0xB and 0xA)
        for i in 2..130 {
            block[i] = 0xAB;
        }
        let mut out = vec![0.0f32; 256];
    ...
    #[test]
    fn test_dequant_iq4_xs_nonzero() {
        let kernels = CpuKernels::<f32>::new();
        // BlockIQ4XS: d(f16=2B) + scales_h(u16=2B) + scales_l[u8;4] + qs[u8;128] = 136 bytes
        let mut block = vec![0u8; 136];
        let d_bytes = half::f16::from_f32(1.0).to_le_bytes();
        block[0] = d_bytes[0];
        block[1] = d_bytes[1];
        // Set scales_h to non-zero
        block[2] = 0x10;
        block[3] = 0x00;
        // Set scales_l to non-zero
        for i in 0..4 {
            block[4 + i] = 0xFF;
        }
        // Set qs to non-zero nibbles
        for i in 0..128 {
            block[8 + i] = 0xAB;
        }
        let mut out = vec![0.0f32; 256];
    ...
    fn test_iq_all_formats_nonzero() {
        ...
        let formats: Vec<(usize, Box<dyn Fn(&CpuKernels<f32>, &[u8], &mut [f32])>)> = vec![
            ...
            (130, Box::new(|k, b, o| k.dequant_iq4_nl(b, o))),
            (136, Box::new(|k, b, o| k.dequant_iq4_xs(b, o))),
        ];
