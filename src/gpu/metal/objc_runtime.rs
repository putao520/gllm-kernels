...
pub unsafe fn sel(name: &str) -> Sel {
    // SAFETY: ObjC selector names are ASCII identifiers; NUL bytes are a programming error
...
pub unsafe fn class(name: &str) -> Class {
    // SAFETY: ObjC class names are ASCII identifiers; NUL bytes are a programming error
