...
impl MetalDevice {
    ...
    /// Create a compute pipeline from a library function and launch it.
    pub fn launch_compute(
        ...
        unsafe {
            ...
            let ns_name = {
                let ns_class = objc_runtime::class("NSString");
                let alloc_sel = objc_runtime::sel("alloc");
                let init_sel = objc_runtime::sel("initWithUTF8String:");
                let c_name = CString::new(function_name)
                    .map_err(|e| GpuError::ShaderCompilation(format!("invalid name: {e}")))?;
                let ns_alloc: Id = objc_runtime::objc_msgSend(ns_class, alloc_sel);
                let ns: Id = objc_runtime::objc_msgSend(ns_alloc, init_sel, c_name.as_ptr());
                let sel = objc_runtime::sel("newFunctionWithName:");
                let func: Id = objc_runtime::objc_msgSend(library, sel, ns);
                objc_runtime::objc_msgSend(ns, objc_runtime::sel("release"));
                if func.is_null() {
                    return Err(GpuError::ShaderCompilation(format!(
                        "function '{function_name}' not found in library"
                    )));
                }
                func
            };
            ...
            // Create command buffer from stream's command queue
            let cmd_buf: Id = {
                let sel = objc_runtime::sel("commandBuffer");
                objc_runtime::objc_msgSend(stream.raw, sel)
            };
            if cmd_buf.is_null() {
                return Err(GpuError::Driver("failed to create command buffer".into()));
            }

            // [command_buffer computeCommandEncoder]
            let encoder: Id = {
                let sel = objc_runtime::sel("computeCommandEncoder");
                objc_runtime::objc_msgSend(cmd_buf, sel)
            };
            if encoder.is_null() {
                return Err(GpuError::Driver("failed to create compute encoder".into()));
            }

            // [encoder setComputePipelineState:]
            let sel = objc_runtime::sel("setComputePipelineState:");
            objc_runtime::objc_msgSend(encoder, sel, pipeline);

            // [encoder setBuffer:offset:atIndex:] for each buffer
            let buf_sel = objc_runtime::sel("setBuffer:offset:atIndex:");
            for (i, &(ptr, _size)) in buffers.iter().enumerate() {
                objc_runtime::objc_msgSend(encoder, buf_sel, ptr as u64, 0u64, i as u64);
            }
            ...
            objc_runtime::objc_msgSend(encoder, end_sel);
            ...
            // Check status
            let status_sel = objc_runtime::sel("status");
            let status: i64 = objc_runtime::objc_msgSend(cmd_buf, status_sel);
            if status != 0 {
                return Err(GpuError::Driver(format!(
                    "compute kernel failed with status {status}"
                )));
            }

            // Release pipeline
            let release_sel = objc_runtime::sel("release");
            objc_runtime::objc_msgSend(pipeline, release_sel);
