...
pub unsafe extern "C" fn gllm_tensor_alloc(
    ...
    let dt = match dtype {
        0 => crate::types::DType::F32,
        1 => crate::types::DType::F16,
        2 => crate::types::DType::BF16,
...
mod tests {
    ...
    #[test]
    fn test_gllm_status_debug_clone_copy() {
        // Arrange
        let status = GllmStatus::CompileError;
        // Act
        let debug_str = format!("{status:?}");
        let cloned = status.clone();
        let copied = status;
        // Assert
        assert_eq!(debug_str, "CompileError");
        assert_eq!(cloned, GllmStatus::CompileError);
        assert_eq!(copied, GllmStatus::CompileError);
    }

    #[test]
    fn test_gllm_status_all_variants_i32_values() {
        // Arrange — all 7 variants
        let variants = [
            (GllmStatus::Ok, 0),
            (GllmStatus::InvalidArg, -1),
            (GllmStatus::OutOfMemory, -2),
            (GllmStatus::CompileError, -3),
            (GllmStatus::RuntimeError, -4),
            (GllmStatus::Unsupported, -5),
            (GllmStatus::IoError, -6),
        ];
        // Act & Assert
        for (variant, expected) in variants {
            assert_eq!(variant as i32, expected);
        }
    }

    #[test]
    fn test_gllm_status_partial_eq_ne() {
        // Arrange
        let a = GllmStatus::Ok;
        let b = GllmStatus::RuntimeError;
        // Act & Assert
        assert_eq!(a, GllmStatus::Ok);
        assert_ne!(a, b);
        assert_ne!(b, GllmStatus::Ok);
    }

    #[test]
    fn test_gllm_weight_field_all_from_i32_roundtrip() {
        // Arrange — every single variant
        let all_fields: &[GllmWeightField] = &[
            GllmWeightField::Embedding,
            GllmWeightField::FinalNorm,
            GllmWeightField::LmHead,
            GllmWeightField::AttnNorm,
            GllmWeightField::Wq,
            GllmWeightField::Wk,
            GllmWeightField::Wv,
            GllmWeightField::Wo,
            GllmWeightField::FfnNorm,
            GllmWeightField::WGate,
            GllmWeightField::WUp,
            GllmWeightField::WDown,
            GllmWeightField::QkvBias,
            GllmWeightField::AttnNormBias,
            GllmWeightField::FfnNormBias,
        ];
        // Act & Assert
        for field in all_fields {
            let raw = *field as i32;
            assert_eq!(GllmWeightField::from_i32(raw), Some(*field));
        }
    }

    #[test]
    fn test_gllm_weight_field_debug_clone_copy() {
        // Arrange
        let field = GllmWeightField::WGate;
        // Act
        let debug_str = format!("{field:?}");
        let cloned = field.clone();
        // Assert
        assert_eq!(debug_str, "WGate");
        assert_eq!(cloned, GllmWeightField::WGate);
    }

    #[test]
    fn test_gllm_weight_field_global_boundary() {
        // Arrange — field 2 (LmHead) is last global, field 10 (AttnNorm) is first per-layer
        let last_global = GllmWeightField::LmHead;
        let first_per_layer = GllmWeightField::AttnNorm;
        // Act & Assert
        assert!(last_global.is_global());
        assert!(!first_per_layer.is_global());
        // Boundary: i32 values 2 < 10, so is_global checks (v < 10)
        assert!((last_global as i32) < 10);
        assert!((first_per_layer as i32) >= 10);
    }

    #[test]
    fn test_gllm_weight_field_from_i32_boundary_values() {
        // Arrange — gaps between global and per-layer fields (3..9 should be None)
    ...
    #[test]
    fn test_gllm_model_config_struct_update_syntax() {
        // Arrange
        ...
        assert_eq!(gemma_cfg.hidden_size, base.hidden_size);
        assert_eq!(gemma_cfg.vocab_size, base.vocab_size);
        assert_eq!(gemma_cfg.rope_theta, base.rope_theta);
    }

    #[test]
    fn test_gllm_model_config_debug_clone() {
        // Arrange
        let cfg = tiny_c_config();
        // Act
        ...
        // Assert
        assert!(debug_str.contains("GllmModelConfig"));
        assert_eq!(cloned.arch, cfg.arch);
        assert_eq!(cloned.hidden_size, cfg.hidden_size);
        assert_eq!(cloned.rope_theta, cfg.rope_theta);
        assert_eq!(cloned.norm_eps, cfg.norm_eps);
    }

    #[test]
    fn test_gllm_model_config_field_boundary_u32_max() {
        // Arrange — u32::MAX for u32 fields should survive round-trip to usize
        ...
        // Act
        let result = cfg.to_model_config();
        // Assert — conversion to ModelConfig should preserve the values (cast to usize)
        let mc = result.unwrap();
        assert_eq!(mc.hidden_size, u32::MAX as usize);
        assert_eq!(mc.num_heads, u32::MAX as usize);
        assert_eq!(mc.num_kv_heads, u32::MAX as usize);
        assert_eq!(mc.head_dim, u32::MAX as usize);
    }

    #[test]
    fn test_gllm_model_config_float_precision_special_values() {
        // Arrange — test with f64::MAX for rope_theta and f32::MIN_POSITIVE for norm_eps
        let cfg = GllmModelConfig {
            rope_theta: f64::MAX,
            norm_eps: f32::MIN_POSITIVE,
        ...
        // Assert
        assert_eq!(mc.rope_theta, f64::MAX);
        assert_eq!(mc.norm_eps, f32::MIN_POSITIVE);
        assert_eq!(mc.partial_rotary_factor, 0.0);
    }

    #[test]
    fn test_gllm_model_config_all_valid_architectures() {
        // Arrange — all 6 arch values (0..5)
        let arch_names = [0, 1, 2, 3, 4, 5];
        let expected = [
            crate::types::ModelArch::Llama,
            crate::types::ModelArch::Gpt2,
            crate::types::ModelArch::Mistral,
            crate::types::ModelArch::Phi,
            crate::types::ModelArch::Qwen,
            crate::types::ModelArch::Gemma,
        ];
        // Act & Assert
        for (arch_val, expected_arch) in arch_names.iter().zip(expected.iter()) {
            let mut cfg = tiny_c_config();
            cfg.arch = *arch_val;
            let mc = cfg.to_model_config().unwrap();
            assert_eq!(mc.arch, *expected_arch, "arch={arch_val} mismatch");
        }
    }

    #[test]
    fn test_gllm_model_config_has_qkv_bias_truthy() {
        // Arrange — any non-zero value should mean true
        let non_zero_values: &[i32] = &[1, 2, -1, 100, i32::MIN, i32::MAX];
        for bias_val in non_zero_values {
            let cfg = GllmModelConfig {
                has_qkv_bias: *bias_val,
                ..tiny_c_config()
            };
            // Act
            let mc = cfg.to_model_config().unwrap();
            // Assert
            assert!(mc.has_qkv_bias, "has_qkv_bias={bias_val} should be true");
        }
        // Zero should be false
        let cfg_zero = GllmModelConfig {
            has_qkv_bias: 0,
            ..tiny_c_config()
        };
        assert!(!cfg_zero.to_model_config().unwrap().has_qkv_bias);
    }

    // ── New tests (wave-12k84) ────────────────────────────────────────

    #[test]
    fn test_gllm_version_returns_non_null_static_string() {
        // Arrange — gllm_version returns a static C string
        // Act
        let ptr = gllm_version();
        // Assert — pointer is non-null and content is a valid UTF-8 C string
        assert!(!ptr.is_null());
    ...
    #[test]
    fn test_ffi_backend_init_null_config_returns_invalid_arg() {
        unsafe {
            // Arrange — null config pointer
            let mut backend: GllmBackend = std::ptr::null_mut();
            // Act
            let rc = gllm_backend_init(std::ptr::null(), &mut backend);
            // Assert
            assert_eq!(rc, GllmStatus::InvalidArg as i32);
        }
    }
    ...
    #[test]
    fn test_ffi_backend_init_null_out_returns_invalid_arg() {
        unsafe {
            // Arrange — valid config but null output pointer
            let cfg = tiny_c_config();
            // Act
            let rc = gllm_backend_init(&cfg, std::ptr::null_mut());
            // Assert
            assert_eq!(rc, GllmStatus::InvalidArg as i32);
        }
    }
    ...
    #[test]
    fn test_ffi_backend_free_null_is_safe() {
        // Arrange — null handle
        // Act — should not panic or crash
        unsafe { gllm_backend_free(std::ptr::null_mut()) };
        // Assert — reaching here means no crash
    }

    #[test]
    fn test_ffi_tensor_alloc_null_backend_returns_invalid_arg() {
        unsafe {
            // Arrange
            let mut tensor: GllmTensor = std::ptr::null_mut();
            // Act
            let rc = gllm_tensor_alloc(std::ptr::null_mut(), 16, 0, &mut tensor);
            // Assert
            assert_eq!(rc, GllmStatus::InvalidArg as i32);
        }
    }
    ...
    #[test]
    fn test_ffi_tensor_alloc_null_out_returns_invalid_arg() {
        unsafe {
            // Arrange — null out pointer
            // Act
            let rc = gllm_tensor_alloc(std::ptr::null_mut(), 16, 0, std::ptr::null_mut());
            // Assert
            assert_eq!(rc, GllmStatus::InvalidArg as i32);
        }
    }
    ...
    #[test]
    fn test_ffi_kv_cache_alloc_null_backend_returns_invalid_arg() {
        unsafe {
            // Arrange
            let mut cache: GllmKvCache = std::ptr::null_mut();
            // Act
            let rc = gllm_kv_cache_alloc(std::ptr::null_mut(), 1, 8, &mut cache);
            // Assert
            assert_eq!(rc, GllmStatus::InvalidArg as i32);
        }
    }
    ...
    #[test]
    fn test_ffi_kv_cache_alloc_null_out_returns_invalid_arg() {
        unsafe {
            // Arrange — null out pointer
            // Act
            let rc = gllm_kv_cache_alloc(std::ptr::null_mut(), 1, 8, std::ptr::null_mut());
            // Assert
            assert_eq!(rc, GllmStatus::InvalidArg as i32);
        }
    }
    ...
    #[test]
    fn test_gllm_model_config_dtype_all_valid_values() {
        // Arrange — dtype: 0=F32, 1=F16, 2=BF16
        let dtype_expected = [
            (0, crate::types::DType::F32),
            (1, crate::types::DType::F16),
            (2, crate::types::DType::BF16),
    ...
    #[test]
    fn test_gllm_model_config_quant_type_all_valid_values() {
        // Arrange — quant_type: -1=None, 0=Q4_0, 1=Q4_1, 2=Q8_0
        let quant_cases = [
            (-1i32, None),
    ...
    #[test]
    fn test_gllm_model_config_defaults_rope_interleaved_and_sliding_window() {
        // Arrange — verify fields not exposed via C ABI always get default values
        let cfg = tiny_c_config();
        // Act
        let mc = cfg.to_model_config().unwrap();
        // Assert
    ...
    #[test]
    fn test_ffi_tensor_upload_f32_null_pointers_return_invalid_arg() {
        unsafe {
            // Arrange — all null
            let rc = gllm_tensor_upload_f32(
                std::ptr::null_mut(),
                std::ptr::null(),
                0,
                std::ptr::null_mut(),
    ...
    #[test]
    fn test_ffi_tensor_download_f32_null_pointers_return_invalid_arg() {
        unsafe {
            // Arrange — all null
            let rc = gllm_tensor_download_f32(
                std::ptr::null_mut(),
                std::ptr::null_mut(),
                std::ptr::null_mut(),
                0,
