// DeFT / Talon Flash Tree-attention CUDA Kernel
...
//
// Ported from flash_tree_attn.hip
...
extern "C" __global__ void flash_tree_attention_forward_f16(
    ...
    const __half* q_ptr = Q + base + q_pos * head_dim;
    __half* o_ptr = O + base + q_pos * head_dim;
    ...
    for (int kv_pos = 0; kv_pos < total_len; ++kv_pos) {
        ...
        const __half* k_ptr = K + base + kv_pos * head_dim;
        float dot = 0.0f;
        for (int d = 0; d < head_dim; ++d) {
    ...
    for (int kv_pos = 0; kv_pos < total_len; ++kv_pos) {
        ...
        const __half* k_ptr = K + base + kv_pos * head_dim;
        const __half* v_ptr = V + base + kv_pos * head_dim;
    ...
    float inv_sum = (sum_exp > 0.0f) ? (1.0f / sum_exp) : 0.0f;
    for (int d = 0; d < head_dim; ++d) {
        o_ptr[d] = __float2half(out[d] * inv_sum);
    }
}
