// Paged attention kernel with online softmax for paged KV cache.
// This is a correctness-focused implementation that mirrors FlashAttention-style
// numerically stable accumulation while indexing paged KV blocks.
...
extern "C" __global__ void paged_attention_forward_f32(
    ...
    const int* __restrict__ BLOCK_OFFSETS, // [B] position offset for each sequence
...
extern "C" __global__ void paged_attention_forward_f16(
    const half* __restrict__ Q,           // [B, H, Q, D]
    const half* __restrict__ K_CACHE,     // [num_blocks, block_size, H, D]
    const half* __restrict__ V_CACHE,     // [num_blocks, block_size, H, D]
    const int* __restrict__ BLOCK_TABLES, // [B, KV] block id per token
