//! SM-aware binary loader for CUDA kernels (SASS/CUBIN preferred, PTX fallback).
//!
//! This module provides automatic binary selection based on the GPU's compute capability.
//! It supports:
//! - Multiple precompiled CUBIN (SASS) versions for different SM architectures
//! - Optional PTX fallback for compatibility (no runtime compilation)
...
//! - All binaries (CUBIN/PTX) must be precompiled and embedded at compile time
//! - Prefer SASS/CUBIN via `include_bytes!`
//! - NO NVRTC runtime compilation
//! - If no matching binary is found, return error (not fallback to compilation)
//!
...
pub enum PtxLoadError {
    ...
    /// No matching precompiled binary found (Fat Binary Only - no runtime compilation).
...
impl std::fmt::Display for PtxLoadError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::NoPtxAvailable(msg) => write!(f, "No binary available: {}", msg),
...
/// Binary collection for a kernel.
/// Contains precompiled CUBIN/PTX for multiple SM versions.
///
/// 🚨 **Fat Binary Only**: No runtime compilation support.
/// All binaries must be precompiled and embedded at compile time.
pub struct PtxCollection {
    /// Kernel name for logging.
    pub kernel_name: &'static str,
    /// Available CUBIN (SASS) versions: (sm_version, cubin_bytes).
    /// CUBIN must match the GPU SM exactly (no forward-compat).
    pub cubin_versions: &'static [(u32, &'static [u8])],
...
impl PtxCollection {
    /// Load the best matching binary for the given CUDA context.
    ///
    /// 🚨 **Fat Binary Only**: This method only loads precompiled binaries.
    /// - Prefer exact-match CUBIN (SASS).
    /// - Fallback to PTX if available (no runtime compilation).
    /// If no matching binary is found, it returns an error.
    pub fn load(&self, ctx: &Arc<CudaContext>) -> Result<Ptx, PtxLoadError> {
        ...
        // Step 2: Prefer exact-match CUBIN (SASS)
        if let Some((_, cubin_bytes)) = self
            .cubin_versions
            .iter()
            .find(|(sm, _)| *sm == gpu_sm)
        {
            if !cubin_bytes.is_empty() {
                log::info!(
                    "Loading {} CUBIN for sm_{}",
                    self.kernel_name, gpu_sm
                );
                return Ok(Ptx::from_binary(cubin_bytes.to_vec()));
            }
        }

        // Step 3: Fallback to PTX (forward-compatible)
        ...
        Err(PtxLoadError::NoPtxAvailable(format!(
            "No precompiled binary for {} kernel (GPU sm_{}). \
             Available PTX SM versions: {:?}. Fat Binary Only: runtime compilation is disabled.",
            self.kernel_name, gpu_sm, available_sms
        )))
    }
}

/// Macro to define a binary collection with embedded PTX files.
///
/// 🚨 **Fat Binary Only**: All binaries must be precompiled and embedded.
...
macro_rules! define_ptx_collection {
    ($name:ident, $kernel_name:literal, [ $(($sm:expr, $ptx:expr)),* $(,)? ]) => {
        pub static $name: $crate::cuda_kernels::binary_loader::PtxCollection =
            $crate::cuda_kernels::binary_loader::PtxCollection {
