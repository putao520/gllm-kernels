...
fn trace_op_flops(op: &TraceOp) -> u64 {
    match op {
        ...
        TraceOp::QuantQ5Decode { .. } => 8,
        TraceOp::QuantQ5KDecode { .. } => 8,
        TraceOp::QuantQ4KDecode { .. } => 8,
...
mod tests {
    ...
    fn estimate_fusion_cost_external_consumer_no_savings() {
        ...
        let add_op = g.add_op(Op::Add, vec![a, b], vec![add_out], "add");
        let tanh_op = g.add_op(Op::Tanh, vec![add_out], vec![tanh_out], "tanh");
        // External consumer of tanh_out
        g.add_op(Op::Tanh, vec![tanh_out], vec![ext_out], "external");
    ...
    fn chain_eliminated_bytes_empty_chain() {
        ...
        let a = g.add_tensor_concrete("a", &[1, 64], dt);
        let out = g.add_tensor_concrete("out", &[1, 64], dt);
        let anchor = g.add_op(Op::Tanh, vec![a], vec![out], "tanh");
    ...
    fn chain_eliminated_bytes_single_consumer() {
        ...
        let anchor = g.add_op(Op::Tanh, vec![a], vec![mid], "tanh");
        let chain_op = g.add_op(Op::Silu, vec![mid], vec![out], "silu");
    ...
    fn chain_eliminated_bytes_multi_consumer_not_eliminated() {
        ...
        let anchor = g.add_op(Op::Tanh, vec![a], vec![mid], "tanh");
        let chain_op = g.add_op(Op::Silu, vec![mid], vec![out1], "silu");
        // Second consumer of mid => mid is NOT eliminated
        g.add_op(Op::Silu, vec![mid], vec![out2], "silu2");
    ...
    fn chain_eliminated_bytes_multi_op_chain() {
        ...
        let anchor = g.add_op(Op::Tanh, vec![a], vec![mid1], "tanh");
        let c0 = g.add_op(Op::Silu, vec![mid1], vec![mid2], "silu");
        let c1 = g.add_op(Op::Tanh, vec![mid2], vec![out], "tanh2");
