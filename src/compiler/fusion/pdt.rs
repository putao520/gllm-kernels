...
mod tests {
    ...
    fn build_simple_graph() -> (CompilerGraph, SemanticDAG, PostDominatorTree) {
        ...
        g.add_op(
            OpKind::Gemm { m: SymDim::Concrete(1), n: 64, k: 64, dtype: dt, trans_b: false },
            vec![a, w],
            vec![gemm_out],
            "gemm",
        );
    ...
    fn test_score_fusion_zero_for_unrelated() {
        ...
        g.add_op(OpKind::Gemm { m: SymDim::Concrete(1), n: 64, k: 64, dtype: dt, trans_b: false }, vec![a, w], vec![out], "gemm");
        g.add_op(OpKind::Silu, vec![b], vec![c], "silu_unrelated");
    ...
    fn test_pdt_diamond_graph() {
        ...
        g.add_op(OpKind::Silu, vec![a], vec![b], "silu_b");
        g.add_op(OpKind::Silu, vec![a], vec![c], "silu_c");
        g.add_op(OpKind::Add, vec![b, c], vec![d_out], "add_d");
    ...
    fn test_compute_bytes_saved_no_shared_tensor() {
        ...
        g.add_op(OpKind::Silu, vec![b], vec![out2], "silu");
    ...
    fn test_discover_candidates_all_opaque_graph() {
        ...
        g.add_op(OpKind::CheckStopCondition, vec![logits], vec![token_out], "stop_check");
    ...
    fn test_discover_candidates_chain_fusion() {
        ...
        g.add_op(OpKind::Silu, vec![a], vec![silu_out], "silu");
        g.add_op(OpKind::Mul, vec![silu_out], vec![mul_out], "mul");
        g.add_op(OpKind::Add, vec![mul_out, b], vec![add_out], "add");
    ...
    fn test_discover_candidates_broken_chain() {
        ...
        g.add_op(OpKind::Silu, vec![a], vec![silu_out], "silu");
        g.add_op(OpKind::CheckStopCondition, vec![logits], vec![sc_out], "stop");
        g.add_op(OpKind::Gelu, vec![silu_out], vec![gelu_out], "gelu");
    ...
    fn pdt_fan_out_graph_ipostdom() {
        ...
        g.add_op(OpKind::Silu, vec![a], vec![b_out], "silu_b");
        g.add_op(OpKind::Silu, vec![a], vec![c_out], "silu_c");
    ...
    fn pdt_four_node_chain_ipostdom_chain() {
        ...
        g.add_op(OpKind::Mul, vec![silu_out], vec![mul_out], "mul");
        g.add_op(OpKind::Add, vec![mul_out, b], vec![add_out], "add");
        g.add_op(OpKind::Silu, vec![add_out], vec![silu2_out], "silu2");
    ...
    fn score_fusion_larger_tensor_higher_score() {
        ...
        g_small.add_op(
            OpKind::Gemm { m: SymDim::Concrete(1), n: 16, k: 16, dtype: dt, trans_b: false },
            vec![a_s, w_s], vec![gemm_out_s], "gemm",
        );
        g_small.add_op(OpKind::Silu, vec![gemm_out_s], vec![silu_out_s], "silu");
        ...
        g_large.add_op(
            OpKind::Gemm { m: SymDim::Concrete(1), n: 256, k: 256, dtype: dt, trans_b: false },
            vec![a_l, w_l], vec![gemm_out_l], "gemm",
        );
        g_large.add_op(OpKind::Silu, vec![gemm_out_l], vec![silu_out_l], "silu");
    ...
    fn pdt_two_disconnected_subgraphs() {
        ...
        g.add_op(OpKind::Silu, vec![a1], vec![s1], "silu_1");
        g.add_op(OpKind::Mul, vec![s1], vec![m1], "mul_1");
        g.add_op(OpKind::Silu, vec![a2], vec![s2], "silu_2");
        g.add_op(OpKind::Mul, vec![s2], vec![m2], "mul_2");
    ...
    fn discover_candidates_injective_to_elemwise() {
        ...
        g.add_op(OpKind::RoPE { num_heads: 4, head_dim: 16, theta: 10000.0, partial: 1.0, rope_scaling: None }, vec![input, freq], vec![rope_out], "rope");
    ...
    fn discover_candidates_reduction_anchor() {
        ...
        g.add_op(OpKind::RmsNorm { feature_dim: 4096, eps: 1e-5 }, vec![input, weight], vec![norm_out], "rmsnorm");
    ...
    fn compute_bytes_saved_proportional_to_element_count() {
        ...
        g_small.add_op(OpKind::Silu, vec![out_s], vec![silu_s], "silu");
        ...
        g_large.add_op(OpKind::Silu, vec![out_l], vec![silu_l], "silu");
