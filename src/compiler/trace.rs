...
pub enum TraceOp {
    ...
    /// Q6_K combined decode (BCE-20260710-Q6K-HIGHBITS).
    /// 修复 emit_unpack NibbleWithHighBits qh<<6 & 0x30 丢高 2 bit bug.
    /// Q6_K quarter 结构: qs[128](低4bit) + qh[64](高2bit) + scales[16 i8] + d(f16) = 210B.
    /// 委托 q6k_decode_step_native 处理位置相关高 2 bit 提取 (同 QuantQ3KDecode 单片模式).
    QuantQ6KDecode {
        block_base: ValueId,
        lane_offset: ValueId,
        d_slot: ValueId,
    ...
    ///   q5 = lo4 | (hi1<<4); hi1=(qh[l=i%32] >> mini=i/32)&1 (转置); lo4 SPLIT per 64-group;
    ///   (sc,m)=get_scale_min_k4(mini, scales).
    /// 委托 q5k_decode_step_native (scalar 循环, 对齐 k_quant.rs:381-435 + llama.cpp dequantize_row_q5_K).
    QuantQ5KDecode {
        block_base: ValueId,
        lane_offset: ValueId,
        d_slot: ValueId,
    ...
    /// Q4_K uses four groups of 32 low nibbles followed by 32 high nibbles;
    /// each half has its own packed 6-bit scale/min pair.
    QuantQ4KDecode {
        block_base: ValueId,
        lane_offset: ValueId,
        d_slot: ValueId,
...
pub fn infer_result_dtype(op: &TraceOp, slots: &[TypedSlot]) -> QuantPrecision {
    ...
    match op {
        ...
        TraceOp::QuantQ5KDecode { .. } => QuantPrecision::F32,
        TraceOp::QuantQ4KDecode { .. } => QuantPrecision::F32,
