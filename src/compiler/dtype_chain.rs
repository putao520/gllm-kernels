...
use crate::compiler::graph::{CompilerGraph, OpId, OpKind, TensorId, TensorMeta};
...
pub fn detect_dtype_breakpoints(graph: &CompilerGraph) -> Vec<DtypeBreakpoint> {
    ...
    for op in &graph.ops {
        ...
        let expected = op_expected_dtype(&op.kind, graph);
...
fn op_expected_dtype(kind: &OpKind, _graph: &CompilerGraph) -> Option<DType> {
    match kind {
        OpKind::Gemm { dtype, .. } | OpKind::GemmBias { dtype, .. } => Some(*dtype),
        OpKind::QuantGemm { .. } => Some(DType::F32), // QuantGemm always outputs F32
        OpKind::RmsNorm { .. } | OpKind::LayerNorm { .. } => {
            // Norm ops expect the same dtype as their first input
            None // defer to actual input dtype
        }
        _ => None,
    }
}
...
mod tests {
    ...
    fn test_detect_breakpoints_consistent_dtype() {
        ...
        g.add_op(
            OpKind::Gemm { m: SymDim::Concrete(1), n: 128, k: 64, dtype: dt, trans_b: false },
            vec![a, b], vec![c], "gemm"
        );
    ...
    fn test_dequant_strategy_gemm_weight() {
        ...
        let c = g.add_tensor_concrete("c", &[1, 128], DType::F32);
        g.add_op(
            OpKind::QuantGemm { m: SymDim::Concrete(1), n: 128, k: 64, quant_type: crate::quant::QuantType::Q4K },
            vec![a, b], vec![c], "qgemm"
        );
    ...
    fn test_dequant_strategy_1d_bias() {
        ...
        let c = g.add_tensor_concrete("c", &[1, 128], DType::F32);
        g.add_op(
            OpKind::GemmBias { m: SymDim::Concrete(1), n: 128, k: 64, dtype: DType::F32, trans_b: false },
    ...
    fn test_kernel_dtype_key_from_graph() {
        ...
        g.inputs = vec![a, w];
        g.add_op(
            OpKind::Gemm { m: SymDim::Concrete(1), n: 512, k: 512, dtype: DType::F32, trans_b: false },
    ...
    fn test_dtype_chain_validation_separates_weight_and_activation() {
        ...
        g.add_op(
            OpKind::Gemm { m: SymDim::Concrete(1), n: 2048, k: 512, dtype: DType::F32, trans_b: false },
