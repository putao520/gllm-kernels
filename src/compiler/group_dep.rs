...
mod tests {
    ...
    fn test_single_group() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t0], vec![t_out], "silu");
    ...
    fn test_independent_groups() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in0], vec![t_out0], "silu0");
    ...
    fn test_diamond_dependency() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_a], "fanout");
    ...
    fn test_missing_producer_tensor() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_ext], vec![t_out], "silu");
    ...
    fn test_wide_fan_out() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_src], "src");
    ...
    fn test_mixed_parallel_and_chain() {
        ...
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "right");
        let op3 = graph.add_op(Op::Add, vec![t_b, t_c], vec![t_out], "merge");
        let op4 = graph.add_op(Op::Silu, vec![t_in4], vec![t_out4], "indep");
    ...
    fn test_self_dependency_ignored() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_mid], "inner0");
        let op1 = graph.add_op(Op::Silu, vec![t_mid], vec![t_out], "inner1");
    ...
    fn test_level_indices_sequential() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t0], vec![t1], "a");
        let op1 = graph.add_op(Op::Silu, vec![t1], vec![t2], "b");
    ...
    fn test_many_groups_deep_chain() {
        ...
        for i in 0..10 {
            ...
            let op = graph.add_op(Op::Silu, vec![prev], vec![t], &format!("op{}", i));
            ops.push(op);
            prev = t;
        }
    ...
    fn test_op_not_in_graph() {
        ...
        let t_in = graph.add_tensor("input", vec![SymDim::Concrete(4)], DType::F32);
        let t_out = graph.add_tensor("out", vec![SymDim::Concrete(4)], DType::F32);
        let real_op = graph.add_op(Op::Silu, vec![t_in], vec![t_out], "real");
    ...
    fn test_input_tensor_missing_from_graph() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_phantom_input], vec![t_out], "solo");
    ...
    fn test_producer_not_in_any_group() {
        ...
        let orphan_op = graph.add_op(Op::Silu, vec![t_in], vec![t_mid], "orphan");
        let consumer_op = graph.add_op(Op::Silu, vec![t_mid], vec![t_out], "consumer");
    ...
    fn test_fan_in_two_producer_groups() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in0], vec![t_a], "prod_a");
        let op1 = graph.add_op(Op::Silu, vec![t_in1], vec![t_b], "prod_b");
    ...
    fn test_duplicate_dependency_deduplication() {
        ...
        let op0a = graph.add_op(Op::Silu, vec![t_in], vec![t_a], "prod_a");
        let op0b = graph.add_op(Op::Silu, vec![t_in], vec![t_b], "prod_b");
        // Group 1 consumes both t_a and t_b
        let op1 = graph.add_op(Op::Add, vec![t_a, t_b], vec![t_out], "consumer");
    ...
    fn test_multi_input_from_different_groups() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_x], vec![t_a], "left");
        let op1 = graph.add_op(Op::Silu, vec![t_y], vec![t_b], "right");
    ...
    fn test_three_tier_pipeline() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_root], vec![t_a], "root");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "g1");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "g2");
        let op3 = graph.add_op(Op::Silu, vec![t_a], vec![t_d], "g3");
        let op4 = graph.add_op(Op::Add, vec![t_b, t_c], vec![t_e], "g4");
        let op5 = graph.add_op(Op::Add, vec![t_c, t_d], vec![t_f], "g5");
    ...
    fn test_shared_input_no_dependency() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_shared], vec![t_out0], "path_a");
    ...
    fn test_wide_fan_in_single_consumer() {
        ...
        for i in 0..6 {
            ...
            let op = graph.add_op(Op::Silu, vec![t_in], vec![t_out], &format!("prod{}", i));
    ...
    fn test_group_with_ops_having_no_inputs() {
        ...
        let mut graph = CompilerGraph::new();
        let t_out = graph.add_tensor("out", vec![SymDim::Concrete(4)], DType::F32);
        let op0 = graph.add_op(Op::Silu, vec![], vec![t_out], "no_input");
    ...
    fn test_input_tensor_with_none_producer() {
        ...
        let t_out = graph.add_tensor("out", vec![SymDim::Concrete(4)], DType::F32);
        let op0 = graph.add_op(Op::Silu, vec![t_weight], vec![t_out], "use_weight");
    ...
    fn test_chain_with_interleaved_independent() {
        ...
        for i in 0..5 {
            ...
            let op = graph.add_op(Op::Silu, vec![prev], vec![t], &format!("chain{}", i));
            chain_ops.push(op);
            prev = t;
        }

        let t_indep_out = graph.add_tensor("indep_out", vec![SymDim::Concrete(4)], DType::F32);
        let op_indep = graph.add_op(Op::Silu, vec![t_indep], vec![t_indep_out], "indep");
    ...
    fn test_reversed_group_id_order() {
        ...
        // chain: op0 → op1 → op2
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_a], "first");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "second");
        let op2 = graph.add_op(Op::Silu, vec![t_b], vec![t_out], "third");
    ...
    fn test_nested_diamond() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_a], "root");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "l1_left");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "l1_right");
        let op3 = graph.add_op(Op::Silu, vec![t_b], vec![t_d], "l2_left");
        let op4 = graph.add_op(Op::Silu, vec![t_c], vec![t_e], "l2_right");
        let op5 = graph.add_op(Op::Add, vec![t_d, t_e], vec![t_f], "merge1");
        let op6 = graph.add_op(Op::Silu, vec![t_f], vec![t_g], "inner_left");
        let op7 = graph.add_op(Op::Silu, vec![t_f], vec![t_h], "inner_right");
        let op8 = graph.add_op(Op::Add, vec![t_g, t_h], vec![t_i], "merge2");
    ...
    fn test_consumer_with_mixed_producer_sources() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_produced], "producer");
    ...
    fn test_two_chains_merging_at_tail() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_a_in], vec![t_a_mid], "chain_a_0");
        let op1 = graph.add_op(Op::Silu, vec![t_b_in], vec![t_b_mid], "chain_b_0");
        let op2 = graph.add_op(Op::Silu, vec![t_a_mid], vec![t_a_out], "chain_a_1");
        let op3 = graph.add_op(Op::Silu, vec![t_b_mid], vec![t_b_out], "chain_b_1");
        let op4 = graph.add_op(Op::Add, vec![t_a_out, t_b_out], vec![t_final], "merge");
    ...
    fn test_topo_level_ordering_invariant() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_root], vec![t_a], "g0");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "g1");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "g2");
        let op3 = graph.add_op(Op::Add, vec![t_b, t_c], vec![t_d], "g3");
        let op4 = graph.add_op(Op::Add, vec![t_b, t_c], vec![t_e], "g4");
        let op5 = graph.add_op(Op::Silu, vec![t_ext], vec![t_f], "g5");
    ...
    fn test_single_group_chain_with_external_weight() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_mid], "silu");
        let op1 = graph.add_op(Op::Mul, vec![t_mid, t_w], vec![t_out], "mul_weight");
    ...
    fn test_long_fan_out_all_independent_consumers() {
        ...
        let t_root = graph.add_tensor("root", vec![SymDim::Concrete(4)], DType::F32);
        let t_src = graph.add_tensor("src", vec![SymDim::Concrete(4)], DType::F32);
        let op0 = graph.add_op(Op::Silu, vec![t_root], vec![t_src], "producer");
    ...
    fn test_symmetric_v_shape() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "left");
        let op1 = graph.add_op(Op::Silu, vec![t_b], vec![t_d], "right");
        let op2 = graph.add_op(Op::Add, vec![t_c, t_d], vec![t_out], "sink");
    ...
    fn test_two_groups_one_empty_ops() {
        ...
        let t_in = graph.add_tensor("in", vec![SymDim::Concrete(4)], DType::F32);
        let t_out = graph.add_tensor("out", vec![SymDim::Concrete(4)], DType::F32);
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_out], "real");
    ...
    fn test_producer_and_consumer_same_tensor_names() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_mid], "producer");
        let op1 = graph.add_op(Op::Silu, vec![t_mid], vec![t_out], "consumer");
    ...
    fn test_four_level_staircase() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_root], vec![t_a], "src");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "l1a");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "l1b");
        let op3 = graph.add_op(Op::Silu, vec![t_b], vec![t_d], "l2a");
        let op4 = graph.add_op(Op::Silu, vec![t_c], vec![t_e], "l2b");
        let op5 = graph.add_op(Op::Add, vec![t_d, t_e], vec![t_out], "sink");
    ...
    fn test_two_isolated_chains_of_different_length() {
        ...
        let op_a0 = graph.add_op(Op::Silu, vec![t_a0], vec![t_a1], "a0");
        let t_a2 = graph.add_tensor("a2", vec![SymDim::Concrete(4)], DType::F32);
        let op_a1 = graph.add_op(Op::Silu, vec![t_a1], vec![t_a2], "a1");
    ...
    fn test_parallel_groups_fan_out_then_merge_back() {
        ...
        for i in 0..3 {
            let t = graph.add_tensor(&format!("mid{}", i), vec![SymDim::Concrete(4)], DType::F32);
            let op = graph.add_op(Op::Silu, vec![t_src], vec![t], &format!("par{}", i));
            mid_ops.push(op);
            mid_outs.push(t);
        }
        ...
        let op5 = graph.add_op(Op::Silu, vec![t_ext5], vec![t_out5], "indep5");
        ...
        let op6 = graph.add_op(Op::Silu, vec![t_ext6], vec![t_out6], "indep6");
    ...
    fn test_single_op_multiple_outputs_to_different_groups() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_a], "producer");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "left_out");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "right_out");
        let op3 = graph.add_op(Op::Silu, vec![t_b], vec![t_out1], "consumer_l");
    ...
    fn test_total_visited_equals_num_groups_complex() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_root], vec![t_a], "r");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "g1");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "g2");
        let op3 = graph.add_op(Op::Silu, vec![t_b], vec![t_d], "g3");
        let op4 = graph.add_op(Op::Silu, vec![t_c], vec![t_e], "g4");
        let op5 = graph.add_op(Op::Add, vec![t_d, t_e], vec![t_f], "g5");
        let op6 = graph.add_op(Op::Silu, vec![t_f], vec![t_g], "g6");
        let op7 = graph.add_op(Op::Silu, vec![t_ext], vec![t_out], "g7_indep");
        let t_final = graph.add_tensor("final", vec![SymDim::Concrete(4)], DType::F32);
        let op8 = graph.add_op(Op::Add, vec![t_g, t_out], vec![t_final], "g8");
    ...
    fn test_cyclic_dependency_drops_affected_groups() {
        ...
        let op_a = graph.add_op(Op::Silu, vec![t_ext], vec![t_a], "op_a");
        let op_b = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "op_b");
        ...
        let t_x = graph.add_tensor("x", vec![SymDim::Concrete(4)], DType::F32);
        let t_y = graph.add_tensor("y", vec![SymDim::Concrete(4)], DType::F32);
        let op0 = graph.add_op(Op::Silu, vec![t_x], vec![t_a], "cycle0");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_y], "cycle1");
        ...
        let op2 = graph.add_op(Op::Silu, vec![t_ext], vec![t_out], "indep");
    ...
    fn test_level_monotonicity_broad_graph() {
        ...
        for i in 0..4 {
            ...
            let op = graph.add_op(Op::Silu, vec![t_src], vec![t], &format!("m{}", i));
            mid_ops.push(op);
            mid_outs.push(t);
        }
        ...
        // Final sink
        let t_final = graph.add_tensor("final", vec![SymDim::Concrete(4)], DType::F32);
        let op_sink = graph.add_op(Op::Add, vec![t_f1, t_f2], vec![t_final], "sink");
    ...
    fn test_group_dependency_after_middle_group_removal() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_mid], "first");
        let op1 = graph.add_op(Op::Silu, vec![t_mid], vec![t_out], "second");
    ...
    fn test_wide_parallel_with_asymmetric_depths() {
        ...
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "g2");
        let op3 = graph.add_op(Op::Silu, vec![t_b], vec![t_d], "g3");
        let op4 = graph.add_op(Op::Silu, vec![t_b], vec![t_e], "g4");
        let op5 = graph.add_op(Op::Silu, vec![t_c], vec![t_f], "g5");
        let op6 = graph.add_op(Op::Add, vec![t_d, t_f], vec![t_final], "g6");
    ...
    fn test_two_consumers_of_same_producer_different_levels() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_a], "producer");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "mid");
        let op2 = graph.add_op(Op::Silu, vec![t_b], vec![t_c], "deep");
        let op3 = graph.add_op(Op::Silu, vec![t_a], vec![t_d], "shallow");
    ...
    fn test_cycle_via_bidirectional_tensor_flow() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_ext], vec![t_a], "op0");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "op1");
        let op2 = graph.add_op(Op::Silu, vec![t_ext], vec![t_out], "indep");
    ...
    fn test_multi_path_redundant_dependency() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_in], vec![t_a], "g0");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "g1");
        let op2 = graph.add_op(Op::Silu, vec![t_b], vec![t_c], "g2");
        // g3 consumes both t_c (from g2) and t_a (from g0) — redundant path to g0
        let op3 = graph.add_op(Op::Mul, vec![t_c, t_a], vec![t_out], "g3");
    ...
    fn test_parallel_groups_with_shared_intermediate() {
        ...
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "g1");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "g2");
        let op3 = graph.add_op(Op::Add, vec![t_b, t_c], vec![t_out], "g3");
        let op4 = graph.add_op(Op::Silu, vec![t_a], vec![t_d], "g4");
    ...
    fn test_single_group_no_tensor_producer_no_inputs() {
        ...
        let op0 = graph.add_op(Op::Mul, vec![t_w0, t_w1], vec![t_out], "weight_mul");
    ...
    fn test_multiple_parallel_subtrees() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_a0], vec![t_a1], "a_root");
        let op1 = graph.add_op(Op::Silu, vec![t_a1], vec![t_a2], "a_mid");
        let op2 = graph.add_op(Op::Silu, vec![t_a2], vec![t_a3], "a_leaf");
        let op3 = graph.add_op(Op::Silu, vec![t_b0], vec![t_b1], "b_root");
        let op4 = graph.add_op(Op::Silu, vec![t_b1], vec![t_b2], "b_mid");
        let op5 = graph.add_op(Op::Silu, vec![t_b2], vec![t_b3], "b_leaf");
    ...
    fn test_group_with_op_having_only_weight_inputs_and_chain_output() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_ext], vec![t_a], "producer");
        let op1 = graph.add_op(Op::Mul, vec![t_a, t_w], vec![t_b], "mixed_consumer");
        let op2 = graph.add_op(Op::Silu, vec![t_b], vec![t_out], "downstream");
        let op3 = graph.add_op(Op::Silu, vec![t_indep], vec![t_indep_out], "indep");
    ...
    fn test_binary_tree_two_levels() {
        ...
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "left");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "right");
        let op3 = graph.add_op(Op::Silu, vec![t_b], vec![t_d], "ll");
        let op4 = graph.add_op(Op::Silu, vec![t_b], vec![t_e], "lr");
        let op5 = graph.add_op(Op::Silu, vec![t_c], vec![t_f], "rl");
        let op6 = graph.add_op(Op::Add, vec![t_d, t_e, t_f], vec![t_g], "rr");
    ...
    fn test_group_consuming_from_two_ops_in_same_producer_group() {
        ...
        let op0a = graph.add_op(Op::Silu, vec![t_in], vec![t_a], "prod_a");
        let op0b = graph.add_op(Op::Mul, vec![t_in], vec![t_b], "prod_b");
        let op1 = graph.add_op(Op::Add, vec![t_a, t_b], vec![t_out], "consumer");
    ...
    fn test_chain_with_branch_rejoining_at_different_depths() {
        ...
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "g1");
        let op2 = graph.add_op(Op::Silu, vec![t_b], vec![t_c], "g2");
        let op3 = graph.add_op(Op::Silu, vec![t_a], vec![t_d], "g3_skip");
        let op4 = graph.add_op(Op::Add, vec![t_c, t_d], vec![t_out], "g4");
    ...
    fn test_level_ordering_invariant_wide_graph() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_r], vec![t_a], "g0");
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "g1");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "g2");
        let op3 = graph.add_op(Op::Add, vec![t_b, t_c], vec![t_d], "g3");
        let op4 = graph.add_op(Op::Silu, vec![t_d], vec![t_e], "g4");
        let op5 = graph.add_op(Op::Silu, vec![t_d], vec![t_f], "g5");
        let op6 = graph.add_op(Op::Add, vec![t_e, t_f], vec![t_g], "g6");
        let op7 = graph.add_op(Op::Silu, vec![t_g], vec![t_h], "g7");
    ...
    fn test_two_producer_groups_one_consumer_with_extra_weight() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_x], vec![t_a], "prod_a");
        let op1 = graph.add_op(Op::Silu, vec![t_y], vec![t_b], "prod_b");
        let op2 = graph.add_op(Op::Mul, vec![t_a, t_b, t_w], vec![t_out], "consumer");
    ...
    fn test_no_deps_with_single_op_having_multiple_external_inputs() {
        ...
        let op0 = graph.add_op(Op::Mul, vec![t_a, t_b, t_c], vec![t_out], "multi_input");
    ...
    fn test_wide_fan_out_with_partial_consumer_dependency() {
        ...
        for i in 0..5 {
            let t = graph.add_tensor(&format!("c{}", i), vec![SymDim::Concrete(4)], DType::F32);
            let op = graph.add_op(Op::Silu, vec![t_src], vec![t], &format!("c{}", i));
            mid_outs.push(t);
            mid_ops.push(op);
        }
        ...
        let t_ext = graph.add_tensor("ext", vec![SymDim::Concrete(4)], DType::F32);
        let t_indep = graph.add_tensor("indep", vec![SymDim::Concrete(4)], DType::F32);
        let op_indep = graph.add_op(Op::Silu, vec![t_ext], vec![t_indep], "indep");
    ...
    fn test_double_diamond_shared_root() {
        ...
        let op0 = graph.add_op(Op::Silu, vec![t_root], vec![t_a], "root");
        // Diamond A
        let op1 = graph.add_op(Op::Silu, vec![t_a], vec![t_b], "a_left");
        let op2 = graph.add_op(Op::Silu, vec![t_a], vec![t_c], "a_right");
        let op3 = graph.add_op(Op::Add, vec![t_b, t_c], vec![t_d], "a_merge");
        // Diamond B
        let op4 = graph.add_op(Op::Silu, vec![t_a], vec![t_e], "b_left");
        let op5 = graph.add_op(Op::Silu, vec![t_a], vec![t_f], "b_right");
        let op6 = graph.add_op(Op::Add, vec![t_e, t_f], vec![t_g], "b_merge");
