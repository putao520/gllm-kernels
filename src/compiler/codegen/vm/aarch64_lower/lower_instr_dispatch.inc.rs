impl AArch64Lower {
    ...
    // L1 变体路由 + L2 叶子 emit
    // lower_instr (L0) 按 VmInstr::category() 路由到 lower_<cat>_aarch64 (L1),
    // L1 内 match 变体路由到 lower_<variant>_aarch64 (L2 叶子 fn)。
    ...
    fn lower_memory_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::LoadPtr { dst, src } => self.lower_load_ptr_aarch64(instr, alloc),
            VmInstr::VecLoad { dst, base, offset, width, dtype , predicate: _predicate } => self.lower_vec_load_aarch64(instr, alloc),
            VmInstr::VecStore { base, src, offset, dtype, .. } => self.lower_vec_store_aarch64(instr, alloc),
            VmInstr::VecNarrow { dst, src, dst_dtype, src_dtype, .. } => self.lower_vec_narrow_aarch64(instr, alloc),
            VmInstr::VecWiden { dst, src, dst_dtype, src_dtype, .. } => self.lower_vec_widen_aarch64(instr, alloc),
            VmInstr::Mov { dst, src, dtype: _ } => self.lower_mov_aarch64(instr, alloc),
            VmInstr::Broadcast { dst, src, width: _, dtype: _ } => self.lower_broadcast_aarch64(instr, alloc),
            VmInstr::Prefetch { base, ref offset, distance, hint } => self.lower_prefetch_aarch64(instr, alloc),
            VmInstr::ScalarLoad { dst, base, offset } => self.lower_scalar_load_aarch64(instr, alloc),
            VmInstr::ScalarStore { base, src, offset } => self.lower_scalar_store_aarch64(instr, alloc),
            VmInstr::IndexToScalar { dst, src } => self.lower_index_to_scalar_aarch64(instr, alloc),
            VmInstr::ScalarToIndex { dst, src, stride } => self.lower_scalar_to_index_aarch64(instr, alloc),
            VmInstr::IntMulStride { dst, src, stride } => self.lower_int_mul_stride_aarch64(instr, alloc),
            VmInstr::ScalarByteLoad { dst, base, offset } => self.lower_scalar_byte_load_aarch64(instr, alloc),
            VmInstr::ActivationSwap { ptr_a, ptr_b } => self.lower_activation_swap_aarch64(instr, alloc),
            VmInstr::AtomicAdd { base, ref offset, value, elem_width } => self.lower_atomic_add_aarch64(instr, alloc),
            VmInstr::MemFence { order } => self.lower_mem_fence_aarch64(instr, alloc),
            VmInstr::AddPtr { dst, base, offset } => self.lower_add_ptr_aarch64(instr, alloc),
            VmInstr::StoreConstToStack { rbp_offset, value, elem_width } => self.lower_store_const_to_stack_aarch64(instr, alloc),
            VmInstr::GatherLoad { dst, base, indices, stride, width , dtype: _dtype, predicate: _predicate, } => self.lower_gather_load_aarch64(instr, alloc),
            VmInstr::ScatterStore { base, indices, src, stride, width , dtype: _dtype, predicate: _predicate, } => self.lower_scatter_store_aarch64(instr, alloc),
            VmInstr::TableLookup { dst, base, row_index, row_bytes, width } => self.lower_table_lookup_aarch64(instr, alloc),
            VmInstr::VecScalarStore { base, src, offset } => self.lower_vec_scalar_store_aarch64(instr, alloc),
            VmInstr::MemCopy { .. } => self.lower_mem_copy_aarch64(instr, alloc),
            VmInstr::AtomicCAS { dst, ptr, expected, desired, elem_width, success_order: _, failure_order: _ } => self.lower_atomic_c_a_s_aarch64(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_memory_aarch64: variant {:?} not in Memory category", instr))),
        }
    }

    /// L1 变体路由: Arith 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_arith_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::VecBinOp { dst, a, b, op, dtype } => self.lower_vec_bin_op_aarch64(instr, alloc),
            VmInstr::VecUnaryOp { dst, a, op } => self.lower_vec_unary_op_aarch64(instr, alloc),
            VmInstr::VecShiftImm { dst, a, amount, op, .. } => self.lower_vec_shift_imm_aarch64(instr, alloc),
            VmInstr::Fma { dst, acc, a, b, dtype: _ } => self.lower_fma_aarch64(instr, alloc),
            VmInstr::HReduce { dst, src, op } => self.lower_h_reduce_aarch64(instr, alloc),
            VmInstr::Accumulate { acc, src } => self.lower_accumulate_aarch64(instr, alloc),
            VmInstr::Transcendental { dst, src, func } => self.lower_transcendental_aarch64(instr, alloc),
            VmInstr::GprBinOp { dst, a: src, b: GprOperand::Imm(amount), op: GprOp::Shl } => self.lower_gpr_bin_op_aarch64(instr, alloc),
            VmInstr::GprBinOp { dst, a: src, b: GprOperand::Imm(amount), op: GprOp::Shr } => self.lower_gpr_bin_op_aarch64(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::Imm(value), op: GprOp::Sub } => self.lower_gpr_bin_op_aarch64(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::VReg(b_vreg), op: GprOp::Add } => self.lower_gpr_bin_op_aarch64(instr, alloc),
            VmInstr::GprLoadImm { dst, value } => self.lower_gpr_load_imm_aarch64(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::VReg(b_vreg), op: GprOp::Sub } => self.lower_gpr_bin_op_aarch64(instr, alloc),
            VmInstr::GprBinOp { dst, a, b, op } => self.lower_gpr_bin_op_aarch64(instr, alloc),
            VmInstr::GprUnaryOp { dst, src, op } => self.lower_gpr_unary_op_aarch64(instr, alloc),
            VmInstr::VecCmp { dst, a, b, pred } => self.lower_vec_cmp_aarch64(instr, alloc),
            VmInstr::ConditionalSelect { dst, mask, true_val, false_val } => self.lower_conditional_select_aarch64(instr, alloc),
            VmInstr::VecCast { dst, src, from_bits, to_bits } => self.lower_vec_cast_aarch64(instr, alloc),
            VmInstr::DotProduct { acc, a, b, input_dtype, .. } => self.lower_dot_product_aarch64(instr, alloc),
            VmInstr::ScaleApply { dst, acc, scale, zero, input_dtype, .. } => self.lower_scale_apply_aarch64(instr, alloc),
            VmInstr::VecShuffle { dst, src, mask, width: _ } => self.lower_vec_shuffle_aarch64(instr, alloc),
            VmInstr::VecExtractLane { dst, src, lane, dtype: _ } => self.lower_vec_extract_lane_aarch64(instr, alloc),
            VmInstr::VecInsertLane { dst, src_vec, src_scalar, lane, dtype: _ } => self.lower_vec_insert_lane_aarch64(instr, alloc),
            VmInstr::VecLoadConst { dst, values, dtype: _, width: _ } => self.lower_vec_load_const_aarch64(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_arith_aarch64: variant {:?} not in Arith category", instr))),
        }
    }
    ...
    fn lower_control_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            VmInstr::BranchIfGprLtU { .. } => self.lower_branch_if_gpr_lt_u_aarch64(instr, alloc),
            VmInstr::UnconditionalBranch { .. } => self.lower_unconditional_branch_aarch64(instr, alloc),
            VmInstr::MarkLabel { label_id } => self.lower_mark_label_aarch64(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_control_aarch64: variant {:?} not in Control category", instr))),
        }
    }

    /// L1 变体路由: Tile 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_tile_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TileConfig { rows, cols, dtype } => self.lower_tile_config_aarch64(instr, alloc),
            VmInstr::TileLoad { dst_tile, base_ptr, k_offset, row_stride, rows, cols, dtype } => self.lower_tile_load_aarch64(instr, alloc),
            VmInstr::TileMma { c, a, b, m, n, k, dtype } => self.lower_tile_mma_aarch64(instr, alloc),
            VmInstr::TileStore { src_tile, base_ptr, out_offset, row_stride, rows, cols, dtype } => self.lower_tile_store_aarch64(instr, alloc),
            VmInstr::TileRelease => self.lower_tile_release_aarch64(instr, alloc),
            VmInstr::TmemAlloc { .. } => self.lower_tmem_alloc_aarch64(instr, alloc),
            VmInstr::TmemLoad { .. } => self.lower_tmem_load_aarch64(instr, alloc),
            VmInstr::TmemStore { .. } => self.lower_tmem_store_aarch64(instr, alloc),
            VmInstr::TmemDealloc { .. } => self.lower_tmem_dealloc_aarch64(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_aarch64: variant {:?} not in Tile category", instr))),
        }
    }

    /// L1 变体路由: Quant 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_quant_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::KiviQuantChannel { src, dst_ptr, scale_ptr, num_channels, .. } => self.lower_kivi_quant_channel_aarch64(instr, alloc),
            VmInstr::KiviQuantToken { src, dst_ptr, scale_ptr, num_elems, .. } => self.lower_kivi_quant_token_aarch64(instr, alloc),
            VmInstr::KiviDequantLoad { dst, src_ptr, scale_ptr, num_elems, .. } => self.lower_kivi_dequant_load_aarch64(instr, alloc),
            VmInstr::SparseMaskIntersect { .. } => self.lower_sparse_mask_intersect_aarch64(instr, alloc),
            VmInstr::GgufSubScaleLoad { .. } | VmInstr::GgufKQuantScaleLoad { .. } => self.lower_gguf_sub_scale_load_aarch64(instr, alloc),
            VmInstr::QuantBlockLoad { dst, base, offset, unpack, width } => self.lower_quant_block_load_aarch64(instr, alloc),
            VmInstr::QuantBiPlaneLoad { dst, qs_base, extra_base, bias, mode, width } => self.lower_quant_bi_plane_load_aarch64(instr, alloc),
            VmInstr::QuantBroadcastInt { dst, value, .. } => self.lower_quant_broadcast_int_aarch64(instr, alloc),
            VmInstr::QuantScalarCvtLoad { dst, base, offset, src_dtype, .. } => self.lower_quant_scalar_cvt_load_aarch64(instr, alloc),
            VmInstr::QuantLoadBytesVec { dst, base, offset, count, .. } => self.lower_quant_load_bytes_vec_aarch64(instr, alloc),
            VmInstr::QuantCodebookLookup { dst, indices, codebook_data, bits_per_entry, .. } => self.lower_quant_codebook_lookup_aarch64(instr, alloc),
            VmInstr::QuantExtractBits { dst, src, bit_offset, bit_width, .. } => self.lower_quant_extract_bits_aarch64(instr, alloc),
            VmInstr::QuantDequantFma { quant_kind, .. } => self.lower_quant_dequant_fma_aarch64(instr, alloc),
            ...
            VmInstr::NativeFp4Gemm { .. } => self.lower_native_fp4_gemm_aarch64(instr, alloc),
            VmInstr::NativeFp6Gemm { .. } => self.lower_native_fp6_gemm_aarch64(instr, alloc),
            VmInstr::TwoCtaFp4Gemm { .. } => self.lower_two_cta_fp4_gemm_aarch64(instr, alloc),
            VmInstr::NativeFp8Gemm { .. } => self.lower_native_fp8_gemm_aarch64(instr, alloc),
            VmInstr::HwQuantDequant { .. } => self.lower_hw_quant_dequant_aarch64(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_aarch64: variant {:?} not in Quant category", instr))),
        }
    }

    /// L1 变体路由: GpuComm 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_gpu_comm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::PageTableAddr { dst, pool_base, page_table_ptr, ki_byte_off, row_bytes, page_size, page_stride, base_offset, seq_pt_offset } => self.lower_page_table_addr_aarch64(instr, alloc),
            VmInstr::PageTableKVWrite { src, pool_base, page_table_ptr, seq_index, row_bytes, page_size, page_stride, base_offset, width, dtype: _ } => self.lower_page_table_k_v_write_aarch64(instr, alloc),
            VmInstr::PageTableKVWriteQuant { src, pool_base, page_table_ptr, seq_index, quant_row_bytes, fp32_row_bytes: _, page_size, page_stride, base_offset, scale_offset, width: _, kivi_mode, num_elems } => self.lower_page_table_k_v_write_quant_aarch64(instr, alloc),
            VmInstr::SharedMemSwizzle { dst, raw_addr, .. } => self.lower_shared_mem_swizzle_aarch64(instr, alloc),
            VmInstr::WarpSync | VmInstr::AsyncCopy { .. } | VmInstr::AsyncWait { .. } => self.lower_warp_sync_aarch64(instr, alloc),
            VmInstr::SharedMemAlloc { .. } => self.lower_shared_mem_alloc_aarch64(instr, alloc),
            VmInstr::SharedMemStore { .. } => self.lower_shared_mem_store_aarch64(instr, alloc),
            VmInstr::SharedMemLoad { .. } => self.lower_shared_mem_load_aarch64(instr, alloc),
            VmInstr::SharedMemAsyncStore { .. } | VmInstr::SharedMemAsyncWaitGroup { .. } => self.lower_shared_mem_async_store_aarch64(instr, alloc),
            VmInstr::WeightPrefetchAsync { .. } | VmInstr::WeightPrefetchWait { .. } => self.lower_weight_prefetch_async_aarch64(instr, alloc),
            VmInstr::WarpRoleDeclare { .. } => self.lower_warp_role_declare_aarch64(instr, alloc),
            VmInstr::WarpBarrierArrive { .. } => self.lower_warp_barrier_arrive_aarch64(instr, alloc),
            VmInstr::WarpBarrierWait { .. } => self.lower_warp_barrier_wait_aarch64(instr, alloc),
            VmInstr::TmaDescriptorInit { .. } | VmInstr::Tma2DCopy { .. } | VmInstr::BarrierInit { .. } => self.lower_tma_descriptor_init_aarch64(instr, alloc),
            VmInstr::BlockSync => self.lower_block_sync_aarch64(instr, alloc),
            VmInstr::WarpReduce { .. } => self.lower_warp_reduce_aarch64(instr, alloc),
            VmInstr::ClusterBarrierInit { .. } => self.lower_cluster_barrier_init_aarch64(instr, alloc),
            VmInstr::ClusterStore { .. } => self.lower_cluster_store_aarch64(instr, alloc),
            VmInstr::ClusterLoad { .. } => self.lower_cluster_load_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::AllReduceChunk { .. } => self.lower_all_reduce_chunk_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::CommBarrier { .. } => self.lower_comm_barrier_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::NvlinkAsyncCopy { .. } => self.lower_nvlink_async_copy_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::RemotePageLookup { .. } => self.lower_remote_page_lookup_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::P2pPageFetch { .. } => self.lower_p2p_page_fetch_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::RdmaPageFetch { .. } => self.lower_rdma_page_fetch_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::RdmaPageFetchCompressed { .. } => self.lower_rdma_page_fetch_compressed_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::RemotePageAttn { .. } => self.lower_remote_page_attn_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::PageMigrationLock { .. } => self.lower_page_migration_lock_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::PageMigrationUnlock { .. } => self.lower_page_migration_unlock_aarch64(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::PageLocationUpdate { .. } => self.lower_page_location_update_aarch64(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpu_comm_aarch64: variant {:?} not in GpuComm category", instr))),
        }
    }
    ...
    fn lower_sampling_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            VmInstr::SoftmaxReduceMax { .. } => self.lower_softmax_reduce_max_aarch64(instr, alloc),
            VmInstr::SoftmaxExpSum { .. } => self.lower_softmax_exp_sum_aarch64(instr, alloc),
            VmInstr::SoftmaxNormalize { .. } => self.lower_softmax_normalize_aarch64(instr, alloc),
            VmInstr::SampleTopKFilter { .. } => self.lower_sample_top_k_filter_aarch64(instr, alloc),
            VmInstr::SampleTopPFilter { .. } => self.lower_sample_top_p_filter_aarch64(instr, alloc),
            VmInstr::SampleMultinomial { .. } => self.lower_sample_multinomial_aarch64(instr, alloc),
            VmInstr::WarpPRNG { .. } => self.lower_warp_p_r_n_g_aarch64(instr, alloc),
            VmInstr::SeqIdLookup { dst, token_index, seq_meta_base, num_seqs, seq_meta_stride } => self.lower_seq_id_lookup_aarch64(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sampling_aarch64: variant {:?} not in Sampling category", instr))),
        }
    }

    /// L1 变体路由: Misc 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_misc_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::DeclareVReg { .. } | VmInstr::ReleaseVReg { .. } | VmInstr::Comment(_) => self.lower_declare_v_reg_aarch64(instr, alloc),
            VmInstr::HotpatchSlot { .. } => self.lower_hotpatch_slot_aarch64(instr, alloc),
            VmInstr::LoadCallbackEntry { table_ptr, slot_id, fn_ptr_out, ctx_out } => self.lower_load_callback_entry_aarch64(instr, alloc),
            VmInstr::NativeCall { ret_val, fn_ptr, ctx_ptr } => self.lower_native_call_aarch64(instr, alloc),
    ...
    // ── Memory ──
    fn lower_load_ptr_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_load_ptr_aarch64: expected VmInstr::LoadPtr, got {:?}", instr))),
        }
    }

    fn lower_vec_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
    ...
    fn lower_vec_narrow_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_narrow_aarch64: expected VmInstr::VecNarrow, got {:?}", instr))),
        }
    }

    fn lower_vec_widen_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_widen_aarch64: expected VmInstr::VecWiden, got {:?}", instr))),
        }
    }

    fn lower_mov_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_mov_aarch64: expected VmInstr::Mov, got {:?}", instr))),
        }
    }

    fn lower_broadcast_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_broadcast_aarch64: expected VmInstr::Broadcast, got {:?}", instr))),
        }
    }

    fn lower_prefetch_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_prefetch_aarch64: expected VmInstr::Prefetch, got {:?}", instr))),
        }
    }

    fn lower_scalar_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scalar_load_aarch64: expected VmInstr::ScalarLoad, got {:?}", instr))),
        }
    }

    fn lower_scalar_store_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scalar_store_aarch64: expected VmInstr::ScalarStore, got {:?}", instr))),
        }
    }

    fn lower_index_to_scalar_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_index_to_scalar_aarch64: expected VmInstr::IndexToScalar, got {:?}", instr))),
        }
    }

    fn lower_scalar_to_index_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scalar_to_index_aarch64: expected VmInstr::ScalarToIndex, got {:?}", instr))),
        }
    }

    fn lower_int_mul_stride_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_int_mul_stride_aarch64: expected VmInstr::IntMulStride, got {:?}", instr))),
        }
    }

    fn lower_scalar_byte_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scalar_byte_load_aarch64: expected VmInstr::ScalarByteLoad, got {:?}", instr))),
        }
    }

    fn lower_activation_swap_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_activation_swap_aarch64: expected VmInstr::ActivationSwap, got {:?}", instr))),
        }
    }

    fn lower_atomic_add_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_atomic_add_aarch64: expected VmInstr::AtomicAdd, got {:?}", instr))),
        }
    }

    fn lower_mem_fence_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_mem_fence_aarch64: expected VmInstr::MemFence, got {:?}", instr))),
        }
    }

    fn lower_add_ptr_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_add_ptr_aarch64: expected VmInstr::AddPtr, got {:?}", instr))),
        }
    }

    fn lower_store_const_to_stack_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_store_const_to_stack_aarch64: expected VmInstr::StoreConstToStack, got {:?}", instr))),
        }
    }

    fn lower_gather_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gather_load_aarch64: expected VmInstr::GatherLoad, got {:?}", instr))),
        }
    }

    fn lower_scatter_store_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scatter_store_aarch64: expected VmInstr::ScatterStore, got {:?}", instr))),
        }
    }

    fn lower_table_lookup_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
    ...
    fn lower_vec_scalar_store_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_scalar_store_aarch64: expected VmInstr::VecScalarStore, got {:?}", instr))),
        }
    }

    fn lower_mem_copy_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::MemCopy { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_mem_copy_aarch64: expected VmInstr::MemCopy, got {:?}", instr))),
        }
    }

    fn lower_atomic_c_a_s_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_atomic_c_a_s_aarch64: expected VmInstr::AtomicCAS, got {:?}", instr))),
        }
    }

    // ── Arith ──
    fn lower_vec_bin_op_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_bin_op_aarch64: expected VmInstr::VecBinOp, got {:?}", instr))),
        }
    }

    fn lower_vec_unary_op_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_unary_op_aarch64: expected VmInstr::VecUnaryOp, got {:?}", instr))),
        }
    }

    fn lower_vec_shift_imm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_shift_imm_aarch64: expected VmInstr::VecShiftImm, got {:?}", instr))),
        }
    }

    fn lower_fma_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_fma_aarch64: expected VmInstr::Fma, got {:?}", instr))),
        }
    }

    fn lower_h_reduce_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_h_reduce_aarch64: expected VmInstr::HReduce, got {:?}", instr))),
        }
    }

    fn lower_accumulate_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_accumulate_aarch64: expected VmInstr::Accumulate, got {:?}", instr))),
        }
    }

    fn lower_transcendental_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_transcendental_aarch64: expected VmInstr::Transcendental, got {:?}", instr))),
        }
    }

    fn lower_gpr_bin_op_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpr_bin_op_aarch64: expected VmInstr::GprBinOp, got {:?}", instr))),
        }
    }

    fn lower_gpr_load_imm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpr_load_imm_aarch64: expected VmInstr::GprLoadImm, got {:?}", instr))),
        }
    }

    fn lower_gpr_unary_op_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpr_unary_op_aarch64: expected VmInstr::GprUnaryOp, got {:?}", instr))),
        }
    }

    fn lower_vec_cmp_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_cmp_aarch64: expected VmInstr::VecCmp, got {:?}", instr))),
        }
    }

    fn lower_conditional_select_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_conditional_select_aarch64: expected VmInstr::ConditionalSelect, got {:?}", instr))),
        }
    }

    fn lower_vec_cast_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_cast_aarch64: expected VmInstr::VecCast, got {:?}", instr))),
        }
    }

    fn lower_dot_product_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_dot_product_aarch64: expected VmInstr::DotProduct, got {:?}", instr))),
        }
    }

    fn lower_scale_apply_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scale_apply_aarch64: expected VmInstr::ScaleApply, got {:?}", instr))),
        }
    }

    fn lower_vec_shuffle_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_shuffle_aarch64: expected VmInstr::VecShuffle, got {:?}", instr))),
        }
    }

    fn lower_vec_extract_lane_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_extract_lane_aarch64: expected VmInstr::VecExtractLane, got {:?}", instr))),
        }
    }

    fn lower_vec_insert_lane_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_insert_lane_aarch64: expected VmInstr::VecInsertLane, got {:?}", instr))),
        }
    }

    fn lower_vec_load_const_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_load_const_aarch64: expected VmInstr::VecLoadConst, got {:?}", instr))),
        }
    }
    ...
    fn lower_loop_begin_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_loop_begin_aarch64: expected VmInstr::LoopBegin, got {:?}", instr))),
        }
    }

    fn lower_loop_end_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::LoopEnd => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_loop_end_aarch64: expected VmInstr::LoopEnd, got {:?}", instr))),
        }
    }

    fn lower_break_loop_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_break_loop_aarch64: expected VmInstr::BreakLoop, got {:?}", instr))),
        }
    }

    fn lower_scope_begin_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::ScopeBegin { .. } | VmInstr::ScopeEnd { .. } => {
Ok(())
            }
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scope_begin_aarch64: expected VmInstr::ScopeBegin, got {:?}", instr))),
        }
    }

    fn lower_conditional_skip_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_conditional_skip_aarch64: expected VmInstr::ConditionalSkip, got {:?}", instr))),
        }
    }
    ...
    fn lower_gpr_cond_action_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpr_cond_action_aarch64: expected VmInstr::GprCondAction, got {:?}", instr))),
        }
    }

    fn lower_indirect_jump_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
    ...
    fn lower_conditional_exit_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_conditional_exit_aarch64: expected VmInstr::ConditionalExit, got {:?}", instr))),
        }
    }
    ...
    fn lower_branch_if_ptr_non_null_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_branch_if_ptr_non_null_aarch64: expected VmInstr::BranchIfPtrNonNull, got {:?}", instr))),
        }
    }
    ...
    fn lower_mark_label_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_mark_label_aarch64: expected VmInstr::MarkLabel, got {:?}", instr))),
        }
    }

    // ── Tile ──
    fn lower_tile_config_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_config_aarch64: expected VmInstr::TileConfig, got {:?}", instr))),
        }
    }

    fn lower_tile_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_load_aarch64: expected VmInstr::TileLoad, got {:?}", instr))),
        }
    }
    ...
    fn lower_tile_mma_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_mma_aarch64: expected VmInstr::TileMma, got {:?}", instr))),
        }
    }

    fn lower_tile_store_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_store_aarch64: expected VmInstr::TileStore, got {:?}", instr))),
        }
    }

    fn lower_tile_release_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TileRelease => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_release_aarch64: expected VmInstr::TileRelease, got {:?}", instr))),
        }
    }

    fn lower_tmem_alloc_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TmemAlloc { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tmem_alloc_aarch64: expected VmInstr::TmemAlloc, got {:?}", instr))),
        }
    }

    fn lower_tmem_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TmemLoad { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tmem_load_aarch64: expected VmInstr::TmemLoad, got {:?}", instr))),
        }
    }

    fn lower_tmem_store_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TmemStore { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tmem_store_aarch64: expected VmInstr::TmemStore, got {:?}", instr))),
        }
    }

    fn lower_tmem_dealloc_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TmemDealloc { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tmem_dealloc_aarch64: expected VmInstr::TmemDealloc, got {:?}", instr))),
        }
    }

    // ── Quant ──
    fn lower_kivi_quant_channel_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_kivi_quant_channel_aarch64: expected VmInstr::KiviQuantChannel, got {:?}", instr))),
        }
    }

    fn lower_kivi_quant_token_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_kivi_quant_token_aarch64: expected VmInstr::KiviQuantToken, got {:?}", instr))),
        }
    }

    fn lower_kivi_dequant_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_kivi_dequant_load_aarch64: expected VmInstr::KiviDequantLoad, got {:?}", instr))),
        }
    }

    fn lower_sparse_mask_intersect_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sparse_mask_intersect_aarch64: expected VmInstr::SparseMaskIntersect, got {:?}", instr))),
        }
    }

    fn lower_gguf_sub_scale_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gguf_sub_scale_load_aarch64: expected VmInstr::GgufSubScaleLoad, got {:?}", instr))),
        }
    }

    fn lower_quant_block_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_block_load_aarch64: expected VmInstr::QuantBlockLoad, got {:?}", instr))),
        }
    }

    fn lower_quant_bi_plane_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_bi_plane_load_aarch64: expected VmInstr::QuantBiPlaneLoad, got {:?}", instr))),
        }
    }

    fn lower_quant_broadcast_int_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_broadcast_int_aarch64: expected VmInstr::QuantBroadcastInt, got {:?}", instr))),
        }
    }

    fn lower_quant_scalar_cvt_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_scalar_cvt_load_aarch64: expected VmInstr::QuantScalarCvtLoad, got {:?}", instr))),
        }
    }

    fn lower_quant_load_bytes_vec_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_load_bytes_vec_aarch64: expected VmInstr::QuantLoadBytesVec, got {:?}", instr))),
        }
    }

    fn lower_quant_codebook_lookup_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_codebook_lookup_aarch64: expected VmInstr::QuantCodebookLookup, got {:?}", instr))),
        }
    }

    fn lower_quant_extract_bits_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_extract_bits_aarch64: expected VmInstr::QuantExtractBits, got {:?}", instr))),
        }
    }

    fn lower_quant_dequant_fma_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_dequant_fma_aarch64: expected VmInstr::QuantDequantFma, got {:?}", instr))),
        }
    }

    fn lower_quant_interleave_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_interleave_aarch64: expected VmInstr::QuantInterleave, got {:?}", instr))),
        }
    }

    fn lower_quant_concat_seq_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_concat_seq_aarch64: expected VmInstr::QuantConcatSeq, got {:?}", instr))),
        }
    }

    fn lower_q3_k_decode_step_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
    ...
    fn lower_q5_decode_step_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        let _ = alloc;
        match instr {
            VmInstr::Q5DecodeStep { .. } => {

                Err(CompilerError::CodegenViolation(
                    "Q5DecodeStep AArch64: not yet implemented".into()
                ))
            }
            _ => Err(CompilerError::CodegenViolation(
    ...
    fn lower_bitwise_gemm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::BitwiseGemm { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_bitwise_gemm_aarch64: expected VmInstr::BitwiseGemm, got {:?}", instr))),
        }
    }

    fn lower_sparse_gemm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::SparseGemm { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sparse_gemm_aarch64: expected VmInstr::SparseGemm, got {:?}", instr))),
        }
    }

    fn lower_sparse_fp8_gemm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sparse_fp8_gemm_aarch64: expected VmInstr::SparseFp8Gemm, got {:?}", instr))),
        }
    }

    fn lower_native_fp4_gemm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_native_fp4_gemm_aarch64: expected VmInstr::NativeFp4Gemm, got {:?}", instr))),
        }
    }

    fn lower_native_fp6_gemm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_native_fp6_gemm_aarch64: expected VmInstr::NativeFp6Gemm, got {:?}", instr))),
        }
    }

    fn lower_two_cta_fp4_gemm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_two_cta_fp4_gemm_aarch64: expected VmInstr::TwoCtaFp4Gemm, got {:?}", instr))),
        }
    }

    fn lower_native_fp8_gemm_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_native_fp8_gemm_aarch64: expected VmInstr::NativeFp8Gemm, got {:?}", instr))),
        }
    }

    fn lower_hw_quant_dequant_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_hw_quant_dequant_aarch64: expected VmInstr::HwQuantDequant, got {:?}", instr))),
        }
    }

    // ── GpuComm ──
    fn lower_page_table_addr_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_table_addr_aarch64: expected VmInstr::PageTableAddr, got {:?}", instr))),
        }
    }

    fn lower_page_table_k_v_write_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_table_k_v_write_aarch64: expected VmInstr::PageTableKVWrite, got {:?}", instr))),
        }
    }

    fn lower_page_table_k_v_write_quant_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_table_k_v_write_quant_aarch64: expected VmInstr::PageTableKVWriteQuant, got {:?}", instr))),
        }
    }

    fn lower_shared_mem_swizzle_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_swizzle_aarch64: expected VmInstr::SharedMemSwizzle, got {:?}", instr))),
        }
    }

    fn lower_warp_sync_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_sync_aarch64: expected VmInstr::WarpSync, got {:?}", instr))),
        }
    }

    fn lower_shared_mem_alloc_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_alloc_aarch64: expected VmInstr::SharedMemAlloc, got {:?}", instr))),
        }
    }

    fn lower_shared_mem_store_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_store_aarch64: expected VmInstr::SharedMemStore, got {:?}", instr))),
        }
    }

    fn lower_shared_mem_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_load_aarch64: expected VmInstr::SharedMemLoad, got {:?}", instr))),
        }
    }

    fn lower_shared_mem_async_store_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_async_store_aarch64: expected VmInstr::SharedMemAsyncStore, got {:?}", instr))),
        }
    }

    fn lower_weight_prefetch_async_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_weight_prefetch_async_aarch64: expected VmInstr::WeightPrefetchAsync, got {:?}", instr))),
        }
    }

    fn lower_warp_role_declare_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_role_declare_aarch64: expected VmInstr::WarpRoleDeclare, got {:?}", instr))),
        }
    }

    fn lower_warp_barrier_arrive_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_barrier_arrive_aarch64: expected VmInstr::WarpBarrierArrive, got {:?}", instr))),
        }
    }

    fn lower_warp_barrier_wait_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_barrier_wait_aarch64: expected VmInstr::WarpBarrierWait, got {:?}", instr))),
        }
    }

    fn lower_tma_descriptor_init_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tma_descriptor_init_aarch64: expected VmInstr::TmaDescriptorInit, got {:?}", instr))),
        }
    }

    fn lower_block_sync_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::BlockSync => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_block_sync_aarch64: expected VmInstr::BlockSync, got {:?}", instr))),
        }
    }

    fn lower_warp_reduce_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::WarpReduce { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_reduce_aarch64: expected VmInstr::WarpReduce, got {:?}", instr))),
        }
    }

    fn lower_cluster_barrier_init_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_cluster_barrier_init_aarch64: expected VmInstr::ClusterBarrierInit, got {:?}", instr))),
        }
    }

    fn lower_cluster_store_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_cluster_store_aarch64: expected VmInstr::ClusterStore, got {:?}", instr))),
        }
    }

    fn lower_cluster_load_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::ClusterLoad { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_cluster_load_aarch64: expected VmInstr::ClusterLoad, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_all_reduce_chunk_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_all_reduce_chunk_aarch64: expected VmInstr::AllReduceChunk, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_comm_barrier_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            VmInstr::CommBarrier { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_comm_barrier_aarch64: expected VmInstr::CommBarrier, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_nvlink_async_copy_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_nvlink_async_copy_aarch64: expected VmInstr::NvlinkAsyncCopy, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_remote_page_lookup_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_remote_page_lookup_aarch64: expected VmInstr::RemotePageLookup, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_p2p_page_fetch_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_p2p_page_fetch_aarch64: expected VmInstr::P2pPageFetch, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_rdma_page_fetch_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_rdma_page_fetch_aarch64: expected VmInstr::RdmaPageFetch, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_rdma_page_fetch_compressed_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_rdma_page_fetch_compressed_aarch64: expected VmInstr::RdmaPageFetchCompressed, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_remote_page_attn_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_remote_page_attn_aarch64: expected VmInstr::RemotePageAttn, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_page_migration_lock_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_migration_lock_aarch64: expected VmInstr::PageMigrationLock, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_page_migration_unlock_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_migration_unlock_aarch64: expected VmInstr::PageMigrationUnlock, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_page_location_update_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            #[cfg(feature = "nccl")]
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_location_update_aarch64: expected VmInstr::PageLocationUpdate, got {:?}", instr))),
        }
    }

    // ── Sampling ──
    fn lower_batch_seq_id_lookup_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_batch_seq_id_lookup_aarch64: expected VmInstr::BatchSeqIdLookup, got {:?}", instr))),
        }
    }
    ...
    fn lower_batch_per_seq_argmax_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_batch_per_seq_argmax_aarch64: expected VmInstr::BatchPerSeqArgmax, got {:?}", instr))),
        }
    }

    fn lower_batch_per_seq_stop_check_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_batch_per_seq_stop_check_aarch64: expected VmInstr::BatchPerSeqStopCheck, got {:?}", instr))),
        }
    }
    ...
    fn lower_temperature_scale_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_temperature_scale_aarch64: expected VmInstr::TemperatureScale, got {:?}", instr))),
        }
    }

    fn lower_store_token_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_store_token_aarch64: expected VmInstr::StoreToken, got {:?}", instr))),
        }
    }

    fn lower_check_stop_condition_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_check_stop_condition_aarch64: expected VmInstr::CheckStopCondition, got {:?}", instr))),
        }
    }

    fn lower_softmax_reduce_max_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_softmax_reduce_max_aarch64: expected VmInstr::SoftmaxReduceMax, got {:?}", instr))),
        }
    }

    fn lower_softmax_exp_sum_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_softmax_exp_sum_aarch64: expected VmInstr::SoftmaxExpSum, got {:?}", instr))),
        }
    }

    fn lower_softmax_normalize_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_softmax_normalize_aarch64: expected VmInstr::SoftmaxNormalize, got {:?}", instr))),
        }
    }

    fn lower_sample_top_k_filter_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sample_top_k_filter_aarch64: expected VmInstr::SampleTopKFilter, got {:?}", instr))),
        }
    }

    fn lower_sample_top_p_filter_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sample_top_p_filter_aarch64: expected VmInstr::SampleTopPFilter, got {:?}", instr))),
        }
    }

    fn lower_sample_multinomial_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sample_multinomial_aarch64: expected VmInstr::SampleMultinomial, got {:?}", instr))),
        }
    }

    fn lower_warp_p_r_n_g_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::WarpPRNG { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_p_r_n_g_aarch64: expected VmInstr::WarpPRNG, got {:?}", instr))),
        }
    }

    fn lower_seq_id_lookup_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_seq_id_lookup_aarch64: expected VmInstr::SeqIdLookup, got {:?}", instr))),
        }
    }

    // ── Misc ──
    fn lower_declare_v_reg_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::DeclareVReg { .. } | VmInstr::ReleaseVReg { .. } | VmInstr::Comment(_) => {
Ok(())
            }
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_declare_v_reg_aarch64: expected VmInstr::DeclareVReg, got {:?}", instr))),
        }
    }

    fn lower_hotpatch_slot_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_hotpatch_slot_aarch64: expected VmInstr::HotpatchSlot, got {:?}", instr))),
        }
    }

    fn lower_load_callback_entry_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_load_callback_entry_aarch64: expected VmInstr::LoadCallbackEntry, got {:?}", instr))),
        }
    }

    fn lower_native_call_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_native_call_aarch64: expected VmInstr::NativeCall, got {:?}", instr))),
        }
    }

    fn lower_lz4_decode_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_lz4_decode_aarch64: expected VmInstr::Lz4Decode, got {:?}", instr))),
        }
    }

    fn lower_bit_pack_rle_decode_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_bit_pack_rle_decode_aarch64: expected VmInstr::BitPackRleDecode, got {:?}", instr))),
        }
    }
    ...
    fn lower_debug_breakpoint_aarch64(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_debug_breakpoint_aarch64: expected VmInstr::DebugBreakpoint, got {:?}", instr))),
        }
    }
