...
fn lower_attention_v2(
    ...
    let (k_attn_ptr, v_attn_ptr) = match spec.kv_source {
        KvSource::FromCache => {
            ...
            // SharedKvRef: use the immutable graph donor table for physical KV
            // addressing. The table is generated by gllm from ModelGeometry's
            // effective_kv_layer() and shared with buffer allocation; kernels do
            // not re-derive attention-pattern semantics. An empty table is the
            // identity path for graphs that do not use SharedKvRef.
            let donor_layer_ctr = if graph.kv_donor_map.is_empty() {
                layer_ctr
            } else {
                let donor = prog.alloc_vreg(VRegKind::Ptr, SimdWidth::Scalar);
                prog.emit(VmInstr::LoadLayerWeightOffset {
                    dst: donor,
                    offset_table: graph.kv_donor_map.clone(),
                    layer_idx_reg: layer_ctr,
                });
                donor
            };
            ...
            // K cache base = kv_cache_ptr + donor_layer_ctr * kv_layer_stride
            let layer_off = prog.alloc_vreg(VRegKind::ByteOffset, SimdWidth::Scalar);
            prog.emit(VmInstr::GprBinOp { dst: layer_off, a: donor_layer_ctr, b: GprOperand::Imm(kv_layer_stride as i64), op: GprOp::Mul });
            ...
            // A consumer attention has no valid K/V projection outputs. Bypass
            // the complete cache-populate copy and read the donor slot only.
            // Donor and consumer iterations share one template, so the identity
            // comparison is emitted as VM control flow (not Rust layer unroll).
            ...
            if spec.kv_write {
                let copy_label = prog.alloc_label();
                let copy_done_label = prog.alloc_label();
                if !graph.kv_donor_map.is_empty() {
                    let layer_diff = prog.alloc_vreg(VRegKind::Scalar, SimdWidth::Scalar);
                    prog.emit(VmInstr::GprBinOp {
                        dst: layer_diff,
                        a: layer_ctr,
                        b: GprOperand::VReg(donor_layer_ctr),
                        op: GprOp::Xor,
                    });
                    prog.emit(VmInstr::GprCondAction {
                        cond: GprCondition::CmpEq(layer_diff, 0),
                        action: GprBranchAction::JumpToLabel(copy_label),
                    });
                    prog.emit(VmInstr::UnconditionalBranch { target_label: copy_done_label });
                    prog.emit(VmInstr::MarkLabel { label_id: copy_label });
                }
                ...
                if !graph.kv_donor_map.is_empty() {
                    prog.emit(VmInstr::MarkLabel { label_id: copy_done_label });
                }
            }
