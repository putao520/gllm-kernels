impl X86Lower {
    ...
    // ════════════════════════════════════════════════════════════════════
    // ARCH-LOWER-DISPATCH-LAYERING: L1 变体路由 + L2 叶子 emit
    // lower_instr_inner (L0) 按 VmInstr::category() 路由到 lower_<cat>_x86 (L1),
    // L1 内 match 变体路由到 lower_<variant>_x86 (L2 叶子 fn)。
    // 每个 L2 叶子 fn = 单变体作用域, 行为与原 god-match arm 完全等价 (行为保持)。
    // ════════════════════════════════════════════════════════════════════

    /// L1 变体路由: Memory 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_memory_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::LoadPtr { dst, src } => self.lower_load_ptr_x86(instr, alloc),
            VmInstr::VecLoad { dst, base, offset, width, dtype , predicate: _predicate } => self.lower_vec_load_x86(instr, alloc),
            VmInstr::VecStore { base, offset, src, width, dtype , predicate: _predicate } => self.lower_vec_store_x86(instr, alloc),
            VmInstr::VecNarrow { dst, src, dst_dtype, src_dtype, width } => self.lower_vec_narrow_x86(instr, alloc),
            VmInstr::VecWiden { dst, src, dst_dtype, src_dtype, width } => self.lower_vec_widen_x86(instr, alloc),
            VmInstr::Mov { dst, src, .. } => self.lower_mov_x86(instr, alloc),
            VmInstr::Broadcast { dst, src, dtype, width: _ } => self.lower_broadcast_x86(instr, alloc),
            VmInstr::Prefetch { base, ref offset, distance, hint } => self.lower_prefetch_x86(instr, alloc),
            VmInstr::AtomicAdd { base, ref offset, value, elem_width } => self.lower_atomic_add_x86(instr, alloc),
            VmInstr::MemFence { order } => self.lower_mem_fence_x86(instr, alloc),
            VmInstr::ScalarLoad { dst, base, offset } => self.lower_scalar_load_x86(instr, alloc),
            VmInstr::ScalarStore { base, src, offset } => self.lower_scalar_store_x86(instr, alloc),
            VmInstr::VecScalarStore { base, src, offset } => self.lower_vec_scalar_store_x86(instr, alloc),
            VmInstr::ScalarToIndex { dst, src, stride } => self.lower_scalar_to_index_x86(instr, alloc),
            VmInstr::IndexToScalar { dst, src } => self.lower_index_to_scalar_x86(instr, alloc),
            VmInstr::IntMulStride { dst, src, stride } => self.lower_int_mul_stride_x86(instr, alloc),
            VmInstr::ScalarByteLoad { dst, base, offset } => self.lower_scalar_byte_load_x86(instr, alloc),
            VmInstr::AddPtr { dst, base, offset } => self.lower_add_ptr_x86(instr, alloc),
            VmInstr::StoreConstToStack { rbp_offset, value, elem_width } => self.lower_store_const_to_stack_x86(instr, alloc),
            VmInstr::ActivationSwap { ptr_a, ptr_b } => self.lower_activation_swap_x86(instr, alloc),
            VmInstr::GatherLoad { dst, base, indices, stride, width , dtype: _dtype, predicate: _predicate, } => self.lower_gather_load_x86(instr, alloc),
            VmInstr::ScatterStore { base, indices, src, stride, width , dtype: _dtype, predicate: _predicate, } => self.lower_scatter_store_x86(instr, alloc),
            VmInstr::TableLookup { dst, base, row_index, row_bytes, width } => self.lower_table_lookup_x86(instr, alloc),
            VmInstr::MemCopy { dst, src, bytes, dtype: _, guard, effect } => self.lower_mem_copy_x86(instr, alloc),
            VmInstr::AtomicCAS { dst, ptr, expected, desired, elem_width, success_order: _, failure_order: _ } => self.lower_atomic_cas_x86(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_memory_x86: variant {:?} not in Memory category", instr))),
        }
    }

    /// L1 变体路由: Arith 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_arith_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::VecBinOp { dst, a, b, op, dtype } => self.lower_vec_bin_op_x86(instr, alloc),
            VmInstr::VecShiftImm { dst, a, amount, op, width } => self.lower_vec_shift_imm_x86(instr, alloc),
            VmInstr::VecUnaryOp { dst, a, op } => self.lower_vec_unary_op_x86(instr, alloc),
            VmInstr::VecCmp { dst, a, b, pred } => self.lower_vec_cmp_x86(instr, alloc),
            VmInstr::VecCast { dst, src, from_bits, to_bits } => self.lower_vec_cast_x86(instr, alloc),
            VmInstr::ConditionalSelect { dst, mask, true_val, false_val } => self.lower_conditional_select_x86(instr, alloc),
            VmInstr::Fma { dst, acc, a, b, dtype } => self.lower_fma_x86(instr, alloc),
            VmInstr::HReduce { dst, src, op } => self.lower_h_reduce_x86(instr, alloc),
            VmInstr::Accumulate { acc, src } => self.lower_accumulate_x86(instr, alloc),
            VmInstr::Transcendental { dst, src, func } => self.lower_transcendental_x86(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::Imm(amount), op: GprOp::Shl } => self.lower_gpr_bin_op_x86(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::Imm(amount), op: GprOp::Shr } => self.lower_gpr_bin_op_x86(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::Imm(value), op: GprOp::Sub } => self.lower_gpr_bin_op_x86(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::VReg(b_vreg), op: GprOp::Add } => self.lower_gpr_bin_op_x86(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::Imm(imm), op: GprOp::Add } => self.lower_gpr_bin_op_x86(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::VReg(b_vreg), op: GprOp::Sub } => self.lower_gpr_bin_op_x86(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::Imm(value), op: GprOp::Mul } => self.lower_gpr_bin_op_x86(instr, alloc),
            VmInstr::GprBinOp { dst, a, b: GprOperand::VReg(b_vreg), op: GprOp::Mul } => self.lower_gpr_bin_op_x86(instr, alloc),
            VmInstr::GprBinOp { dst, a, b, op } => self.lower_gpr_bin_op_x86(instr, alloc),
            VmInstr::GprLoadImm { dst, value } => self.lower_gpr_load_imm_x86(instr, alloc),
            VmInstr::DotProduct { acc, a, b, input_dtype, width } => self.lower_dot_product_x86(instr, alloc),
            VmInstr::ScaleApply { dst, acc, scale, zero, input_dtype, .. } => self.lower_scale_apply_x86(instr, alloc),
            VmInstr::GprUnaryOp { dst, src, op } => self.lower_gpr_unary_op_x86(instr, alloc),
            VmInstr::VecShuffle { dst, src, ref mask, width } => self.lower_vec_shuffle_x86(instr, alloc),
            VmInstr::VecExtractLane { dst, src, lane, dtype: _ } => self.lower_vec_extract_lane_x86(instr, alloc),
            VmInstr::VecInsertLane { dst, src_vec, src_scalar, lane, dtype: _ } => self.lower_vec_insert_lane_x86(instr, alloc),
            VmInstr::VecLoadConst { dst, ref values, dtype: _, width } => self.lower_vec_load_const_x86(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_arith_x86: variant {:?} not in Arith category", instr))),
        }
    }

    /// L1 变体路由: Control 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_control_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            VmInstr::BranchIfGprLtU { a, b, target_label } => self.lower_branch_if_gpr_lt_u_x86(instr, alloc),
            VmInstr::UnconditionalBranch { target_label } => self.lower_unconditional_branch_x86(instr, alloc),
            VmInstr::BreakLoop { return_value } => self.lower_break_loop_x86(instr, alloc),
            VmInstr::MarkLabel { label_id } => self.lower_mark_label_x86(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_control_x86: variant {:?} not in Control category", instr))),
        }
    }

    /// L1 变体路由: Tile 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_tile_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TileConfig { rows, cols, dtype } => self.lower_tile_config_x86(instr, alloc),
            VmInstr::TileLoad { dst_tile, base_ptr, k_offset, row_stride, rows, cols, dtype, } => self.lower_tile_load_x86(instr, alloc),
            VmInstr::TileMma { c, a, b, m: _, n: _, k: _, dtype } => self.lower_tile_mma_x86(instr, alloc),
            VmInstr::TileStore { src_tile, base_ptr, out_offset, row_stride, rows, cols, dtype, } => self.lower_tile_store_x86(instr, alloc),
            VmInstr::TileRelease => self.lower_tile_release_x86(instr, alloc),
            VmInstr::TmemAlloc { .. } => self.lower_tmem_alloc_x86(instr, alloc),
            VmInstr::TmemLoad { .. } => self.lower_tmem_load_x86(instr, alloc),
            VmInstr::TmemStore { .. } => self.lower_tmem_store_x86(instr, alloc),
            VmInstr::TmemDealloc { .. } => self.lower_tmem_dealloc_x86(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_x86: variant {:?} not in Tile category", instr))),
        }
    }

    /// L1 变体路由: Quant 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_quant_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::SparseMaskIntersect { dst_k0, dst_k1, a, b } => self.lower_sparse_mask_intersect_x86(instr, alloc),
            VmInstr::KiviQuantChannel { src, dst_ptr, scale_ptr, num_channels, width } => self.lower_kivi_quant_channel_x86(instr, alloc),
            VmInstr::KiviQuantToken { src, dst_ptr, scale_ptr, num_elems, width } => self.lower_kivi_quant_token_x86(instr, alloc),
            VmInstr::KiviDequantLoad { dst, src_ptr, scale_ptr, num_elems, width } => self.lower_kivi_dequant_load_x86(instr, alloc),
            VmInstr::QuantBlockLoad { dst, base, offset, unpack, width } => self.lower_quant_block_load_x86(instr, alloc),
            VmInstr::QuantBiPlaneLoad { dst, qs_base, extra_base, bias, mode, width } => self.lower_quant_bi_plane_load_x86(instr, alloc),
            VmInstr::GgufSubScaleLoad { dst, scales_base, sub_block_idx, width } => self.lower_gguf_sub_scale_load_x86(instr, alloc),
            VmInstr::GgufKQuantScaleLoad { dst, scales_base, sub_block_idx, scales_count, is_q3k_extended, is_min, width } => self.lower_gguf_k_quant_scale_load_x86(instr, alloc),
            VmInstr::QuantBroadcastInt { dst, value, .. } => self.lower_quant_broadcast_int_x86(instr, alloc),
            VmInstr::QuantScalarCvtLoad { dst, base, offset, src_dtype, .. } => self.lower_quant_scalar_cvt_load_x86(instr, alloc),
            VmInstr::QuantLoadBytesVec { dst, base, offset, count, signed, .. } => self.lower_quant_load_bytes_vec_x86(instr, alloc),
            VmInstr::QuantCodebookLookup { dst, indices, codebook_data, bits_per_entry, width, .. } => self.lower_quant_codebook_lookup_x86(instr, alloc),
            VmInstr::QuantExtractBits { dst, src, bit_offset, bit_width, .. } => self.lower_quant_extract_bits_x86(instr, alloc),
            VmInstr::QuantDequantFma { quant_kind, .. } => self.lower_quant_dequant_fma_x86(instr, alloc),
            ...
            VmInstr::Q5DecodeStep { dst, block_base, lane_offset, d_vreg: _, qs_offset, qh_offset, has_min, lanes, width } => self.lower_q5_decode_step_x86(instr, alloc),
            VmInstr::Q5KDecodeStep { dst, block_base, lane_offset, d_vreg: _, qs_offset, qh_offset, lanes, width } => self.lower_q5k_decode_step_x86(instr, alloc),
            VmInstr::Q4KDecodeStep { dst, block_base, lane_offset, d_vreg: _, qs_offset, lanes, width } => self.lower_q4k_decode_step_x86(instr, alloc),
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_x86: variant {:?} not in Quant category", instr))),
        }
    }

    /// L1 变体路由: GpuComm 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_gpu_comm_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::WarpSync | VmInstr::AsyncCopy { .. } | VmInstr::AsyncWait { .. } => self.lower_warp_sync_x86(instr, alloc),
            VmInstr::PageTableAddr { dst, pool_base, page_table_ptr, ki_byte_off, row_bytes, page_size, page_stride, base_offset, seq_pt_offset } => self.lower_page_table_addr_x86(instr, alloc),
            VmInstr::PageTableKVWrite { src, pool_base, page_table_ptr, seq_index, row_bytes, page_size, page_stride, base_offset, width, dtype } => self.lower_page_table_kv_write_x86(instr, alloc),
            VmInstr::PageTableKVWriteQuant { src, pool_base, page_table_ptr, seq_index, quant_row_bytes, fp32_row_bytes: _, page_size, page_stride, base_offset, scale_offset, width: _, kivi_mode, num_elems } => self.lower_page_table_kv_write_quant_x86(instr, alloc),
            VmInstr::SharedMemSwizzle { dst, raw_addr, .. } => self.lower_shared_mem_swizzle_x86(instr, alloc),
            VmInstr::SharedMemAlloc { .. } => self.lower_shared_mem_alloc_x86(instr, alloc),
            VmInstr::SharedMemStore { .. } => self.lower_shared_mem_store_x86(instr, alloc),
            VmInstr::SharedMemLoad { .. } => self.lower_shared_mem_load_x86(instr, alloc),
            VmInstr::SharedMemAsyncStore { .. } | VmInstr::SharedMemAsyncWaitGroup { .. } => self.lower_shared_mem_async_store_x86(instr, alloc),
            VmInstr::WeightPrefetchAsync { .. } | VmInstr::WeightPrefetchWait { .. } => self.lower_weight_prefetch_async_x86(instr, alloc),
            VmInstr::WarpRoleDeclare { .. } => self.lower_warp_role_declare_x86(instr, alloc),
            VmInstr::WarpBarrierArrive { .. } => self.lower_warp_barrier_arrive_x86(instr, alloc),
            VmInstr::WarpBarrierWait { .. } => self.lower_warp_barrier_wait_x86(instr, alloc),
            VmInstr::TmaDescriptorInit { .. } | VmInstr::Tma2DCopy { .. } | VmInstr::BarrierInit { .. } => self.lower_tma_descriptor_init_x86(instr, alloc),
            VmInstr::BlockSync => self.lower_block_sync_x86(instr, alloc),
            VmInstr::WarpReduce { .. } => self.lower_warp_reduce_x86(instr, alloc),
            VmInstr::ClusterBarrierInit { .. } => self.lower_cluster_barrier_init_x86(instr, alloc),
            VmInstr::ClusterStore { .. } => self.lower_cluster_store_x86(instr, alloc),
            VmInstr::ClusterLoad { .. } => self.lower_cluster_load_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::AllReduceChunk { sendbuf, recvbuf, count, dtype, op, rank, world_size, chunk_idx } => self.lower_all_reduce_chunk_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::CommBarrier { .. } => self.lower_comm_barrier_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::NvlinkAsyncCopy { .. } => self.lower_nvlink_async_copy_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::RemotePageLookup { .. } => self.lower_remote_page_lookup_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::P2pPageFetch { .. } => self.lower_p2p_page_fetch_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::RdmaPageFetch { .. } => self.lower_rdma_page_fetch_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::RdmaPageFetchCompressed { .. } => self.lower_rdma_page_fetch_compressed_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::RemotePageAttn { .. } => self.lower_remote_page_attn_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::PageMigrationLock { .. } => self.lower_page_migration_lock_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::PageMigrationUnlock { .. } => self.lower_page_migration_unlock_x86(instr, alloc),
            #[cfg(feature = "nccl")]
            VmInstr::PageLocationUpdate { .. } => self.lower_page_location_update_x86(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpu_comm_x86: variant {:?} not in GpuComm category", instr))),
        }
    }

    /// L1 变体路由: Sampling 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_sampling_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            VmInstr::SoftmaxReduceMax { dst, logits_ptr, vocab_bytes, width } => self.lower_softmax_reduce_max_x86(instr, alloc),
            VmInstr::SoftmaxExpSum { sum_dst, logits_ptr, max_val, vocab_bytes, width } => self.lower_softmax_exp_sum_x86(instr, alloc),
            VmInstr::SoftmaxNormalize { logits_ptr, sum_val, vocab_bytes, width } => self.lower_softmax_normalize_x86(instr, alloc),
            VmInstr::SampleTopKFilter { probs_ptr, indices_ptr, k_ptr, vocab_bytes, width } => self.lower_sample_top_k_filter_x86(instr, alloc),
            VmInstr::SampleTopPFilter { probs_ptr, p_ptr, vocab_bytes, width } => self.lower_sample_top_p_filter_x86(instr, alloc),
            VmInstr::SampleMultinomial { dst, probs_ptr, rng_state_ptr, vocab_bytes, width } => self.lower_sample_multinomial_x86(instr, alloc),
            VmInstr::WarpPRNG { dst, rng_state_ptr } => self.lower_warp_prng_x86(instr, alloc),
            VmInstr::SeqIdLookup { dst, token_index, seq_meta_base, num_seqs, seq_meta_stride } => self.lower_seq_id_lookup_x86(instr, alloc),
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sampling_x86: variant {:?} not in Sampling category", instr))),
        }
    }

    /// L1 变体路由: Misc 类 VmInstr → L2 叶子 emit fn。
    /// ISA 不支持的变体 (类别错配) 返回 Err (NO_SILENT_FALLBACK, 非 catch-all NOP)。
    fn lower_misc_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::HotpatchSlot { slot_id, .. } => self.lower_hotpatch_slot_x86(instr, alloc),
            VmInstr::LoadCallbackEntry { table_ptr, slot_id, fn_ptr_out, ctx_out } => self.lower_load_callback_entry_x86(instr, alloc),
            VmInstr::NativeCall { ret_val, fn_ptr, ctx_ptr } => self.lower_native_call_x86(instr, alloc),
            VmInstr::DeclareVReg { .. } | VmInstr::ReleaseVReg { .. } | VmInstr::Comment(_) => self.lower_declare_v_reg_x86(instr, alloc),
            VmInstr::Lz4Decode { src_ptr, dst_ptr, compressed_size, decompressed_size } => self.lower_lz4_decode_x86(instr, alloc),
            VmInstr::BitPackRleDecode { src_ptr, dst_ptr, compressed_size, nibble_bits, element_count } => self.lower_bit_pack_rle_decode_x86(instr, alloc),
            VmInstr::DebugBreakpoint { label } => self.lower_debug_breakpoint_x86(instr, alloc),
    ...
    // ── L2 叶子 emit 函数 (单变体作用域, 行为与原 arm 等价) ──

    fn lower_load_ptr_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_load_ptr_x86: expected VmInstr::LoadPtr, got {:?}", instr))),
        }
    }

    fn lower_vec_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_load_x86: expected VmInstr::VecLoad, got {:?}", instr))),
        }
    }

    fn lower_vec_store_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_store_x86: expected VmInstr::VecStore, got {:?}", instr))),
        }
    }

    fn lower_vec_narrow_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_narrow_x86: expected VmInstr::VecNarrow, got {:?}", instr))),
        }
    }

    fn lower_vec_widen_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_widen_x86: expected VmInstr::VecWiden, got {:?}", instr))),
        }
    }

    fn lower_mov_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::Mov { dst, src, .. } => {
                if self.use_avx512 {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_mov_x86: expected VmInstr::Mov, got {:?}", instr))),
        }
    }

    fn lower_broadcast_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_broadcast_x86: expected VmInstr::Broadcast, got {:?}", instr))),
        }
    }

    // Prefetch: x86 prefetcht0/t1/t2/nta
    fn lower_prefetch_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_prefetch_x86: expected VmInstr::Prefetch, got {:?}", instr))),
        }
    }

    // §13.6 AtomicAdd: lock add [base + offset], value
    // elem_width=4 → dword (u32, relaxed), elem_width=8 → mfence + qword (u64, AcqRel)
    fn lower_atomic_add_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_atomic_add_x86: expected VmInstr::AtomicAdd, got {:?}", instr))),
        }
    }
    ...
    fn lower_mem_fence_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_mem_fence_x86: expected VmInstr::MemFence, got {:?}", instr))),
        }
    }

    fn lower_scalar_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scalar_load_x86: expected VmInstr::ScalarLoad, got {:?}", instr))),
        }
    }

    fn lower_scalar_store_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scalar_store_x86: expected VmInstr::ScalarStore, got {:?}", instr))),
        }
    }

    fn lower_vec_scalar_store_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_scalar_store_x86: expected VmInstr::VecScalarStore, got {:?}", instr))),
        }
    }

    fn lower_scalar_to_index_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scalar_to_index_x86: expected VmInstr::ScalarToIndex, got {:?}", instr))),
        }
    }

    fn lower_index_to_scalar_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_index_to_scalar_x86: expected VmInstr::IndexToScalar, got {:?}", instr))),
        }
    }

    fn lower_int_mul_stride_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_int_mul_stride_x86: expected VmInstr::IntMulStride, got {:?}", instr))),
        }
    }

    fn lower_scalar_byte_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scalar_byte_load_x86: expected VmInstr::ScalarByteLoad, got {:?}", instr))),
        }
    }

    fn lower_add_ptr_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_add_ptr_x86: expected VmInstr::AddPtr, got {:?}", instr))),
        }
    }

    fn lower_store_const_to_stack_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_store_const_to_stack_x86: expected VmInstr::StoreConstToStack, got {:?}", instr))),
        }
    }

    // §3.7 ActivationSwap: 交换 ping-pong buffer 指针寄存器
    fn lower_activation_swap_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
    ...
    fn lower_gather_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gather_load_x86: expected VmInstr::GatherLoad, got {:?}", instr))),
        }
    }

    // ScatterStore: 向量索引存储 — 生成标量循环（每次存储一个元素）。
    fn lower_scatter_store_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scatter_store_x86: expected VmInstr::ScatterStore, got {:?}", instr))),
        }
    }

    // TableLookup: 行查表 — 组合 IntMulStride + AddPtr + VecLoad。
    fn lower_table_lookup_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TableLookup { dst, base, row_index, row_bytes, width } => {
                self.emit_table_lookup(*dst, *base, *row_index, *row_bytes, *width, alloc)
            }
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_table_lookup_x86: expected VmInstr::TableLookup, got {:?}", instr))),
        }
    }

    fn lower_mem_copy_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_mem_copy_x86: expected VmInstr::MemCopy, got {:?}", instr))),
        }
    }

    // ── REQ-VR-009: AtomicCAS — x86_64 lock cmpxchg ──
    fn lower_atomic_cas_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_atomic_cas_x86: expected VmInstr::AtomicCAS, got {:?}", instr))),
        }
    }

    fn lower_vec_bin_op_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_bin_op_x86: expected VmInstr::VecBinOp, got {:?}", instr))),
        }
    }
    ...
    fn lower_vec_shift_imm_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_shift_imm_x86: expected VmInstr::VecShiftImm, got {:?}", instr))),
        }
    }

    fn lower_vec_unary_op_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_unary_op_x86: expected VmInstr::VecUnaryOp, got {:?}", instr))),
        }
    }
    ...
    fn lower_vec_cmp_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_cmp_x86: expected VmInstr::VecCmp, got {:?}", instr))),
        }
    }

    fn lower_vec_cast_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_cast_x86: expected VmInstr::VecCast, got {:?}", instr))),
        }
    }

    fn lower_conditional_select_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_conditional_select_x86: expected VmInstr::ConditionalSelect, got {:?}", instr))),
        }
    }

    fn lower_fma_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_fma_x86: expected VmInstr::Fma, got {:?}", instr))),
        }
    }

    fn lower_h_reduce_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_h_reduce_x86: expected VmInstr::HReduce, got {:?}", instr))),
        }
    }

    fn lower_accumulate_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_accumulate_x86: expected VmInstr::Accumulate, got {:?}", instr))),
        }
    }

    fn lower_transcendental_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_transcendental_x86: expected VmInstr::Transcendental, got {:?}", instr))),
        }
    }

    fn lower_gpr_load_imm_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpr_load_imm_x86: expected VmInstr::GprLoadImm, got {:?}", instr))),
        }
    }

    fn lower_dot_product_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_dot_product_x86: expected VmInstr::DotProduct, got {:?}", instr))),
        }
    }
    ...
    fn lower_scale_apply_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scale_apply_x86: expected VmInstr::ScaleApply, got {:?}", instr))),
        }
    }

    fn lower_gpr_unary_op_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpr_unary_op_x86: expected VmInstr::GprUnaryOp, got {:?}", instr))),
        }
    }

    // ── REQ-VR-005: VecShuffle — x86_64 vpshufb ──
    fn lower_vec_shuffle_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_shuffle_x86: expected VmInstr::VecShuffle, got {:?}", instr))),
        }
    }

    // ── REQ-VR-006: VecExtractLane — x86_64 vpextrd/vpextrq ──
    fn lower_vec_extract_lane_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_extract_lane_x86: expected VmInstr::VecExtractLane, got {:?}", instr))),
        }
    }

    // ── REQ-VR-006: VecInsertLane — x86_64 vpinsrd ──
    fn lower_vec_insert_lane_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_insert_lane_x86: expected VmInstr::VecInsertLane, got {:?}", instr))),
        }
    }

    // ── REQ-VR-008: VecLoadConst — x86_64 broadcast + vmovd ──
    fn lower_vec_load_const_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_vec_load_const_x86: expected VmInstr::VecLoadConst, got {:?}", instr))),
        }
    }

    fn lower_gpr_bin_op_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpr_bin_op_x86: expected VmInstr::GprBinOp, got {:?}", instr))),
        }
    }
    ...
    fn lower_loop_begin_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_loop_begin_x86: expected VmInstr::LoopBegin, got {:?}", instr))),
        }
    }
    ...
    fn lower_loop_end_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_loop_end_x86: expected VmInstr::LoopEnd, got {:?}", instr))),
        }
    }

    fn lower_scope_begin_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::ScopeBegin { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scope_begin_x86: expected VmInstr::ScopeBegin, got {:?}", instr))),
        }
    }

    fn lower_scope_end_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::ScopeEnd { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_scope_end_x86: expected VmInstr::ScopeEnd, got {:?}", instr))),
        }
    }

    // ── §13 FP1: 条件跳过 (Gate-First 死神经元掩码) ──
    fn lower_conditional_skip_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_conditional_skip_x86: expected VmInstr::ConditionalSkip, got {:?}", instr))),
        }
    }

    fn lower_gpr_cond_action_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gpr_cond_action_x86: expected VmInstr::GprCondAction, got {:?}", instr))),
        }
    }

    // ── §15 MoE 核内分发: 间接跳转 ──
    fn lower_indirect_jump_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
    ...
    fn lower_conditional_exit_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_conditional_exit_x86: expected VmInstr::ConditionalExit, got {:?}", instr))),
        }
    }

    fn lower_branch_if_ptr_non_null_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_branch_if_ptr_non_null_x86: expected VmInstr::BranchIfPtrNonNull, got {:?}", instr))),
        }
    }

    fn lower_branch_if_gpr_zero_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_branch_if_gpr_zero_x86: expected VmInstr::BranchIfGprZero, got {:?}", instr))),
        }
    }

    fn lower_branch_if_gpr_lt_u_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_branch_if_gpr_lt_u_x86: expected VmInstr::BranchIfGprLtU, got {:?}", instr))),
        }
    }

    fn lower_unconditional_branch_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_unconditional_branch_x86: expected VmInstr::UnconditionalBranch, got {:?}", instr))),
        }
    }

    fn lower_break_loop_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_break_loop_x86: expected VmInstr::BreakLoop, got {:?}", instr))),
        }
    }

    fn lower_mark_label_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_mark_label_x86: expected VmInstr::MarkLabel, got {:?}", instr))),
        }
    }

    fn lower_tile_config_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_config_x86: expected VmInstr::TileConfig, got {:?}", instr))),
        }
    }

    fn lower_tile_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_load_x86: expected VmInstr::TileLoad, got {:?}", instr))),
        }
    }

    fn lower_tile_mma_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_mma_x86: expected VmInstr::TileMma, got {:?}", instr))),
        }
    }

    fn lower_tile_store_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_store_x86: expected VmInstr::TileStore, got {:?}", instr))),
        }
    }

    fn lower_tile_release_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TileRelease => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tile_release_x86: expected VmInstr::TileRelease, got {:?}", instr))),
        }
    }

    fn lower_tmem_alloc_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TmemAlloc { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tmem_alloc_x86: expected VmInstr::TmemAlloc, got {:?}", instr))),
        }
    }

    fn lower_tmem_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TmemLoad { .. } => {
                Err(CompilerError::CodegenViolation(
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tmem_load_x86: expected VmInstr::TmemLoad, got {:?}", instr))),
        }
    }

    fn lower_tmem_store_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TmemStore { .. } => {
                Err(CompilerError::CodegenViolation(
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tmem_store_x86: expected VmInstr::TmemStore, got {:?}", instr))),
        }
    }

    fn lower_tmem_dealloc_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::TmemDealloc { .. } => {
                Err(CompilerError::CodegenViolation(
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tmem_dealloc_x86: expected VmInstr::TmemDealloc, got {:?}", instr))),
        }
    }

    // ── SPARSE_MASK_INTERSECT: 稀疏掩码硬件交集 (AVX-512 / Granite Rapids+) ──
    fn lower_sparse_mask_intersect_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sparse_mask_intersect_x86: expected VmInstr::SparseMaskIntersect, got {:?}", instr))),
        }
    }

    // KIVI 量化: x86 暂用标量循环 fallback (未来 AVX-512 优化)
    fn lower_kivi_quant_channel_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_kivi_quant_channel_x86: expected VmInstr::KiviQuantChannel, got {:?}", instr))),
        }
    }

    fn lower_kivi_quant_token_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_kivi_quant_token_x86: expected VmInstr::KiviQuantToken, got {:?}", instr))),
        }
    }

    fn lower_kivi_dequant_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_kivi_dequant_load_x86: expected VmInstr::KiviDequantLoad, got {:?}", instr))),
        }
    }

    fn lower_quant_block_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::QuantBlockLoad { dst, base, offset, unpack, width } => {
                self.lower_quant_block_load(*dst, *base, offset, unpack, width, alloc)
            }
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_block_load_x86: expected VmInstr::QuantBlockLoad, got {:?}", instr))),
        }
    }

    fn lower_quant_bi_plane_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_bi_plane_load_x86: expected VmInstr::QuantBiPlaneLoad, got {:?}", instr))),
        }
    }

    fn lower_gguf_sub_scale_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gguf_sub_scale_load_x86: expected VmInstr::GgufSubScaleLoad, got {:?}", instr))),
        }
    }

    fn lower_gguf_k_quant_scale_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_gguf_k_quant_scale_load_x86: expected VmInstr::GgufKQuantScaleLoad, got {:?}", instr))),
        }
    }
    ...
    fn lower_quant_broadcast_int_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_broadcast_int_x86: expected VmInstr::QuantBroadcastInt, got {:?}", instr))),
        }
    }
    ...
    fn lower_quant_scalar_cvt_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_scalar_cvt_load_x86: expected VmInstr::QuantScalarCvtLoad, got {:?}", instr))),
        }
    }
    ...
    fn lower_quant_load_bytes_vec_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_load_bytes_vec_x86: expected VmInstr::QuantLoadBytesVec, got {:?}", instr))),
        }
    }

    fn lower_quant_codebook_lookup_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_codebook_lookup_x86: expected VmInstr::QuantCodebookLookup, got {:?}", instr))),
        }
    }
    ...
    fn lower_quant_extract_bits_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_extract_bits_x86: expected VmInstr::QuantExtractBits, got {:?}", instr))),
        }
    }

    fn lower_quant_dequant_fma_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_dequant_fma_x86: expected VmInstr::QuantDequantFma, got {:?}", instr))),
        }
    }
    ...
    fn lower_quant_interleave_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_interleave_x86: expected VmInstr::QuantInterleave, got {:?}", instr))),
        }
    }
    ...
    fn lower_quant_concat_seq_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_quant_concat_seq_x86: expected VmInstr::QuantConcatSeq, got {:?}", instr))),
        }
    }
    ...
    /// Q5_0/Q5_1 单步解码 x86 lowering (BCE-20260710-Q5_0-HIGHBITS 同类横扫).
    /// 委托 q5_0/q5_1_decode_step_native (has_min 区分).
    fn lower_q5_decode_step_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::Q5DecodeStep { dst, block_base, lane_offset, d_vreg: _, qs_offset, qh_offset, has_min, lanes, width } => {
                ...
                let fn_ptr = if *has_min {
                    crate::asm::x86_64::quant_gemv::q5_1_decode_step_native as *const () as u64
                } else {
                    crate::asm::x86_64::quant_gemv::q5_0_decode_step_native as *const () as u64
                };
                self.asm.mov(rax, fn_ptr).map_err(Self::err)?;
                self.asm.call(rax).map_err(Self::err)?;
                ...
                match width {
                    ...
                    SimdWidth::Warp(_) | SimdWidth::Scalable => {
                        return Err(CompilerError::CodegenViolation(
                            "Q5DecodeStep: Warp/Scalable SIMD width not supported on x86".into()
                        ));
                    }
                }
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_q5_decode_step_x86: expected VmInstr::Q5DecodeStep, got {:?}", instr))),
        }
    }

    /// Q5_K 单步解码 lowering (BCE-20260710-Q5_K-HIGHBITS).
    /// 委托 q5k_decode_step_native (d+dmin+scales+转置 qh+SPLIT qs per 64-group+get_scale_min_k4).
    fn lower_q5k_decode_step_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::Q5KDecodeStep { dst, block_base, lane_offset, d_vreg: _, qs_offset, qh_offset, lanes, width } => {
                ...
                let fn_ptr = crate::asm::x86_64::quant_gemv::q5k_decode_step_native as *const () as u64;
                self.asm.mov(rax, fn_ptr).map_err(Self::err)?;
                self.asm.call(rax).map_err(Self::err)?;
                ...
                match width {
                    ...
                    SimdWidth::Warp(_) | SimdWidth::Scalable => {
                        return Err(CompilerError::CodegenViolation(
                            "Q5KDecodeStep: Warp/Scalable SIMD width not supported on x86".into()
                        ));
                    }
                }
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_q5k_decode_step_x86: expected VmInstr::Q5KDecodeStep, got {:?}", instr))),
        }
    }

    /// Q4_K 单步解码 lowering (BCE-20260731-Q4K-MONOLITHIC-DECODE).
    /// The native helper performs the four 32+32 nibble groups and packed scale/min decode.
    fn lower_q4k_decode_step_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::Q4KDecodeStep { dst, block_base, lane_offset, d_vreg: _, qs_offset, lanes, width } => {
                let is_w512 = matches!(width, SimdWidth::W512);
                if is_w512 && !self.use_avx512 {
                    return Err(CompilerError::CodegenViolation(
                ...
                self.asm.vxorps(xmm0, xmm0, xmm0).map_err(Self::err)?;
                self.asm.mov(rdx, *qs_offset as u64).map_err(Self::err)?;
                self.asm.mov(rcx, *lanes as u64).map_err(Self::err)?;
                let aligned_out_bytes = ((*lanes as i32 + 3) & !3) * 4;
                self.asm.mov(r10, rsp).map_err(Self::err)?;
                self.asm.and(rsp, -16i32).map_err(Self::err)?;
                self.asm.sub(rsp, aligned_out_bytes).map_err(Self::err)?;
                self.asm.lea(r8, qword_ptr(rsp)).map_err(Self::err)?;
                let fn_ptr = crate::asm::x86_64::quant_gemv::q4k_decode_step_native as *const () as u64;
                self.asm.mov(rax, fn_ptr).map_err(Self::err)?;
                self.asm.call(rax).map_err(Self::err)?;
                match width {
                    SimdWidth::W128 => self.asm.vmovups(dst_ymm, xmmword_ptr(rsp)).map_err(Self::err)?,
                    SimdWidth::W256 => self.asm.vmovups(dst_ymm, ymmword_ptr(rsp)).map_err(Self::err)?,
                    SimdWidth::W512 => {
                        let (dst_zmm, _) = self.resolve_zmm_or_spill_write(*dst, alloc, 0)?;
                        self.asm.vmovups(dst_zmm, zmmword_ptr(rsp)).map_err(Self::err)?;
                    }
                    ...
                    SimdWidth::Warp(_) | SimdWidth::Scalable => {
                        return Err(CompilerError::CodegenViolation(
                            "Q4KDecodeStep: Warp/Scalable SIMD width not supported on x86".into(),
                        ));
                    }
                }
                self.asm.mov(rsp, r10).map_err(Self::err)?;
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_q4k_decode_step_x86: expected VmInstr::Q4KDecodeStep, got {:?}", instr))),
        }
    }

    fn lower_bitwise_gemm_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_bitwise_gemm_x86: expected VmInstr::BitwiseGemm, got {:?}", instr))),
        }
    }

    fn lower_sparse_gemm_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::SparseGemm { .. } => {
                Err(CompilerError::CodegenViolation(
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sparse_gemm_x86: expected VmInstr::SparseGemm, got {:?}", instr))),
        }
    }

    fn lower_sparse_fp8_gemm_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sparse_fp8_gemm_x86: expected VmInstr::SparseFp8Gemm, got {:?}", instr))),
        }
    }

    fn lower_native_fp4_gemm_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
    ...
    fn lower_native_fp8_gemm_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_native_fp8_gemm_x86: expected VmInstr::NativeFp8Gemm, got {:?}", instr))),
        }
    }

    fn lower_hw_quant_dequant_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_hw_quant_dequant_x86: expected VmInstr::HwQuantDequant, got {:?}", instr))),
        }
    }

    // ── GPU 操作 (x86 不支持，返回错误) ──
    fn lower_warp_sync_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_sync_x86: expected VmInstr::WarpSync, got {:?}", instr))),
        }
    }

    // PagedAttention: 从 page table 计算物理地址
    fn lower_page_table_addr_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_table_addr_x86: expected VmInstr::PageTableAddr, got {:?}", instr))),
        }
    }

    // PagedAttention: 将 KV 行写入 page pool (§20 BCI-005: seq_index 为运行时 GPR)
    fn lower_page_table_kv_write_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_table_kv_write_x86: expected VmInstr::PageTableKVWrite, got {:?}", instr))),
        }
    }

    // PagedAttention + KIVI quantized KV write-back
    fn lower_page_table_kv_write_quant_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_table_kv_write_quant_x86: expected VmInstr::PageTableKVWriteQuant, got {:?}", instr))),
        }
    }

    // SharedMemSwizzle: x86_64 无 shared memory banking，直接传递 raw_addr → dst
    fn lower_shared_mem_swizzle_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_swizzle_x86: expected VmInstr::SharedMemSwizzle, got {:?}", instr))),
        }
    }

    // GPU 专用指令 — x86 不支持
    fn lower_shared_mem_alloc_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_alloc_x86: expected VmInstr::SharedMemAlloc, got {:?}", instr))),
        }
    }

    fn lower_shared_mem_store_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_store_x86: expected VmInstr::SharedMemStore, got {:?}", instr))),
        }
    }

    fn lower_shared_mem_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_load_x86: expected VmInstr::SharedMemLoad, got {:?}", instr))),
        }
    }

    fn lower_shared_mem_async_store_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_shared_mem_async_store_x86: expected VmInstr::SharedMemAsyncStore, got {:?}", instr))),
        }
    }

    fn lower_weight_prefetch_async_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_weight_prefetch_async_x86: expected VmInstr::WeightPrefetchAsync, got {:?}", instr))),
        }
    }

    fn lower_warp_role_declare_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_role_declare_x86: expected VmInstr::WarpRoleDeclare, got {:?}", instr))),
        }
    }

    fn lower_warp_barrier_arrive_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_barrier_arrive_x86: expected VmInstr::WarpBarrierArrive, got {:?}", instr))),
        }
    }

    fn lower_warp_barrier_wait_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_barrier_wait_x86: expected VmInstr::WarpBarrierWait, got {:?}", instr))),
        }
    }

    fn lower_tma_descriptor_init_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_tma_descriptor_init_x86: expected VmInstr::TmaDescriptorInit, got {:?}", instr))),
        }
    }

    fn lower_block_sync_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::BlockSync => {
                Err(CompilerError::CodegenViolation(
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_block_sync_x86: expected VmInstr::BlockSync, got {:?}", instr))),
        }
    }

    fn lower_warp_reduce_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::WarpReduce { .. } => {
                Err(CompilerError::CodegenViolation(
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_reduce_x86: expected VmInstr::WarpReduce, got {:?}", instr))),
        }
    }

    fn lower_cluster_barrier_init_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_cluster_barrier_init_x86: expected VmInstr::ClusterBarrierInit, got {:?}", instr))),
        }
    }

    fn lower_cluster_store_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_cluster_store_x86: expected VmInstr::ClusterStore, got {:?}", instr))),
        }
    }

    fn lower_cluster_load_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::ClusterLoad { .. } => {
                Err(CompilerError::CodegenViolation(
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_cluster_load_x86: expected VmInstr::ClusterLoad, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_all_reduce_chunk_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_all_reduce_chunk_x86: expected VmInstr::AllReduceChunk, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_comm_barrier_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::CommBarrier { .. } => {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_comm_barrier_x86: expected VmInstr::CommBarrier, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_nvlink_async_copy_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_nvlink_async_copy_x86: expected VmInstr::NvlinkAsyncCopy, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_remote_page_lookup_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_remote_page_lookup_x86: expected VmInstr::RemotePageLookup, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_p2p_page_fetch_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_p2p_page_fetch_x86: expected VmInstr::P2pPageFetch, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_rdma_page_fetch_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_rdma_page_fetch_x86: expected VmInstr::RdmaPageFetch, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_rdma_page_fetch_compressed_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_rdma_page_fetch_compressed_x86: expected VmInstr::RdmaPageFetchCompressed, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_remote_page_attn_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_remote_page_attn_x86: expected VmInstr::RemotePageAttn, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_page_migration_lock_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_migration_lock_x86: expected VmInstr::PageMigrationLock, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_page_migration_unlock_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_migration_unlock_x86: expected VmInstr::PageMigrationUnlock, got {:?}", instr))),
        }
    }

    #[cfg(feature = "nccl")]
    fn lower_page_location_update_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_page_location_update_x86: expected VmInstr::PageLocationUpdate, got {:?}", instr))),
        }
    }

    // §20 BCI-004: Batch per-token seq_id lookup via cumsum linear scan.
    fn lower_batch_seq_id_lookup_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_batch_seq_id_lookup_x86: expected VmInstr::BatchSeqIdLookup, got {:?}", instr))),
        }
    }
    ...
    fn lower_batch_per_seq_argmax_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_batch_per_seq_argmax_x86: expected VmInstr::BatchPerSeqArgmax, got {:?}", instr))),
        }
    }

    // §20 BCI-006: Per-sequence stop condition check + active_flag update.
    fn lower_batch_per_seq_stop_check_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_batch_per_seq_stop_check_x86: expected VmInstr::BatchPerSeqStopCheck, got {:?}", instr))),
        }
    }
    ...
    fn lower_argmax_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_argmax_x86: expected VmInstr::Argmax, got {:?}", instr))),
        }
    }

    fn lower_temperature_scale_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_temperature_scale_x86: expected VmInstr::TemperatureScale, got {:?}", instr))),
        }
    }

    fn lower_store_token_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_store_token_x86: expected VmInstr::StoreToken, got {:?}", instr))),
        }
    }

    fn lower_check_stop_condition_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_check_stop_condition_x86: expected VmInstr::CheckStopCondition, got {:?}", instr))),
        }
    }

    fn lower_softmax_reduce_max_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_softmax_reduce_max_x86: expected VmInstr::SoftmaxReduceMax, got {:?}", instr))),
        }
    }

    fn lower_softmax_exp_sum_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_softmax_exp_sum_x86: expected VmInstr::SoftmaxExpSum, got {:?}", instr))),
        }
    }

    fn lower_softmax_normalize_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_softmax_normalize_x86: expected VmInstr::SoftmaxNormalize, got {:?}", instr))),
        }
    }

    fn lower_sample_top_k_filter_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sample_top_k_filter_x86: expected VmInstr::SampleTopKFilter, got {:?}", instr))),
        }
    }

    fn lower_sample_top_p_filter_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sample_top_p_filter_x86: expected VmInstr::SampleTopPFilter, got {:?}", instr))),
        }
    }

    fn lower_sample_multinomial_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_sample_multinomial_x86: expected VmInstr::SampleMultinomial, got {:?}", instr))),
        }
    }

    fn lower_warp_prng_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::WarpPRNG { dst, rng_state_ptr } => {
                self.lower_warp_prng(*dst, *rng_state_ptr, alloc)
            }
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_warp_prng_x86: expected VmInstr::WarpPRNG, got {:?}", instr))),
        }
    }

    // ── §20 BCI-004: SeqIdLookup — cumsum(prompt_lens) linear search ──
    fn lower_seq_id_lookup_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_seq_id_lookup_x86: expected VmInstr::SeqIdLookup, got {:?}", instr))),
        }
    }

    // ── §9.2 热修补: 5-byte NOP 占位符 ──
    fn lower_hotpatch_slot_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_hotpatch_slot_x86: expected VmInstr::HotpatchSlot, got {:?}", instr))),
        }
    }

    fn lower_load_callback_entry_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_load_callback_entry_x86: expected VmInstr::LoadCallbackEntry, got {:?}", instr))),
        }
    }

    fn lower_native_call_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_native_call_x86: expected VmInstr::NativeCall, got {:?}", instr))),
        }
    }

    // 元操作: 不生成代码
    fn lower_declare_v_reg_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            VmInstr::DeclareVReg { .. } | VmInstr::ReleaseVReg { .. } | VmInstr::Comment(_) => {
                Ok(())
            }
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_declare_v_reg_x86: expected VmInstr::DeclareVReg, got {:?}", instr))),
        }
    }

    fn lower_lz4_decode_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_lz4_decode_x86: expected VmInstr::Lz4Decode, got {:?}", instr))),
        }
    }

    fn lower_bit_pack_rle_decode_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_bit_pack_rle_decode_x86: expected VmInstr::BitPackRleDecode, got {:?}", instr))),
        }
    }

    fn lower_debug_breakpoint_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_debug_breakpoint_x86: expected VmInstr::DebugBreakpoint, got {:?}", instr))),
        }
    }

    fn lower_debug_marker_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_debug_marker_x86: expected VmInstr::DebugMarker, got {:?}", instr))),
        }
    }

    fn lower_debug_probe_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
            ...
            _ => Err(CompilerError::CodegenViolation(
                format!("lower_debug_probe_x86: expected VmInstr::DebugProbe, got {:?}", instr))),
        }
    }

    fn lower_debug_break_if_x86(&mut self, instr: &VmInstr, alloc: &RegAllocation) -> Result<(), CompilerError> {
        match instr {
