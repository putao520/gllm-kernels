...
impl TemplateInterpreter {
    ...
    fn interpret_body_step(&self, step: &AlgoStep, body_ops: &mut Vec<TraceOp>, env: &SlotEnv) {
        match step {
            ...
            AlgoStep::Barrier { barrier_name } => {
                body_ops.push(TraceOp::SyncBarrier {
                    name: barrier_name.to_string(),
                });
            }
    ...
    fn interpret_trace_step(&self, ts: &AlgoTraceStep, ops: &mut Vec<TraceOp>, env: &SlotEnv) {
        match ts {
            ...
            AlgoTraceStep::VecLoadIndexed { base, offset } => {
                ...
                ops.push(TraceOp::VecLoadIndexed {
                    base: base_slot,
                    offset: offset_slot,
                });
            }
            AlgoTraceStep::VecStoreIndexed { base, offset, src } => {
                ...
                let offset_slot = env.get(offset);
                let src_slot = env.get(src);
                ops.push(TraceOp::VecStoreIndexed {
                    base: base_slot,
                    offset: offset_slot,
                    value: src_slot,
                });
...
mod tests {
    ...
    fn test_interpret_gemnaive() {
        ...
        assert!(
            ...
            "First op should be Input(0) for a_ptr"
        );
        assert!(
            matches!(ops[1], TraceOp::Input(1)),
            "Second op should be Input(1) for b_ptr"
        );
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::Loop { .. })),
    ...
    fn test_interpret_gemblis() {
        ...
        let inputs = TemplateInputs::gemm();
        let ops = interp.instantiate(
            &crate::compiler::codegen::vm::algo_templates::GEMM_BLIS,
            &inputs,
        );
        ...
        assert!(
            contains_panel_load(&ops),
            "GEMM_BLIS body should contain PanelLoad"
        );
    }
    ...
    fn test_interpret_norm_rms() {
        ...
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::HReduce { .. })),
            ...
            for op in ops {
                match op {
                    TraceOp::PanelLoad { base, offset, .. } => {
                        assert!(
                            (base.0 as usize) < num_inputs,
                            "PanelLoad base {} must reference inputs (< {})",
                            base,
                            num_inputs
                        );
                        assert!(
                            (offset.0 as usize) < num_inputs,
                            "PanelLoad offset {} must reference inputs (< {})",
                            offset,
                            num_inputs
                        );
                    }
            ...
            for op in body {
                match op {
                    TraceOp::Fma(a, b, c) => {
                        ...
                        assert!(
                            (a.0 as usize) < num_inputs || *a == ValueId(0),
                            "Fma a={} must reference valid outer slot",
                            a
                        );
                    }
    ...
    fn test_gemm_blis_body_fma_refs_outer() {
        ...
        assert!(
            !fmas.is_empty(),
            "GEMM_BLIS should produce FMA ops in loop body"
        );
    }
    ...
    fn count_instr_kinds(
        prog: &crate::compiler::codegen::vm::instr::VmProgram,
    ) -> (usize, usize, usize) {
    ...
    fn test_at011_gemm_template_vs_direct_emit_alignment() {
        ...
        assert!(
            trace_ops
                .iter()
                .any(|op| matches!(op, TraceOp::Loop { .. })),
        ...
        let result = auto_select::auto_lower_trace_raw(
            &mut prog,
            &trace_ops,
            &[a_ptr, b_ptr, c_ptr, a_off, b_off, c_off],
            width,
            QuantPrecision::F32,
        );
        assert!(
    ...
    fn test_at011_norm_rms_template_produces_valid_trace() {
        ...
        assert!(
            result.is_ok(),
            "NORM_RMS auto_lower should succeed: {:?}",
            result
        );
    ...
    fn test_at011_rope_template_produces_valid_trace() {
        ...
        assert!(
            result.is_ok(),
            "ROPE_STANDARD auto_lower should succeed: {:?}",
            result
        );
        assert!(
            prog.instrs.len() > 0,
            "ROPE_STANDARD should produce VmInstrs"
        );
    }
    ...
    fn test_at011_sampling_templates_all_valid() {
        ...
        let templates: &[(
            &str,
            &crate::compiler::codegen::vm::algo_template::AlgoTemplate,
        )] = &[
            (
                "ARGMAX",
                &crate::compiler::codegen::vm::algo_templates::sampling::SAMPLING_ARGMAX,
            ),
            (
                "TEMPERATURE",
                &crate::compiler::codegen::vm::algo_templates::sampling::SAMPLING_TEMPERATURE,
            ),
            (
                "SOFTMAX",
                &crate::compiler::codegen::vm::algo_templates::sampling::SAMPLING_SOFTMAX,
            ),
            (
                "TOP_K",
                &crate::compiler::codegen::vm::algo_templates::sampling::SAMPLING_TOP_K,
            ),
            (
                "TOP_P",
                &crate::compiler::codegen::vm::algo_templates::sampling::SAMPLING_TOP_P,
            ),
            (
                "MULTINOMIAL",
                &crate::compiler::codegen::vm::algo_templates::sampling::SAMPLING_MULTINOMIAL,
            ),
        ];
        ...
        for (name, template) in templates {
            ...
            assert!(
                ops.iter().any(|op| matches!(op, TraceOp::Input(0))),
                "SAMPLING_{} should reference logits input",
                name
            );
            ...
            assert!(
                result.is_ok(),
                "SAMPLING_{} auto_lower should succeed: {:?}",
                name,
                result
            );
            assert!(
                prog.instrs.len() > 0,
                "SAMPLING_{} should produce VmInstrs",
                name
            );
        }
    }
    ...
    fn test_param_table_new_is_empty() {
        ...
        assert_eq!(
            ...
            None,
            "empty ParamTable should return None for any key"
        );
        assert_eq!(
            table.resolve("nonexistent"),
            1,
            "resolve should return default 1 for missing key"
        );
    }
    ...
    fn test_param_table_usize_boundary_values() {
        ...
        assert_eq!(
            table.resolve("zero"),
            0,
            "resolve returns actual value when present"
        );
        assert_eq!(
            table.resolve("max"),
            usize::MAX,
            "resolve handles usize::MAX"
        );
    }
    ...
    fn test_interpreter_instantiate_empty_template() {
        ...
        assert_eq!(
            ops.len(),
            1,
            "empty template should produce exactly one Input(0) op"
        );
    ...
    fn test_interpreter_with_params_resolves_loop_bound() {
        ...
        assert!(
            bound.is_some(),
            "GEMM_NAIVE with params should have a Loop with Const bound"
        );
    ...
    fn test_interpreter_activation_silu_produces_two_ops() {
        use crate::compiler::codegen::vm::algo_template::{ActivationKind, AlgoStep, AlgoTemplate};
        use crate::compiler::codegen::vm::algo_template::{AlgoStrategy, DeviceReq};
        ...
        assert!(
            ops.len() >= 3,
            "Silu activation should produce Input + Sigmoid + Mul, got {} ops",
            ops.len()
        );
        assert!(
            matches!(ops[0], TraceOp::Input(0)),
    ...
    fn test_interpreter_activation_relu_produces_max() {
        ...
        assert_eq!(
            ops.len(),
            2,
            "Relu should produce Input + Max, got {} ops",
            ops.len()
        );
        assert!(
            matches!(ops[1], TraceOp::Max(_, _)),
            "second op should be Max"
        );
    }

    #[test]
    fn test_interpreter_softmax_produces_softmax_op() {
        use crate::compiler::codegen::vm::algo_template::{AlgoStep, AlgoTemplate};
        use crate::compiler::codegen::vm::algo_template::{AlgoStrategy, DeviceReq};
        ...
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::Softmax { .. })),
    ...
    fn test_interpreter_shared_mem_declare_and_barrier() {
        ...
        assert!(
            ops.iter()
                .any(|op| matches!(op, TraceOp::SharedMemDeclare { bytes: 4096, .. })),
            "should produce SharedMemDeclare with 4096 bytes"
        );
        assert!(
            ops.iter()
                .any(|op| matches!(op, TraceOp::SyncBarrier { .. })),
    ...
    fn test_param_table_from_template_returns_empty() {
        ...
        assert_eq!(
            table.get("any_key"),
            None,
            "from_template returns empty table"
        );
    ...
    fn test_interpreter_trace_body_binop_sub() {
        use crate::compiler::codegen::vm::algo_template::{
            AlgoStep, AlgoStrategy, AlgoTemplate, AlgoTraceStep, DeviceReq, TraceBinOp,
        };
        ...
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::Sub(_, _))),
    ...
    fn test_interpreter_seq_nests_steps() {
        use crate::compiler::codegen::vm::algo_template::{
            AlgoStep, AlgoStrategy, AlgoTemplate, DeviceReq,
        };
        ...
        let template = AlgoTemplate {
            ...
            strategy: AlgoStrategy::GemmNaive,
            device_req: DeviceReq::CpuAny,
            steps: &[AlgoStep::Seq(&[AlgoStep::Softmax, AlgoStep::Softmax])],
            params: &[],
            micro_kernel: None,
        };
        ...
        let softmax_count = ops
            .iter()
            .filter(|op| matches!(op, TraceOp::Softmax { .. }))
            .count();
    ...
    fn test_interpreter_tile_config_mma_release() {
        ...
        assert!(
            ops.iter()
                .any(|op| matches!(op, TraceOp::TileConfig { rows: 16, cols: 16 })),
            "should produce TileConfig with resolved params"
        );
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::TileMma { .. })),
            "should produce TileMma"
        );
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::TileRelease)),
    ...
    fn test_interpreter_activation_gelu_produces_tanh() {
        use crate::compiler::codegen::vm::algo_template::{
            ActivationKind, AlgoStep, AlgoStrategy, AlgoTemplate, DeviceReq,
        };
        ...
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::Tanh(_))),
    ...
    fn test_interpreter_trace_body_load_const() {
        use crate::compiler::codegen::vm::algo_template::{
            AlgoStep, AlgoStrategy, AlgoTemplate, AlgoTraceStep, DeviceReq,
        };
        ...
        let template = AlgoTemplate {
            ...
            strategy: AlgoStrategy::GemmNaive,
            device_req: DeviceReq::CpuAny,
            steps: &[AlgoStep::TraceBody(&[AlgoTraceStep::LoadConst {
                value: 3.14159,
            }])],
            params: &[],
            micro_kernel: None,
        ...
        assert!(
            ops.iter()
                .any(|op| matches!(op, TraceOp::Const(v) if (*v - 3.14159).abs() < 1e-10)),
    ...
    fn test_interpreter_embedding_gather_produces_vec_load_indexed() {
        ...
        assert!(
            ops.iter()
                .any(|op| matches!(op, TraceOp::VecLoadIndexed { .. })),
    ...
    fn test_interpreter_moe_router_gemv_produces_fma() {
        ...
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::Fma(_, _, _))),
    ...
    fn test_interpreter_async_wait_and_zero_fill() {
        ...
        assert!(
            ops.iter()
                .any(|op| matches!(op, TraceOp::AsyncWaitGroup { n: 0 })),
            "should produce AsyncWaitGroup with group=0"
        );
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::Input(_))),
    ...
    fn test_interpreter_trace_body_unary_exp() {
        use crate::compiler::codegen::vm::algo_template::{
            AlgoStep, AlgoStrategy, AlgoTemplate, AlgoTraceStep, DeviceReq, TraceUnaryOp,
        };
        ...
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::Exp(_))),
    ...
    fn test_interpreter_trace_body_unary_neg_abs_recip_log() {
        ...
        assert!(
            neg_ops.iter().any(|op| matches!(op, TraceOp::Neg(_))),
            "Neg should produce TraceOp::Neg"
        );

        let mut interp = TemplateInterpreter::new(params.clone());
        let abs_ops = interp.instantiate(&ABS_TMPL, &inputs);
        assert!(
            abs_ops.iter().any(|op| matches!(op, TraceOp::Abs(_))),
            "Abs should produce TraceOp::Abs"
        );

        let mut interp = TemplateInterpreter::new(params.clone());
        let recip_ops = interp.instantiate(&RECIP_TMPL, &inputs);
        assert!(
            recip_ops.iter().any(|op| matches!(op, TraceOp::Recip(_))),
            "Recip should produce TraceOp::Recip"
        );

        let mut interp = TemplateInterpreter::new(params);
        let log_ops = interp.instantiate(&LOG_TMPL, &inputs);
        assert!(
            log_ops.iter().any(|op| matches!(op, TraceOp::Log(_))),
            "Log should produce TraceOp::Log"
        );
    }
    ...
    fn test_interpreter_trace_body_binop_mul_and_max() {
        ...
        assert!(
            mul_ops.len() >= 3,
            "Mul BinOp should produce at least 3 ops, got {}",
            mul_ops.len()
        );
        assert!(
            mul_ops.iter().any(|op| matches!(op, TraceOp::Mul(_, _))),
        ...
        assert!(
            max_ops.len() >= 3,
            "Max BinOp should produce at least 3 ops, got {}",
            max_ops.len()
        );
        assert!(
            max_ops.iter().any(|op| matches!(op, TraceOp::Max(_, _))),
    ...
    fn test_interpreter_trace_body_fma_step() {
        ...
        assert!(
            ops.len() >= 4,
            "Fma step should produce at least 4 ops, got {}",
            ops.len()
        );
    }
    ...
    fn test_interpreter_trace_body_hreduce_sum_and_max() {
        use crate::compiler::codegen::vm::algo_template::{
            AlgoStep, AlgoStrategy, AlgoTemplate, AlgoTraceStep, DeviceReq, ReduceKind,
        };
        ...
        let max_ops = interp2.instantiate(&MAX_TMPL, &inputs);
        assert!(
            max_ops.iter().any(|op| matches!(
                op,
                TraceOp::HReduce {
                    op: crate::compiler::trace::ReduceKind::Max,
                    ..
                }
            )),
    ...
    fn test_interpreter_trace_body_broadcast_and_vec_store_indexed() {
        ...
        assert!(
            ops.iter()
                .any(|op| matches!(op, TraceOp::BroadcastScalar { .. })),
        ...
        } = outer_loop.unwrap()
        {
            ...
            } = inner_loop.unwrap()
            {
                assert_eq!(
                    *inner_bound,
                    BoundExpr::Const(8),
                    "inner loop bound should be 8"
                );
                assert!(
                    inner_body
                        .iter()
                        .any(|op| matches!(op, TraceOp::Softmax { .. })),
    ...
    fn test_interpreter_epilogue_step() {
        use crate::compiler::codegen::vm::algo_template::{
            AlgoStep, AlgoStrategy, AlgoTemplate, DeviceReq, EpilogueOp,
        };
        ...
        assert!(
            ops.iter()
                .any(|op| matches!(op, TraceOp::EpilogueChain { .. })),
        ...
        if let Some(TraceOp::EpilogueChain { ops: ep_ops }) = ops.iter().find_map(|op| {
            ...
            assert_eq!(
                ep_ops.len(),
                2,
                "epilogue should contain 2 ops (BiasAdd + Relu)"
            );
        }
    }
    ...
    fn test_interpreter_moe_top_k_step() {
        ...
        assert!(
            ops.len() > 6,
            "MoeTopK with gemm inputs should produce Input ops for inputs plus one more, got {}",
            ops.len()
        );
    }
    ...
    fn test_interpreter_row_copy_step() {
        ...
        let input_count = ops
            .iter()
            .filter(|op| matches!(op, TraceOp::Input(_)))
            .count();
        assert!(
            input_count >= 8,
            "RowCopy should produce at least 8 Input ops total (6 gemm + 2 rowcopy), got {}",
            input_count
        );
    }
    ...
    fn test_interpreter_body_reduce_uses_env_panel_a() {
        ...
        static TMPL: AlgoTemplate = AlgoTemplate {
            ...
            steps: &[AlgoStep::Loop {
                bound: "outer",
                step: "outer_step",
                body: &[AlgoStep::Reduce {
                    op: crate::compiler::codegen::vm::algo_template::ReduceOp::Sum,
                }],
            }],
            params: &[],
            micro_kernel: None,
        ...
        if let Some(TraceOp::Loop { body, .. }) =
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            assert!(
                body.iter().any(|op| matches!(op, TraceOp::HReduce { .. })),
            ...
            let src_value = body.iter().find_map(|op| {
                if let TraceOp::HReduce { src, op: _ } = op {
                    Some(*src)
                } else {
                    None
                }
    ...
    fn test_interpreter_body_dequantize_uses_env_panel_a() {
        use crate::compiler::codegen::vm::algo_template::{
            AlgoStep, AlgoStrategy, AlgoTemplate, BlockUnpackMode, DeviceReq,
        };
        ...
        static TMPL: AlgoTemplate = AlgoTemplate {
            ...
            steps: &[AlgoStep::Loop {
                bound: "n",
                step: "n_step",
                body: &[AlgoStep::Dequantize {
                    mode: BlockUnpackMode::Q4_0,
                }],
            }],
            params: &[],
            micro_kernel: None,
        ...
        {
            assert!(
                body.iter().any(|op| matches!(op, TraceOp::Input(_))),
    ...
    fn test_interpreter_body_softmax_uses_env_panel_a() {
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            ...
            if let Some(TraceOp::Softmax { src, dst }) = softmax {
                assert_eq!(
                    *src, *dst,
                    "Body Softmax src and dst should be identical (in-place)"
                );
                assert_eq!(*src, ValueId(0),
    ...
    fn test_interpreter_dequantize_mxfp4_step() {
        ...
        static TMPL: AlgoTemplate = AlgoTemplate {
            ...
            strategy: AlgoStrategy::GemmNaive,
            device_req: DeviceReq::CpuAny,
            steps: &[AlgoStep::Dequantize {
                mode: BlockUnpackMode::Mxfp4,
            }],
            params: &[],
            micro_kernel: None,
        };
        ...
        assert!(
            ops.len() >= 2,
            "Should have at least Input + Dequantize output, got {}",
            ops.len()
        );
        assert!(
            matches!(ops[0], TraceOp::Input(0)),
            "First op should be Input(0) for data"
        );
    }
    ...
    fn test_interpreter_reduce_max_and_min_variants() {
        use crate::compiler::codegen::vm::algo_template::{
            AlgoStep, AlgoStrategy, AlgoTemplate, DeviceReq, ReduceOp,
        };
        ...
        assert!(
            max_ops
                .iter()
                .any(|op| matches!(op, TraceOp::HReduce { .. })),
    ...
    fn test_interpreter_trace_body_binop_div_and_min() {
        ...
        assert!(
            div_ops.iter().any(|op| matches!(op, TraceOp::Div(_, _))),
        ...
        let min_ops = interp2.instantiate(&MIN_TMPL, &inputs);
        assert!(
            min_ops.iter().any(|op| matches!(op, TraceOp::Min(_, _))),
    ...
    fn test_interpreter_trace_body_unary_sqrt_rsqrt_sigmoid() {
        ...
        assert!(
            sqrt_ops.iter().any(|op| matches!(op, TraceOp::Sqrt(_))),
        ...
        assert!(
            rsqrt_ops.iter().any(|op| matches!(op, TraceOp::Rsqrt(_))),
        ...
        assert!(
            sigmoid_ops
                .iter()
                .any(|op| matches!(op, TraceOp::Sigmoid(_))),
    ...
    fn test_interpreter_body_smem_async_barrier() {
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            assert!(
                body.iter()
                    .any(|op| matches!(op, TraceOp::SharedMemDeclare { bytes: 8192, .. })),
                "Body should contain SharedMemDeclare with 8192 bytes"
            );
            assert!(
                body.iter()
                    .any(|op| matches!(op, TraceOp::AsyncCopyToShared { bytes: 4096, .. })),
                "Body should contain AsyncCopyToShared with 4096 bytes"
            );
            assert!(
                body.iter()
                    .any(|op| matches!(op, TraceOp::AsyncWaitGroup { n: 1 })),
    ...
    fn test_interpreter_body_tile_config_mma_release() {
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            ...
            assert!(
                body.iter().any(|op| matches!(op, TraceOp::TileMma { .. })),
                "Body should contain TileMma"
            );
            assert!(
                body.iter().any(|op| matches!(op, TraceOp::TileRelease)),
    ...
    fn test_interpreter_body_activation_silu_and_relu() {
        ...
        if let Some(TraceOp::Loop { body, .. }) = silu_ops
            .iter()
            .find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            assert!(
                body.iter().any(|op| matches!(op, TraceOp::Sigmoid(_))),
                "Body Silu should produce Sigmoid"
            );
            assert!(
                body.iter().any(|op| matches!(op, TraceOp::Mul(_, _))),
        ...
        if let Some(TraceOp::Loop { body, .. }) = relu_ops
            .iter()
            .find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            assert!(
                body.iter().any(|op| matches!(op, TraceOp::Max(_, _))),
    ...
    fn test_interpreter_body_embedding_gather() {
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            let vli = body
                .iter()
                .find(|op| matches!(op, TraceOp::VecLoadIndexed { .. }));
            assert!(
                vli.is_some(),
                "Body EmbeddingGather should produce VecLoadIndexed"
            );
            if let Some(TraceOp::VecLoadIndexed { base, offset }) = vli {
                ...
                assert_eq!(
                    *offset,
                    ValueId(3),
                    "offset should be a_offset = ValueId(3)"
                );
            }
        } else {
            panic!("Template should produce a Loop op");
    ...
    fn test_interpreter_pack_buffer_resolves_params() {
        ...
        let pack = ops
            .iter()
            .find(|op| matches!(op, TraceOp::PackBuffer { .. }));
        assert!(
            pack.is_some(),
            "PackBuffer step should produce TraceOp::PackBuffer"
        );
        if let Some(TraceOp::PackBuffer {
            rows, cols, layout, ..
        }) = pack
        {
            ...
            assert_eq!(
                *layout,
                PackLayout::RowMajor,
                "PackBuffer layout should be RowMajor"
            );
        }
    }
    ...
    fn test_interpreter_store_result_panel_store() {
        ...
        let store = ops
            .iter()
            .find(|op| matches!(op, TraceOp::PanelStore { .. }));
        assert!(
            store.is_some(),
            "StoreResult step should produce TraceOp::PanelStore"
        );
        if let Some(TraceOp::PanelStore {
            base,
            offset,
            rows,
            cols,
        }) = store
        {
            ...
            assert_eq!(
                *base,
                ValueId(2),
                "PanelStore base should be c_ptr = ValueId(2)"
            );
            assert_eq!(
                *offset,
                ValueId(5),
                "PanelStore offset should be c_offset = ValueId(5)"
            );
    ...
    fn test_interpreter_load_panel_matrix_b_and_c() {
        ...
        }) = panels.first()
        {
            assert_eq!(
                *base,
                ValueId(1),
                "Panel B base should be b_ptr = ValueId(1)"
            );
            assert_eq!(
                *offset,
                ValueId(4),
                "Panel B offset should be b_offset = ValueId(4)"
            );
            assert_eq!(*rows, 32);
            assert_eq!(*cols, 16);
        }
        ...
        }) = panels.last()
        {
            assert_eq!(
                *base,
                ValueId(2),
                "Panel C base should be c_ptr = ValueId(2)"
            );
            assert_eq!(
                *offset,
                ValueId(5),
                "Panel C offset should be c_offset = ValueId(5)"
            );
            assert_eq!(*rows, 8);
            assert_eq!(*cols, 16);
        }
    ...
    fn test_interpreter_async_copy_to_smem_top_level() {
        ...
        let copy = ops
            .iter()
            .find(|op| matches!(op, TraceOp::AsyncCopyToShared { .. }));
        assert!(
            copy.is_some(),
            "AsyncCopyToSmem should produce TraceOp::AsyncCopyToShared"
        );
    ...
    fn test_interpreter_body_store_result() {
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            ...
            assert!(
                store.is_some(),
                "Body StoreResult should produce PanelStore"
            );
            if let Some(TraceOp::PanelStore {
                base,
                offset,
    ...
    fn test_template_inputs_moe_preset_name_ordering() {
        ...
        assert_eq!(
            moe.names[0], "hidden_ptr",
            "first moe input should be hidden_ptr"
        );
        assert_eq!(
            moe.names[1], "weight_ptr",
            "second moe input should be weight_ptr"
        );
        assert_eq!(
            moe.names[2], "output_ptr",
            "third moe input should be output_ptr"
        );
        assert_eq!(
            moe.names[3], "seq_offset",
            "fourth moe input should be seq_offset"
        );
    }
    ...
    fn test_interpreter_trace_body_binop_add_slot_indices() {
        ...
        if let Some(TraceOp::Add(a, b)) = add_op {
            assert_eq!(
                *a,
                ValueId(0),
                "Add a should reference env slot for x = ValueId(0)"
            );
            assert_eq!(
                *b,
                ValueId(1),
                "Add b should reference env slot for y = ValueId(1)"
            );
        }
    }
    ...
    fn test_interpreter_trace_body_hreduce_min() {
        ...
        assert!(
            hreduce.is_some(),
            "HReduce Min should produce TraceOp::HReduce"
        );
        if let Some(TraceOp::HReduce { src, op }) = hreduce {
            ...
            assert_eq!(
                *src,
                ValueId(0),
                "HReduce src should reference env slot for src = ValueId(0)"
            );
            assert!(
                matches!(op, crate::compiler::trace::ReduceKind::Min),
    ...
    fn test_interpreter_trace_body_vec_load_indexed_slots() {
        ...
        assert!(
            vli.is_some(),
            "VecLoadIndexed trace step should produce TraceOp::VecLoadIndexed"
        );
        if let Some(TraceOp::VecLoadIndexed { base, offset }) = vli {
            assert_eq!(
                *base,
                ValueId(0),
                "base should reference env slot for table = ValueId(0)"
            );
            assert_eq!(
                *offset,
                ValueId(1),
                "offset should reference env slot for idx = ValueId(1)"
            );
        }
    }
    ...
    fn test_param_table_resolve_zero_vs_missing() {
        ...
        assert_eq!(
            table.resolve("zero_key"),
            0,
            "resolve should return 0 when explicitly set"
        );
        assert_eq!(
            table.resolve("missing_key"),
            1,
            "resolve should return default 1 for missing"
        );
    }
    ...
    fn test_template_inputs_gemm_names_exact_match() {
        ...
        assert_eq!(
            gemm.names,
            vec!["a_ptr", "b_ptr", "c_ptr", "a_offset", "b_offset", "c_offset"],
    ...
    fn test_interpreter_empty_template_exactly_n_inputs() {
        use crate::compiler::codegen::vm::algo_template::{AlgoStrategy, AlgoTemplate, DeviceReq};
        ...
        assert_eq!(
            ops.len(),
            4,
            "empty template with 4 inputs should produce exactly 4 Input ops"
        );
        for (i, op) in ops.iter().enumerate() {
            assert!(
                matches!(op, TraceOp::Input(n) if *n as usize == i),
                "op[{}] should be Input({})",
                i,
                i
            );
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            ...
            }) = pack
            {
                assert_eq!(
                    *src,
                    ValueId(6),
                    "PackBuffer src should reference panel_a = ValueId(6)"
                );
                assert_eq!(
                    *dst,
                    ValueId(6),
                    "PackBuffer dst should reference panel_a = ValueId(6)"
                );
                assert_eq!(*rows, 4, "PackBuffer rows should resolve to 4");
    ...
    fn test_interpreter_trace_body_multiple_load_const_ordering() {
        ...
        assert!(
            (consts[1] - 2.0).abs() < 1e-10,
            "Second const should be 2.0"
        );
    ...
    fn test_interpreter_trace_body_cast_preserves_precisions() {
        ...
        if let Some(TraceOp::Cast { src, from, to }) = cast {
            assert_eq!(
                *src,
                ValueId(0),
                "Cast src should reference env slot for data"
            );
    ...
    fn test_interpreter_load_panel_a_slot_indices_and_dims() {
        ...
        let panel = ops
            .iter()
            .find(|op| matches!(op, TraceOp::PanelLoad { .. }));
        assert!(
            panel.is_some(),
            "LoadPanel A should produce TraceOp::PanelLoad"
        );
        if let Some(TraceOp::PanelLoad {
            base,
            offset,
        ...
        }) = panel
        {
            assert_eq!(
                *base,
                ValueId(0),
                "Panel A base should be a_ptr = ValueId(0)"
            );
            assert_eq!(
                *offset,
                ValueId(3),
                "Panel A offset should be a_offset = ValueId(3)"
            );
            assert_eq!(*rows, 8, "rows should resolve to 8");
    ...
    fn test_interpreter_seq_preserves_step_order() {
        ...
        static STEPS: &[AlgoStep] = &[AlgoStep::Seq(&[
        ...
        let const_positions: Vec<(usize, f64)> = ops
            .iter()
            .enumerate()
            .filter_map(|(i, op)| {
                if let TraceOp::Const(v) = op {
        ...
        assert_eq!(
            const_positions.len(),
            2,
            "Should produce exactly 2 Const ops"
        );
        assert!(
            (const_positions[0].1 - 7.0).abs() < 1e-10,
            "First const should be 7.0"
        );
        assert!(
            (const_positions[1].1 - 11.0).abs() < 1e-10,
            "Second const should be 11.0"
        );
        assert!(
            const_positions[0].0 < const_positions[1].0,
            "Consts should appear in declared order"
        );

        let softmax_pos = ops
            .iter()
            .position(|op| matches!(op, TraceOp::Softmax { .. }));
        assert!(softmax_pos.is_some(), "Should produce Softmax");
        assert!(
            softmax_pos.unwrap() > const_positions[1].0,
    ...
    fn test_interpreter_loop_empty_body() {
        ...
        }) = loop_op
        {
            ...
            assert!(
                body.is_empty(),
                "empty body step list should produce empty Loop body"
            );
        }
    }
    ...
    fn test_interpreter_trace_body_load_then_store_indexed() {
        ...
        if let Some(TraceOp::VecLoadIndexed { base, offset }) = vli {
            assert_eq!(
                *base,
                ValueId(0),
                "VLI base should be env 'base' = ValueId(0)"
            );
            assert_eq!(
                *offset,
                ValueId(1),
                "VLI offset should be env 'off' = ValueId(1)"
            );
        }

        let vsi = ops
            .iter()
            .find(|op| matches!(op, TraceOp::VecStoreIndexed { .. }));
        assert!(vsi.is_some(), "Should produce VecStoreIndexed");
        if let Some(TraceOp::VecStoreIndexed {
            base,
            offset,
            value,
        }) = vsi
        {
            assert_eq!(
                *base,
                ValueId(0),
                "VSI base should be env 'base' = ValueId(0)"
            );
            assert_eq!(
                *offset,
                ValueId(1),
                "VSI offset should be env 'off' = ValueId(1)"
            );
            assert_eq!(
                *value,
                ValueId(2),
                "VSI value should be env 'val' = ValueId(2)"
            );
        }
    }
    ...
    fn test_interpreter_body_zero_fill_uses_c_ptr() {
        ...
        static TMPL: AlgoTemplate = AlgoTemplate {
            ...
            steps: &[AlgoStep::Loop {
                bound: "tiles",
                step: "tile_step",
                body: &[AlgoStep::ZeroFill {
                    bytes_param: "fill_bytes",
                }],
            }],
            params: &[],
            micro_kernel: None,
    ...
    fn test_interpreter_param_overwrite_affects_loop_bound() {
        ...
        if let Some(TraceOp::Loop { bound, .. }) =
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            assert_eq!(
                *bound,
    ...
    fn test_interpreter_body_row_copy_uses_a_ptr_and_c_ptr() {
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            ...
            assert_eq!(
                body_inputs.len(),
                2,
                "Body RowCopy should produce exactly 2 Input ops, got {}",
                body_inputs.len()
            );
    ...
    fn test_interpreter_body_trace_body_fma_uses_env() {
        ...
        static TMPL: AlgoTemplate = AlgoTemplate {
            ...
            steps: &[AlgoStep::Loop {
                bound: "tiles",
                step: "tile_step",
                body: &[AlgoStep::TraceBody(&[AlgoTraceStep::Fma {
                    acc: "a_ptr",
                    a: "b_ptr",
                    b: "c_ptr",
                }])],
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            ...
            assert!(
                fma.is_some(),
                "Body TraceBody Fma should produce TraceOp::Fma"
            );
            if let Some(TraceOp::Fma(a, b, c)) = fma {
    ...
    fn test_interpreter_body_activation_gelu_produces_tanh() {
        ...
        {
            assert!(
                body.iter().any(|op| matches!(op, TraceOp::Tanh(_))),
    ...
    fn test_interpreter_body_epilogue_preserves_ops() {
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            let ep = body
                .iter()
                .find(|op| matches!(op, TraceOp::EpilogueChain { .. }));
    ...
    fn test_interpreter_conditional_multiple_body_steps() {
        ...
        assert_eq!(
            softmax_count, 2,
            "Conditional with 2 Softmax steps should emit both, got {} softmaps",
            softmax_count
        );
    }
    ...
    fn test_interpreter_body_conditional_emits_steps_into_loop() {
        ...
        {
            assert!(
                body.iter().any(|op| matches!(op, TraceOp::Softmax { .. })),
    ...
    fn test_interpreter_body_trace_body_hreduce_uses_env() {
        ...
        static TMPL: AlgoTemplate = AlgoTemplate {
            ...
            steps: &[AlgoStep::Loop {
                bound: "n",
                step: "n_step",
                body: &[AlgoStep::TraceBody(&[AlgoTraceStep::HReduce {
                    src: "data",
                    op: ReduceKind::Sum,
                }])],
            }],
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            ...
            assert!(
                hreduce.is_some(),
                "Body TraceBody HReduce should produce TraceOp::HReduce"
            );
            if let Some(TraceOp::HReduce { src, op }) = hreduce {
                assert_eq!(
                    *src,
                ...
                assert!(
                    matches!(op, crate::compiler::trace::ReduceKind::Sum),
    ...
    fn test_interpreter_body_load_panel_b_resolves_gemm_slots() {
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            ...
            assert!(
                panel.is_some(),
                "Body LoadPanel B should produce TraceOp::PanelLoad"
            );
            if let Some(TraceOp::PanelLoad {
                base,
                offset,
    ...
    fn test_interpreter_body_tile_mma_uses_env_slots() {
            ...
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            ...
            assert!(
                tile_mma.is_some(),
                "Body TileMma should produce TraceOp::TileMma"
            );
            if let Some(TraceOp::TileMma {
                c,
                a,
                b,
                m: _,
    ...
    fn test_interpreter_seq_mixed_trace_and_structural_order() {
        ...
        assert!(
            (const_positions[0].1 - 42.0).abs() < 1e-10,
            "First const should be 42.0"
        );
        assert!(
            (const_positions[1].1 - 99.0).abs() < 1e-10,
            "Second const should be 99.0"
        );
        assert!(softmax_pos.is_some(), "Should produce Softmax");
        // Softmax must appear between the two Const ops
        let softmax_idx = softmax_pos.unwrap();
        assert!(
            const_positions[0].0 < softmax_idx,
            "Softmax should appear after first Const"
        );
        assert!(
            softmax_idx < const_positions[1].0,
    ...
    fn test_interpreter_idempotent_instantiate() {
        ...
        for (i, (op1, op2)) in ops1.iter().zip(ops2.iter()).enumerate() {
            assert!(
                ...
                "op[{}] variant mismatch between two instantiations",
                i
            );
        }
    }
    ...
    fn test_interpreter_full_gemm_pipeline_ops_order() {
        ...
        fn contains_op_recursive(ops: &[TraceOp], pred: &dyn Fn(&TraceOp) -> bool) -> bool {
            for op in ops {
                ...
                if let TraceOp::Loop { body, .. } = op {
                    if contains_op_recursive(body, pred) {
                        return true;
                    }
                }
            }
        ...
        assert!(
            contains_op_recursive(&ops, &|op| matches!(op, TraceOp::PanelLoad { .. })),
        ...
        assert!(
            contains_op_recursive(&ops, &|op| matches!(op, TraceOp::Fma(_, _, _))),
            "Pipeline should contain Fma (from MicroKernel inside Loop)"
        );
        assert!(
            ops.iter()
                .any(|op| matches!(op, TraceOp::PanelStore { .. })),
            "Pipeline should contain PanelStore (from StoreResult)"
        );

        // Verify ordering: PanelLoad before Loop, PanelStore after Loop
        let panel_load_pos = ops
            .iter()
            .position(|op| matches!(op, TraceOp::PanelLoad { .. }))
            .unwrap();
        let loop_pos = ops
            .iter()
            .position(|op| matches!(op, TraceOp::Loop { .. }))
            .unwrap();
        let store_pos = ops
            .iter()
            .position(|op| matches!(op, TraceOp::PanelStore { .. }))
            .unwrap();
        assert!(
            panel_load_pos < loop_pos,
            "PanelLoad should appear before Loop"
        );
    ...
    fn test_interpreter_body_seq_wraps_multiple_steps() {
        ...
        if let Some(TraceOp::Loop { body, .. }) =
            ops.iter().find(|op| matches!(op, TraceOp::Loop { .. }))
        {
            let softmax_count = body
                .iter()
                .filter(|op| matches!(op, TraceOp::Softmax { .. }))
                .count();
            assert_eq!(
                softmax_count, 3,
                "Seq with 3 Softmax inside loop body should emit all 3, got {}",
                softmax_count
            );
        } else {
            panic!("Template should produce a Loop op");
    ...
    fn test_interpreter_trace_body_multi_step_produces_all_ops() {
        use crate::compiler::codegen::vm::algo_template::{
            AlgoStep, AlgoStrategy, AlgoTemplate, AlgoTraceStep, DeviceReq, ReduceKind, TraceBinOp,
            TraceUnaryOp,
        };
        ...
        assert!(
            ops.iter().any(|op| matches!(op, TraceOp::Add(_, _))),
    ...
    fn test_interpreter_seq_softmax_then_silu_order() {
        ...
        assert!(
            softmax_pos.unwrap() < sigmoid_pos.unwrap(),
    ...
    fn test_interpreter_trace_body_broadcast_scalar_slot() {
        ...
        let broadcast = ops
            .iter()
            .find(|op| matches!(op, TraceOp::BroadcastScalar { .. }));
        assert!(
            broadcast.is_some(),
            "Broadcast step should produce TraceOp::BroadcastScalar"
        );
