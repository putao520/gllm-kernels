...
mod tests {
    ...
    fn test_centroid_extraction_sharp() {
        ...
        let softmax: Vec<f32> = input
            .iter()
            .map(|&x| ((x - max_val).exp()) / exp_sum)
            .collect();

        let (centroid_idx, _max_prob) = softmax
            .iter()
            .enumerate()
