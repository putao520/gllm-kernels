...
fn detect_isa_features() -> IsaLevel {
    ...
    {
        IsaLevel::Avx512Amx
    } else if is_x86_feature_detected!("avx512fp16") {
        IsaLevel::Avx512Fp16
...
impl<E: Element> Kernels<E> for CpuKernels<E> {
    ...
    fn gemm_bt(&self, a: &[E], b_t: &[E], c: &mut [E], m: usize, n: usize, k: usize) {
        ...
        assert!(b_t.len() >= n * k);
        assert!(c.len() >= m * n);
        assert!((2..=32).contains(&m), "gemm_bt currently supports M=2..32");
    ...
    fn kquant_matmul(
        ...
        match quant_type {
            ...
            QuantType::Mxfp4 { .. } => unimplemented!("kquant_matmul does not support MXFP4; dedicated MXFP4 matmul path not yet wired (T-MXFP4-LOWER)"),
            QuantType::Nvfp4 => unimplemented!("kquant_matmul does not support NVFP4; use JIT codegen path"),
            QuantType::TQ1_0 | QuantType::TQ2_0 => unimplemented!("kquant_matmul does not support Ternary {:?}; JIT codegen path required", quant_type),
            QuantType::Fp8E4M3 | QuantType::Fp8E5M2 => unimplemented!("kquant_matmul does not support FP8 {:?}; use JIT codegen path", quant_type),
            QuantType::Bf16 | QuantType::F16 | QuantType::F32 =>
                unimplemented!("kquant_matmul does not support native float format {:?}, use standard GEMM", quant_type),
        }
    }
    ...
    fn classic_matmul(
        ...
        match quant_type {
            ...
            QuantType::Mxfp4 { .. } => unimplemented!("classic_matmul does not support MXFP4; dedicated MXFP4 matmul path not yet wired (T-MXFP4-LOWER)"),
            QuantType::Nvfp4 => unimplemented!("classic_matmul does not support NVFP4; use JIT codegen path"),
            QuantType::TQ1_0 | QuantType::TQ2_0 => unimplemented!("classic_matmul does not support Ternary {:?}; JIT codegen path required", quant_type),
            QuantType::Fp8E4M3 | QuantType::Fp8E5M2 => unimplemented!("classic_matmul does not support FP8 {:?}; use JIT codegen path", quant_type),
            QuantType::Bf16 | QuantType::F16 | QuantType::F32 =>
                unimplemented!("classic_matmul does not support native float format {:?}, use standard GEMM", quant_type),
        }
    }
    ...
    fn iq_matmul(
        ...
        match quant_type {
            ...
            QuantType::Mxfp4 { .. } => unimplemented!("iq_matmul does not support MXFP4; dedicated MXFP4 matmul path not yet wired (T-MXFP4-LOWER)"),
            QuantType::Nvfp4 => unimplemented!("iq_matmul does not support NVFP4; use JIT codegen path"),
            QuantType::TQ1_0 | QuantType::TQ2_0 => unimplemented!("iq_matmul does not support Ternary {:?}; JIT codegen path required", quant_type),
            QuantType::Fp8E4M3 | QuantType::Fp8E5M2 => unimplemented!("iq_matmul does not support FP8 {:?}; use JIT codegen path", quant_type),
            QuantType::Bf16 | QuantType::F16 | QuantType::F32 =>
                unimplemented!("iq_matmul does not support native float format {:?}, use standard GEMM", quant_type),
        }
    }
