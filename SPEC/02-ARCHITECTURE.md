...
│  ├── detect_backend()         → 运行时检测（CUDA>ROCm>Metal>CPU）    │
...
pub fn auto_select_backend() -> Arc<dyn Backend> {
    ...
    if rocm_available() { return Arc::new(RocmBackend::new()); }  // Linux (可扩展到 Windows)
    if metal_available() { return Arc::new(MetalBackend::new()); }  // macOS only
...
| topk | ✅ | ⚠️ CPU | ⚠️ CPU | ⚠️ CPU | Top-K 采样 |
| apply_temperature | ✅ | ⚠️ CPU | ⚠️ CPU | ⚠️ CPU | 温度缩放 |
| sample_tokens | ✅ | ⚠️ CPU | ⚠️ CPU | ⚠️ CPU | Token 采样 |
| argmax | ✅ | ⚠️ CPU | ⚠️ CPU | ⚠️ CPU | 贪婪解码 |
| moe_route | ✅ | ⚠️ CPU | ⚠️ CPU | ⚠️ CPU | MoE 路由 |
| compute_routing_logits | ✅ | ⚠️ CPU | ⚠️ CPU | ⚠️ CPU | 路由 logits |
| add_bias | ✅ | ⚠️ CPU | ⚠️ CPU | ⚠️ CPU | 偏置加法 |
| backend_type | ✅ | ✅ | ✅ | ✅ | 类型查询 |
...
| quantize | ✅ | ✅ | ⚠️ partial | ❌ | 量化操作 |
| dequantize | ✅ | ✅ | ⚠️ partial | ❌ | 反量化操作 |
| fused_attention | ✅ | ✅ | ✅ | ⚠️ fallback | 融合注意力 |
| moe_forward | ✅ | ⚠️ CPU | ⚠️ CPU | ⚠️ CPU | MoE 前向 |
...
4. **每个后端独立完整**：CUDA/ROCm/Metal/CPU 各自实现，无依赖

#### Backend trait 算子清单（18 个方法）

| # | 算子方法 | 用途 | GPU 支持 |
|---|----------|------|----------|
| 1 | `flash_attention()` | Flash Attention 计算 | ✅ CUDA/ROCm/Metal |
| 2 | `paged_attention()` | 分页注意力（KV Cache） | ✅ CUDA/ROCm/Metal |
| 3 | `softmax()` | Softmax 归一化 | ✅ CUDA/ROCm/Metal |
| 4 | `matmul()` | 矩阵乘法 | ✅ CUDA/ROCm/Metal |
...
| 11 | `argmax()` | 贪心解码 | CPU |
| 12 | `moe_route()` | MoE 稀疏路由 | ✅ CUDA/ROCm/Metal |
| 13 | `compute_routing_logits()` | MoE 路由 logits | CPU |
| 14 | `rms_norm()` | RMS 归一化 | ✅ CUDA/ROCm/Metal |
| 15 | `rms_norm_inplace()` | RMS 归一化（原地） | ✅ CUDA/ROCm/Metal |
| 16 | `silu()` | SiLU 激活 | ✅ CUDA/ROCm/Metal |
| 17 | `silu_inplace()` | SiLU 激活（原地） | ✅ CUDA/ROCm/Metal |
...
### ARCH-PARITY-001: 后端算子覆盖率 100% 铁律（🚨 FROZEN - 2026-01）

**核心要求**：所有 GPU 后端（CUDA/ROCm/Metal）必须对每个 Backend trait 算子提供原生 GPU 实现，CPU fallback 仅作为最后兜底。

#### 设计原则

| 原则 | 说明 | 理由 |
|------|------|------|
| **100% GPU 原生覆盖** | 每个算子都必须有 GPU kernel 实现 | CPU fallback 会破坏 GPU 流水线，性能损失巨大 |
| **CPU fallback 仅作兜底** | GPU kernel 加载失败时静默回退 | 保证功能可用性，但不应是常态 |
| **后端对等** | CUDA/ROCm/Metal 算子实现必须完全对等 | 避免某后端成为"二等公民" |
| **禁止选择性实现** | 不允许"这个算子太难，用 CPU 也行" | 违反全流程单后端原则 |

#### 必须实现的算子清单（18 + 4 = 22 个）

**Backend trait 方法（18 个）**：

| # | 算子 | CUDA | ROCm | Metal | 说明 |
|---|------|------|------|-------|------|
| 1 | `flash_attention` | ✅ | 🔴 需实现 | ✅ | 核心计算密集型 |
| 2 | `paged_attention` | ✅ | 🔴 需实现 | ✅ | KV Cache 管理 |
| 3 | `softmax` | ✅ | 🔴 需实现 | ✅ | 注意力计算 |
| 4 | `matmul` | ✅ | 🔴 需实现 | ✅ | **最核心算子** |
| 5 | `rope_precompute` | CPU | CPU | CPU | 预计算，CPU 足够 |
| 6 | `rope_apply` | ✅ | 🔴 需实现 | ✅ | 位置编码 |
| 7 | `rope_apply_inplace` | ✅ | 🔴 需实现 | ✅ | 位置编码（原地） |
| 8 | `topk` | ✅ | 🔴 需实现 | ✅ | 采样 |
| 9 | `apply_temperature` | CPU | CPU | CPU | 简单标量运算 |
| 10 | `sample_tokens` | CPU | CPU | CPU | 随机采样，CPU 更合适 |
| 11 | `argmax` | ✅ | 🔴 需实现 | ✅ | 贪心解码 |
| 12 | `moe_route` | ✅ | 🔴 需实现 | ✅ | MoE 路由 |
| 13 | `compute_routing_logits` | CPU | CPU | CPU | 逻辑简单 |
| 14 | `rms_norm` | ✅ | 🔴 需实现 | ✅ | 归一化 |
| 15 | `rms_norm_inplace` | ✅ | 🔴 需实现 | ✅ | 归一化（原地） |
| 16 | `silu` | ✅ | 🔴 需实现 | ✅ | 激活函数 |
| 17 | `silu_inplace` | ✅ | 🔴 需实现 | ✅ | 激活函数（原地） |
| 18 | `add_bias` | CPU | CPU | CPU | 简单向量加法 |

**量化算子（独立函数）**：

| # | 算子 | CUDA | ROCm | Metal | 说明 |
|---|------|------|------|-------|------|
| 19 | `q4_dequantize` | ✅ | 🔴 需实现 | ✅ | Q4 解量化 |
| 20 | `q8_dequantize` | ✅ | 🔴 需实现 | ✅ | Q8 解量化 |
| 21 | `awq_dequantize` | ✅ | 🔴 需实现 | ✅ | AWQ 解量化 |
| 22 | `q4_matmul` / `q8_matmul` | ✅ | 🔴 需实现 | ✅ | 量化矩阵乘法 |

#### ROCm 缺失算子修复计划

**当前状态**：ROCm 后端 7 个核心算子缺失 GPU 实现，直接调用 CPU fallback。

| 算子 | 当前状态 | 修复方式 | HIP Kernel 文件 |
|------|----------|----------|-----------------|
| `matmul` | ❌ 调用 cpu_matmul() | 创建 HIP matmul kernel | `hip_kernels/linear.hip` |
| `rms_norm` | ❌ 调用 cpu_rms_norm() | 创建 HIP rms_norm kernel | `hip_kernels/rms_norm.hip` |
| `silu` | ❌ 调用 cpu_silu() | 创建 HIP silu kernel | `hip_kernels/silu.hip` |
| `rms_norm_inplace` | ❌ 调用 CPU | 复用 rms_norm kernel | `hip_kernels/rms_norm.hip` |
| `silu_inplace` | ❌ 调用 CPU | 复用 silu kernel | `hip_kernels/silu.hip` |
| `q4_dequantize` | ❌ 调用 CPU | 创建 HIP quantized kernel | `hip_kernels/quantized.hip` |
| `awq_dequantize` | ❌ 调用 CPU | 创建 HIP awq kernel | `hip_kernels/quantized.hip` |

#### GPU Kernel 实现模式（HIP 示例）

每个算子的 GPU 实现必须遵循以下模式：

| 组件 | 说明 | 位置 |
|------|------|------|
| Kernel 源码 | HIP/CUDA/Metal 计算内核 | `hip_kernels/xxx.hip` |
| HSACO 二进制 | 预编译的 GPU 代码 | `hip_kernels/xxx.hsaco` |
| Rust Wrapper | OnceLock 懒加载 + 参数校验 | `hip_backend.rs` |
| Fallback 路径 | GPU kernel 加载失败时调用 CPU | `hip_backend.rs` |

#### 验收标准

- [ ] CUDA 后端：所有 18 个 Backend trait 方法 + 4 个量化算子有 GPU 实现
- [ ] ROCm 后端：所有 18 个 Backend trait 方法 + 4 个量化算子有 GPU 实现
- [ ] Metal 后端：所有 18 个 Backend trait 方法 + 4 个量化算子有 GPU 实现
- [ ] CPU fallback 仅在 GPU kernel 加载失败时触发
- [ ] 默认推理路径无任何 CPU fallback 调用

---

### ARCH-TRANSFER-001: 零冗余数据传输（🚨 FROZEN - 2026-01）

**核心要求**：推理过程中消除所有非必要的 GPU↔CPU 数据传输，数据类型转换和中间计算必须在 GPU 上完成。

#### 设计原则

| 原则 | 说明 | 理由 |
|------|------|------|
| **GPU 上做类型转换** | FP16→F32 等转换必须在 GPU 执行 | PCIe 带宽是瓶颈，GPU 类型转换几乎免费 |
| **中间结果保持 GPU** | 量化解码结果、激活函数输出保持在显存 | 避免 readback + upload 的双重开销 |
| **最小化传输次数** | 整个推理过程只有输入上传和输出下载 | 每次 PCIe 传输都有 ~100-400μs 延迟 |
| **Pipeline 不中断** | 激活函数不能打断 GPU 流水线 | CPU 执行会强制同步，破坏并行 |
...
| 阶段 | 数据位置 | 操作 |
|------|----------|------|
| 输入 | Host → GPU | 一次 upload |
| 推理全程 | GPU | 所有计算在 GPU 完成 |
| 输出 | GPU → Host | 一次 readback |

**禁止模式（冗余传输）**：

| 违规模式 | 问题 | 修复方式 |
|----------|------|----------|
| FP16 数据下载到 CPU 做 F32 转换后再上传 | 双重 PCIe 开销 | GPU kernel 内做类型转换 |
| 量化解码后立即 readback | 解码结果应继续参与计算 | 保持在 GPU，用于后续 matmul |
| 激活函数（silu/gelu）在 CPU 执行 | 强制同步，打断流水线 | 使用 GPU silu/gelu kernel |
| 中间 tensor 下载做 debug 打印 | 生产环境不应有 | 移除或条件编译 |

#### 数据流拓扑

```
Host Memory                    GPU Memory (显存)
─────────────────────────────────────────────────────────
                               ┌─────────────────────────┐
  input_tokens ──────upload────▶│ embedding              │
                               │    ↓                    │
                               │ rms_norm (GPU)          │
                               │    ↓                    │
                               │ linear (GPU)            │
                               ...
                               │ final_norm (GPU)        │
                               │    ↓                    │
  output_logits ◄──readback────│ lm_head (GPU)          │
                               └─────────────────────────┘

❌ 禁止的传输（虚线表示）：
  - 任何中间层的 readback
  - 任何类型转换的 download/upload
  - 任何 debug 目的的数据读取（生产环境）
```
...
| 转换类型 | 执行位置 | 实现方式 |
|----------|----------|----------|
| FP16 → F32 | GPU | Kernel 内部 `__half2float()` |
| BF16 → F32 | GPU | Kernel 内部位运算 |
| Q4 → FP16/F32 | GPU | Dequantize kernel |
| Q8 → FP16/F32 | GPU | Dequantize kernel |
| AWQ → FP16/F32 | GPU | AWQ dequantize kernel |
...
❌ 禁止：q4_dequantize 后 readback 到 CPU
✅ 正确：解量化结果直接用于 GPU matmul
```

#### 验收标准

- [ ] FP16↔F32 转换在 GPU kernel 内完成
- [ ] 量化解码结果保持在 GPU 显存
- [ ] 激活函数（silu/gelu）使用 GPU kernel
- [ ] 整个推理过程只有 1 次 upload + 1 次 readback
- [ ] 无任何中间层的 GPU→CPU→GPU 往返

---

### ARCH-RUNTIME-001: 运行时模型管理（🚨 FROZEN - 2026-01）

**核心要求**：支持多模型动态加载/卸载，基于 GPU 显存容量自动决策，提供 OOM 恢复机制。

#### 设计原则

| 原则 | 说明 | 理由 |
|------|------|------|
| **延迟加载** | 模型先加载到内存，首次推理时才传输到显存 | 减少启动时间，按需使用显存 |
| **LRU 淘汰** | 显存不足时自动卸载最久未使用的模型 | 最大化显存利用率 |
| **OOM 恢复** | 显存分配失败时自动卸载模型重试 | 避免进程崩溃 |
| **显存预算** | 可配置显存使用上限 | 为系统/其他任务预留空间 |
...
| 状态 | 权重位置 | KV Cache | 可推理 |
|------|----------|----------|--------|
| Unloaded | 无 | 无 | ❌ |
| Memory | 主内存 | 无 | ❌ |
| GPU | 显存 | 显存 | ✅ |
| Evicted | 主内存 | 已清除 | ❌（需 promote） |

#### 显存管理 API

| API | 参数 | 返回 | 说明 |
|-----|------|------|------|
| `get_gpu_memory_info()` | device_id | (total, free, used) | 查询显存状态 |
| `set_memory_budget()` | max_bytes | Result | 设置显存预算上限 |
| `get_loaded_models()` | - | Vec<ModelInfo> | 获取已加载模型列表 |
| `unload_model()` | model_id | Result | 手动卸载指定模型 |
| `evict_lru()` | min_free_bytes | Result | LRU 淘汰直到满足空间需求 |

#### LRU 缓存管理

| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `max_cached_models` | usize | 3 | 最大缓存模型数 |
| `memory_budget_percent` | f32 | 0.9 | 显存预算占比 |
| `eviction_threshold` | f32 | 0.85 | 触发淘汰的显存占用阈值 |
| `min_free_memory` | usize | 512MB | 最小保留空闲显存 |
...
尝试分配显存
    ...
    │ 3. 按 LRU 顺序逐个卸载模型         │
    │    - 每次卸载后检查是否满足需求    │
    │    - 最多卸载 max_eviction_rounds  │
    │ 4. 重试分配                        │
    │    - 成功：继续推理                │
    │    - 失败：返回 OutOfMemory 错误   │
    └────────────────────────────────────┘
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `model_id` | String | 唯一标识（repo_name + revision） |
| `state` | ModelState | 当前状态（Unloaded/Memory/GPU/Evicted） |
| `memory_size` | usize | 内存占用（字节） |
| `gpu_memory_size` | usize | 显存占用（字节） |
| `last_used` | Instant | 最后使用时间（LRU 排序） |
| `load_count` | usize | 加载次数（统计） |

#### 验收标准

- [ ] `get_gpu_memory_info()` 返回准确的显存信息
- [ ] 模型首次推理时延迟上传到 GPU
- [ ] 显存不足时自动 LRU 淘汰
- [ ] OOM 发生时自动恢复（卸载模型重试）
- [ ] 支持手动 `unload_model()` API
- [ ] 支持配置显存预算上限
- [ ] 多模型切换无内存泄漏
...
- **Fat Binary**：编译时嵌入所有平台的预编译中间态（PTX/HSACO/metallib）
...
.metal ──xcrun metallib──→ metallib ─┘           ↓
                            ...
                            ┌──────────────────┼──────────────────┐
                            ↓                  ↓                  ↓
                      CUDA Driver         HSA Runtime        Metal Framework
                      (PTX→SASS)         (HSACO→执行)       (metallib→执行)
```
...
**🚨 铁律约束**：
- ✅ **中间态运行时加载**：PTX/HSACO/metallib 在运行时由 Driver/Runtime 加载执行
- ❌ **禁止源码运行时编译**：不允许从 .cu/.hip/.metal 源码运行时编译
- ❌ **禁止环境变量配置**：删除所有 `GLLM_*` 环境变量
- ❌ **禁止配置文件**：无需任何外部配置文件
- ✅ **自动检测硬件**：启动时自动检测 GPU 类型
- ✅ **自动选择最优**：按检测顺序（CUDA > ROCm > Metal > CPU）选择

**中间态 vs 源码编译的区别**：

| 类型 | 定义 | 示例 | 允许 |
|------|------|------|------|
| 中间态加载 | 加载预编译的中间表示 | PTX→SASS, HSACO→执行, metallib→执行 | ✅ |
...
- **零配置**：不读取任何环境变量
- **确定性顺序**：按性能优先级检测（CUDA > ROCm > Metal > CPU）
