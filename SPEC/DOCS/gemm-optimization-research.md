...
- AMX 是 CPU 平台的微内核加速能力，不是独立 Platform（见 SPEC/04 §3.1）
- 生产路径由 JIT 编译器 Phase 3 自动生成 AMX tile 指令（ARCH-JIT-FIRST）
- Rust 目前没有稳定的 AMX intrinsic，JIT 需要通过 iced-x86 直接编码 TDPBF16PS 等指令
...
- 如果实现，生产路径由 JIT 编译器 Phase 3 的 aarch64 后端自动生成 SVE 指令（ARCH-JIT-FIRST）
- 注意：dynasm-rs 暂不支持 SVE（见 SPEC/02 §8.6），需要评估替代方案
...
5. **VNNI INT8 路径**：JIT Phase 3 生成 `vpdpbusd` 指令，4× 吞吐提升
6. **BF16 路径**：JIT Phase 3 生成 `vdpbf16ps` 指令，2× 吞吐提升

### P2: 长期改进

7. **AMX 支持**：JIT Phase 3 生成 tile 指令（TDPBF16PS 等），见 SPEC/04 §3.1
8. **SVE 支持**：JIT aarch64 后端生成 SVE 可变宽度指令
